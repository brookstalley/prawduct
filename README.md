...
Everything else is governed by 26 principles and four methodology guides that stay in context via CLAUDE.md.
...
- **Learnings fire where the mistake gets made** — rules that used to wait in a file to be read now print at the command that runs at the moment they apply, in any language rather than only Python; and every agent turn closes with a fixed three-line block — state, disposition, clear-verdict
