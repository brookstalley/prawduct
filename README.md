...
- **The gates stopped costing more than the work they guard** — asking whether a review was needed ran 29–120 s per call in a live session, two of them killed by a 2-minute timeout; a memoized coverage verdict makes it 20.0 s → 0.35 s. And a base sync no longer voids review coverage a branch already earned: both the PR and session-end gates compute a **transfer** when the judgeable files are byte-identical across the two spans, so re-syncing your base stops buying a re-review
