...
### When a review finds the SAME class twice, stop fixing instances and enumerate the domain — two spot fixes in two rounds is one missing act, not two mistakes. List every state the thing can be in and make each a case. Tell: your fix cites the reviewer's example

### Hand-verifying at the terminal leaves nothing behind — the corpus you ran the grep against IS a fixture, so make it one in the same breath. A guard warning over every repo's real data shipped with only its positive case asserted. Tell: "I checked it against the real file" with no test naming it

### A docstring written in the same keystroke as its code describes the design you INTEND, not the code you shipped — "the one reader", "every surface", "always". Twice in one session, both wrong when written. Before a sentence claims reach, grep for the callers; a claim about scope is checkable in seconds and unfalsifiable once it ships

### The prose that REPLACES a deleted control is load-bearing logic — pin every rendered branch of it. A message covering two states must be DERIVED from the state, never written for the one in mind: a sentence asserting "no plan has chunks left" met an operator whose plans had them. Tell: you changed a user-facing string and no assertion names it
...
### A zero from a scan is suspicious until the scan is shown able to return non-zero — `getattr(x, "body", "")` on a field the dataclass lacks measured nothing and reported clean. Before believing a count, feed it a case you know it should catch. Tell: a survey that confirms exactly what you hoped
...
### When a trim is justified by the surrounding prose's OWN instruction, run the suite before trusting it — the dangerous cut is the one the file appears to endorse, and a sentence that restates a nearby rule where the reader SKIPS it is placement, not a copy
...
### Changing HOW data ARRIVES silently re-scopes every aggregate over it — the code still computes, the value still looks plausible, and nothing fails, so no test catches it. After a switch from full scan to incremental (or batch to streaming, snapshot to log), walk every aggregate, watermark and age over that data and ask what each one MEANS now, not whether it still computes. **Consumer side, the harder half:** a field ADDED to a store creates that boundary in every historical series over it, long after the change is forgotten — so before comparing an aggregate across time, measure when each field it reads BEGAN being written. A verify empty rate from `findings` alone rose 25→57→83%, which was the `observations` array starting 2026-09-16; three readings in one session took it, one promoted into a handoff as the next build's case; the tool reported it separately throughout. Tell: your series has a trend and you never asked when its inputs began existing.
...
### A rule that LOWERS a severity outranks every rule that raises one unless you say so — state the floor with the ceiling, in one sentence, or the suppression quietly becomes the file's highest authority. The exits that LIFT a ceiling are not the severities it must never touch. Tell: your new rule caps a severity and all you wrote next was how to escape the cap

### Scrub the WHOLE diff before dispatching a review, tests and their comments included, and scrub a grep-able ban BY GREP — a rule you just wrote is one you are still violating elsewhere in the same commit, re-reading missed it twice on one branch, and a reviewer's list of instances is a sample rather than a census. Tell: you scrubbed by re-reading

### "Fail closed" means the CHANNEL's blocking value, not merely a non-zero one — mapping every refusal to a generic error code fails OPEN wherever the contract reads a SPECIFIC code as "block". Check the contract for the surface the refusal can REACH, not the one you were writing. Tell: you wrote "a refused gate is a blocked gate" and never opened the exit-code table

### A recommendation an advisory prints ships to every consumer's repo, so test its EFFECT, not just that it fires — make the change on a fixture, observe the promised outcome, and keep the un-made case as the control. Tell: every test you wrote asserts on the advisory object and none on what following it does

### A `try/except` around a producer that RETURNS its degraded states guards nothing — and the comment above it will read as if it does, so the intent survives while the mechanism does not. Read what the callee actually does on its bad paths before writing the guard, and answer the returned states where the read already is. Tell: your `except` names exception types the producer's docstring never mentions raising


### A correct decision defended by an unread mechanism is still a defect — when you write the *reason* for a choice ("X forces this", "that span is a superset"), open X first, because review checks the code against the claim and rarely the claim itself, so a false reason outlives the round. **A composing gate SHORT-CIRCUITS**, so the clause you were reading may never run: `test-status` reported `current` over a tree that broke the suite, and the missing `suite_coupled_prefixes` entry — the mechanism most recently in hand — survived a live reproduction, a six-way mutation sweep and an OWNER DECISION before an end-to-end check disproved it, because `tests_are_current` RETURNED on session-freshness without examining the tree at all — past tense from #767, which asks the tree clause unconditionally and labels which disjunct answered. The instance is repaired; the rule is not about it. What the repair did and did not change has ONE home, in `reviews.md`'s live-review rule, where the behaviour is met. Read the function that RETURNS the verdict, not the data you assume it consults, and make the acceptance test "does the gate answer DIFFERENTLY", never "is my new input correct". Worst when you hand it over as options: a wrong model then propagates into someone else's decision. Tell: your justification names a gate, flag or span you have not opened — or your explanation names the mechanism you were reading most recently
...
### When an addition would breach a prose token ceiling, the only two moves are pay-in-place from genuine duplication or DECLARE a raise with its reason — never trim until it fits. A ceiling forces a decision; resolving it by trimming spends whichever clause is least defended, and the clause no test asserts is the expensive one. #644's new BLOCKING obligation was funded by trimming `goals-1-3.md`, and the trim took both the clause `test_record_lint` requires on every reviewer surface AND the reviewer-independence sentence nothing guarded. Tell: your diff holds a measured count constant while adding a new obligation

### In parallel multi-agent work the dominant integration cost is PROSE that another agent's code falsified, not merge conflicts — nine worktree-isolated agents over ~9,600 insertions produced one conflict and three stale assertions no test could catch, one of which cited the very item that falsified it (`pr/SKILL.md` said resolve-base "does not consult origin/HEAD (#254)"). File-disjointness prevents collisions; it does nothing for a claim about a mechanism someone else is concurrently changing. Tell: an agent's file set excludes the file that DESCRIBES what it just changed

### Archiving an unmerged branch as "intent captured in the backlog" must verify the IMPLEMENTATION landed, not that an item exists — the two are independent, and the item's own body may read past-tense as if it shipped (that is how it got archived). Nine branches were tagged in one pass; ~11,000 lines of already-Critic-reviewed work went with them, including 775 lines of tests. Tell: you are closing or archiving on the strength of a description rather than a diff

### A closing block asserting that state exists ONLY in context is the trigger to WRITE IT DOWN, not a mitigation — `DO NOT CLEAR` asks the user to change their behavior, while `.handoff-notes.md` (which only `/clear` consumes) is the agent changing its own, and only the second is free of their compliance. **Bounded, because this rule's own remedy has since misfired:** `.handoff-notes.md` carries state one hop, to the next `/clear` that consumes it — so it is the right home for "what the next batch needs to know" and the WRONG one for anything that must outlive the session. An accepted review deferral, an owed backlog close, a decision to revisit: those need a home that survives, which is a backlog item or an artifact. A build plan shipped asserting three accepted deferrals "are carried in `.prawduct/.handoff-notes.md`" when that file did not exist, was gitignored, and would have been consumed anyway — a drop with a sentence in front of it, caught by a PR review as a warning. Tell: you are writing the handoff note to discharge an obligation rather than to brief a successor. Twelve owed backlog closes were left unwritten behind an explicit warning that nothing on disk recorded them; the session was cleared and the debt survived solely because the user pasted the block back as text. Tell: your own words are "nothing else on disk records this" and you are about to end the turn

### A precondition recorded only in PROSE is never re-evaluated when it is discharged — wire it as the machine-readable dependency the tool already honours (`blocked_by`, which `pick` re-checks on every call), because nothing revisits an item whose blocker quietly closed. Three items at the 2026-09-02 design-queue triage had sat for weeks on conditions already met — #237 behind two guardrails that had shipped, the janitor's skip clause behind a #529 that had closed, #677 describing gates that had landed before it was filed — and a stale REASON under a coincidentally-true conclusion is the hardest kind to notice. Tell: you are writing "GATED by X" into a body, and nothing but a human reader will ever check X again

### "Why is this a problem NOW?" is answered by a TREND, never by the instance — a fail-open mechanism is fail-open every day, so if nothing about it changed, what changed is the surface it fails open over, and that reframes the fix class rather than refining it. Two CI reds in a day read as a gate defect; measured, `tests/`'s real-repo path anchors had gone 51 -> 201 since July and the corpora those tests sweep 63 -> 265 files, which multiply into roughly a 17x growth in "edit a doc after your suite run, break the suite" surface. Two of the three failing tests were born the day before they failed; the third was a month old and failed on a NEW file because it is parametrized over a glob, so its failure surface grows with the repo while nobody edits the test. The consequence is decisive: any remedy that a human maintains by hand loses a race growing 25 anchors a week, so the trend selects the mechanism (observe what the run READ) over the instance fix (declare another prefix). Tell: you are about to fix the instance and cannot say what changed on the day it started failing

### When GitHub says CONFLICTING and a local trial merge says CLEAN, look for a `.gitattributes` merge driver — the server applies none, so a branch stale enough to also touch `.prawduct/change-log.md` (`merge=union`) merges everywhere except on GitHub, and `gh pr merge` refuses with *the merge commit cannot be cleanly created*. Land the union resolution as a real sync commit on the branch and push it; nothing else makes the two oracles agree. Tell: `git merge-tree --write-tree` exits 0 and the PR is red

### A refusal hands the caller a REPLACEMENT route, and it is checked by its PROPERTIES, never its name — on both axes, because a mode name carries neither. **Span:** a mode name carries goal count, not interval, so a demotion can hand back a span NARROWER than the one just refused for being too wide; have the refusal name the mode rather than let the reader pick. **Cost:** a refusal does not end the obligation that prompted the call, so whatever still-standing gate the caller meets next is the real price, and it can exceed what was refused — #167's exit 5 declined a `verify-resolutions` (median 300s) and, because `is_self_inflicted_verify` fires only on a NON-EMPTY judgeable delta while `review_edges` composes only `kind == "review"` facts, left a gap whose sole non-`--force` route was `cumulative` (median 720s): a saving of 5 minutes that costs 12, found by two independent reviewers and confirmed by one `review-stats` call the design never ran. The deeper half of the cost axis is a STRENGTH MISMATCH — the evidence behind that refusal (`diagnose_fix_churn`, whose own docstring says it rules out work in a file the review never saw, *not new work written into one it named*) is FILE-level, which is advisory strength, while a refusal is authority; so the fix is never to add machinery letting weak evidence clear a gate, it is to leave the advice as advice. Tells: a refusal tells the caller to re-dispatch and does not say as what; or you can state what your guard saves but not what the caller pays the moment after it fires; or you are adding a mechanism so a predicate you already have can carry more weight than it was built for

### A derived table's INTERNAL CONSISTENCY is evidence about its arithmetic and says nothing about its CORPUS — and checking it feels exactly like verifying the report. Reconcile the INPUT SET first: how many things were scanned, and can the scan reach the ones you would not notice missing? A fleet instrument globbed `*/.prawduct/.governance-ledger.jsonl`, found 17 of 20, and every published digit was wrong; an independent review verified the table "sums to 2,225 runs and 291.3h, every share rounds correctly" and it did, because sums and shares are computed from whatever arrived. The excluded set is rarely random — here it was exactly the delegated work, which runs in worktrees INSIDE a repo, so the rows most likely to differ were the ones systematically absent. **Bound a corpus by the PROPERTY (a ledger exists) rather than by a container (depth, a path prefix, a hand-kept list of filenames), and make the corpus SIZE an output** so "every X on this machine" is checkable rather than asserted — a completeness claim nobody can check is the one that goes stale silently. Same session, the same shape in a test: a guard against an unnormalised `fromisoformat` policed a hardcoded two-name `TOOLS` tuple, so the tool that shipped that exact crash was outside the scan while reading as covered. Tell: your prose says "every" / "all" / "the whole fleet" about a set some code assembled, and you have verified the OUTPUT reconciles but never asked the assembler what it skipped
