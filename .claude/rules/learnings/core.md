...
### A control that REFUSES expensive work is priced against the route its refusal forces, never the one it declines — a refusal does not end the obligation that prompted the call, so whatever still-standing gate the caller meets next is the real cost, and it can exceed what was refused. #167's exit 5 declined a `verify-resolutions` (median 300s) and, because `is_self_inflicted_verify` fires only on a NON-EMPTY judgeable delta while `review_edges` composes only `kind == "review"` facts, left a gap whose sole non-`--force` route was `cumulative` (median 720s) — a saving of 5 minutes that costs 12, found by two independent reviewers and confirmed by one `review-stats` call that the design never ran. Sibling of [[A fallback must be checked against the SIZE of the interval it replaces]] on the cost axis rather than the span axis, and the deeper half is a strength mismatch: the evidence behind it (`diagnose_fix_churn`, whose own docstring says it rules out work in a file the review never saw, *not new work written into one it named*) is FILE-level, which is advisory strength, and a refusal is authority — so the fix is never to add machinery that lets weak evidence clear a gate, it is to leave the advice as advice. Tell: you are adding a mechanism so that a predicate you already have can carry more weight than it was built for — or you can state what your guard saves but not what the caller pays immediately after it fires
