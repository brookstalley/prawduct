...
### When a gate returns a verdict you believe is wrong, read the FUNCTION THAT RETURNS IT before blaming the data it consults — a gate that composes clauses SHORT-CIRCUITS, so the clause you happen to know about may never run, and a diagnosis built on it is coherent, checkable and wrong. `test-status` said `current` over a tree that broke the suite; I had just been reading `suite_coupled_prefixes`, so the missing entry was the available explanation, and it survived a live reproduction, a mutation sweep and an owner decision before an end-to-end check disproved it: `tests_are_current` returns on session-freshness WITHOUT examining the tree, so no setting of that declaration could have changed the answer. The damage is not the wasted hour — a wrong model put into a scoped set of options propagates into the OWNER's decision, and they authorise a fix that cannot do what they picked it for. Ask the gate to answer DIFFERENTLY as the acceptance test, never "is my new input now correct": the guard was green and the mutants all died on an input the verdict never reads. Tell: your explanation names the mechanism you were reading most recently
