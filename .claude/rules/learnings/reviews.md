...
- Check WHICH interval a Critic mode reviews: `chunk` = HEAD tree→working tree, so committing first reviews an EMPTY interval; `cumulative` = a commit range, so NOT committing misses your work. The Signals interval line says. Tell: mode from the plan
