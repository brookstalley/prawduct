...
- **Deliverables:** `plugin/lib/backlog/core.py` (`_UPDATE_BLOCK` allowlist, block composition in `update_item`, `mark_reviewed` + injected `now` clock seam following `claim`'s precedent, `refs` param on `file_item`), `plugin/lib/backlog/cli.py` (`_run_update` valued/boolean flag sets, `_run_file`, `_HELP` usage text)
...
- **Deliverables:** `plugin/skills/backlog/SKILL.md` (the `update` line's "always set `reviewed:` on any touch" becomes explicit re-confirmation; triage step 2's `refs:` instruction and the ship workflow's `closed-by=` gain their real flag spellings), `documentation/backlog-service-data-model.md` (§2 block schema gains `refs`/`revisit`/`closed-by`/`reviewed` under the §7 additive-only-forever rule; §1.2 records which are writable and which stay import-only), `documentation/backlog-service-api-contract.md` (the `update` field list), `tests/test_backlog_instruction_surface.py` (the new check)
