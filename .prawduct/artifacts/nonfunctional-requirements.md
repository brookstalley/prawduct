...
- **State-file growth past its size threshold is surfaced as an advisory warning that prompts compaction — it is never a hard block or mechanical enforcement.**
  ...
  Status: steady-state.
  Rulings: **owner exception, 2026-09-24 (learnings-one-line): the learnings rules files leave this norm's scope.** `[DECISION: the Stop gate blocks a learnings rules file over its budget that grew, and any rule line breaking the one-line format; core.md's cap moves only with an owner_approved date | this norm's why is that blocking a session on a state file's size is disproportionate for advice about context weight. The learnings corpus is not advice the session may ignore: it is loaded into EVERY session, so its size is a cost every session pays, and it regrew four times behind advisory nudges and agent-raisable budgets (to 107KB here and 139KB in discodon). The owner ruled in-session: "We can't have giant learnings like that", with a hard cap that only the owner can raise. The v3.6.0 budget gate already blocked on these files with no decision recorded here; this records that departure rather than widening it silently | user can veto/override]` Every other state file (project-state, backlog, change-log) stays advisory under this norm, and the corpus is frozen rather than stopped: a session that does not grow it is never blocked by its size.
- **Review rigor is stage-keyed. The *inner stage* is any review of an uncommitted diff (`chunk`, `final`, `verify-resolutions`): it blocks only on the inner BLOCKING set below and reports everything else as an observation — pre-priced, carried to the boundary, never a finding that buys a round. The *boundary stage* is any review over merge-base…HEAD (`cumulative`) and the PR review: it runs the full severity table and is never skipped or inferred away. The failure direction is symmetric: an inner-stage review run at boundary rigor is a defect, priced in minutes and the rounds it manufactures; a boundary review run at inner rigor is a defect, priced in what ships. Unsure defaults to the inner-stage review of whatever interval exists.**
  ...
  Rulings: 2026-09-25, owner — **the inner stage is keyed on cycle position, not on commit order.** `[DECISION: a mid-plan review of the unreviewed interval is inner stage whether or not the builder committed first — covered frontier → working tree, or merge-base → working tree when nothing on the branch is reviewed; the boundary is merge-base…HEAD at the PR point (the plan complete, or its last chunk committed) | the statement's "uncommitted diff" read by its own why: committing a chunk before reviewing it moves the commit, not the review's place in the cycle, and pricing that review at boundary rigor is the defect the statement names — the 2026-09-25 sibling survey found 33 `cumulative` reviews that mode inference chose on a clean tree in the middle of a plan since 2026-09-20 | owner can veto/override]` Owner, answering the review-friction plan's stage fork: "Yes, key on cycle position". The statement above keeps its ratified words; the mode router (`lib/critic_mode.py`) and the interval's owner (`critic_consolidate.working_tree_interval_base`) implement the ruling.
