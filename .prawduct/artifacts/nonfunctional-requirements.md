...
- **Proportionality ratchets both ways. A control that has fired repeatedly and never produced a blocking finding is removed by default; keeping it requires a reason. Adding a control names the yield it expects **and emits that yield observably**, so there is something to measure it against later — a control whose findings are printed and forgotten satisfies the letter and defeats the point, since it can never be retired on evidence, only defended on principle.**
  ...
  Live exception: **doctor Health Checks #13 and #15** are recorded **bounded exceptions** to the emission arm — each names its expected yield and cannot emit it, because doctor has no fact-emitting path at all and building one for a single check is the accumulation this norm exists to stop. #13 is the learnings descent-obligation check (shipped 2026-08-02); **#15 is the running-plugin-vs-marketplace staleness check (added 2026-08-04**, decision block: `artifacts/build-plan-release-integrity.md` § `governed_by` → nonfunctional-requirements; **owner-ruled 2026-08-04** — "the proportionality is fine if we can't emit, keep the norm"). **The list is the registry: a control does not hold this exception by resembling one that does, and #15 was added here rather than claiming to inherit #13's** — an exception asserted inside the control that benefits from it is a governance change acting as its own witness. **The ruling grants the exception without relaxing the norm**, and the qualifying condition it names is narrow: the *surface* has no emission path at all, which is true of doctor and of nothing else today. Inconvenience does not qualify, and a third control does not join this list by resembling the two on it. Clock: `#563` — the trigger is *when doctor gains that path*, at which point both are its worked first cases. **That trigger is prose here and on the item, not a `revisit:` field, because the Issues backend has no write path for one**: the adapter's `update` deliberately strips a caller-pasted `prawduct:` block and re-appends the existing one (`lib/backlog/core.py` `_body_update_preserving_block`), and no op takes a `revisit` flag — the field is read only off the frozen markdown model (`lib/backlog/legacy.py`). So the walker is the janitor's Norm Health sweep reading #563, exactly as it is for every event-bound trigger (`probe_revisit_due` fires on *dated* values and is dark post-cutover regardless). The missing write path is filed as `#564`. Decision block: `artifacts/build-plan-drift-burndown.md` § `governed_by` → nonfunctional-requirements.
