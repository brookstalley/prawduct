...
- **Every issue written to the backlog store conforms to the issue standard's §1 title rules — on every write path, `file` / `update` / `import` / `file-upstream` alike. Where a source title does not conform, the agent rewrites it; a non-conforming title is not written.**
  ...
  Status: **steady-state** as of #614. Every write path refuses a non-conforming title: `file` and `update` through `core._title_refusal`, `import` through `migrate.preflight_titles`, which validates the whole corpus before its first write, and `file-upstream`'s send arm through `upstream.send`. The interim hand-conformance rule is retired — conformance now rests on a refusal rather than on the agent having read the standard.
  ...
  Mechanism: `issuefmt.lint_title` (`title-too-long` / `-too-short` / `-placeholder` / `-non-atomic`), blocking on every write path. Body lints stay WARN-only by the same ruling — a body budget blocking an edit to an unrelated field is the confirmation-fatigue shape `security-model.md`'s approval norm rejects.
  ...
  Ruled 2026-09-07, `file-upstream` is the fourth write path and is bound: the send arm refuses a non-conforming title before filing; the preview arm reports the same findings advisorily, because nothing is written there and an advisory finding is exactly what lets an author fix a title *before* approving it. Engaging this norm's why — the title is the handle every later reader triages by — the why binds *harder* here than in-repo: a non-collaborator filer cannot relabel or retitle after the fact, and the write does not come back. **One scope difference the refusal respects:** the upstream title convention is `[prawduct] <component>: <symptom>`, not §1's `area: summary`, so the area-prefix expectation does not apply to it; the budget, placeholder and atomicity rules do. This ruling widened the statement above from three named paths to four — the code already conformed, and the norm's text was describing the world before the path existed.
