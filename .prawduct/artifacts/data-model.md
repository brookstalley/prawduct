...
- **Every issue written to the backlog store conforms to the issue standard's §1 title rules — on every write path, `file` / `update` / `import` alike. Where a source title does not conform, the agent rewrites it; a non-conforming title is not written.**
  ...
  Status: in-transition — tracking item **#614**. `issuefmt._lint_title` implements all four checks today, but `file`/`update` treat them as advisory and `import` never calls them, so **all three paths are non-conforming**.
  Interim rule (while in-transition): the agent hand-conforms every title it writes to §1 — the lint is advisory today, so conformance rests on the agent reading the standard rather than on a refusal. A title that would fail the lint is rewritten before the write, not filed and fixed later. This is the whole reason the transition is not simply "wait for #614": the norm binds now; only its *mechanism* is pending.
  ...
  Mechanism: `issuefmt._lint_title` (`title-too-long` / `-too-short` / `-placeholder` / `-non-atomic`), blocking on the three write paths once #614 lands. Body lints stay WARN-only by the same ruling — a body budget blocking an edit to an unrelated field is the confirmation-fatigue shape `security-model.md`'s approval norm rejects.
