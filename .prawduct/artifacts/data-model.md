...
- **Every issue written to the backlog store conforms to the issue standard's §1 title rules — on every write path, `file` / `update` / `import` alike. Where a source title does not conform, the agent rewrites it; a non-conforming title is not written.**
  ...
  Status: in-transition — tracking item **#614**. `issuefmt._lint_title` implements all four checks today, but `file`/`update` treat them as advisory and `import` never calls them, so **all three paths are non-conforming**. Existing corpora are deliberately NOT retro-conformed: per the standard's §1 altitude rule, a scrub that rewrites titles item-by-item cannot see that three issues are one defect, so enforcement must ship together with the dedup sweep's shared-root-cause question or it entrenches an over-split backlog.
