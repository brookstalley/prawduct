...
- **Text emitted into a governed product names no prawduct-internal identifier.** Advisory fields, reviewer findings, and hook stdout/stderr carry the plain-language reason; requirement, chunk, and backlog ids stay on the non-emitted side — comments, docstrings, tests, build plans, learnings, and the instruction prose a skill reads but never speaks.
  ...
  Status: in-transition — tracking item **OBS-7M4D** (six emitted sites in `bin/prawduct-hook`, `lib/backlog/cli.py`, `lib/backlog/migrate.py`, `lib/critic_consolidate.py` still carry ids). **On `main` at v3.1.1 only the `bin/prawduct-hook` and `lib/critic_consolidate.py` sites exist** — the two `lib/backlog/` files are on `feature/backlog-service`, withheld from this release. OBS-7M4D's full site count is against that branch, not the shipped tree. Interim rule: emitted text a changeset **writes or edits** complies; sites the changeset does not touch wait for OBS-7M4D.
