...
- **Text emitted into a governed product names no prawduct-internal identifier.** Advisory fields, reviewer findings, and hook stdout/stderr carry the plain-language reason; requirement, chunk, and backlog ids stay on the non-emitted side — comments, docstrings, tests, build plans, learnings, and the instruction prose a skill reads but never speaks.
  ...
  Status: in-transition — tracking item **OBS-7M4D** (six emitted sites in `bin/prawduct-hook`, `lib/backlog/cli.py`, `lib/backlog/migrate.py`, `lib/critic_consolidate.py` still carry ids). **On `main` at v3.1.1 only the `bin/prawduct-hook` and `lib/critic_consolidate.py` sites exist.** The two `lib/backlog/` files were withheld from that release; as of 2026-07-21 they are merged and now live at `plugin/lib/backlog/`, so OBS-7M4D's full site count applies to the v3.2.0 tree rather than to a side branch. (Site inventory only — the norm's binding content is unchanged: still `in-transition`, still tracked by OBS-7M4D, same interim rule.) Interim rule: emitted text a changeset **writes or edits** complies; sites the changeset does not touch wait for OBS-7M4D.
