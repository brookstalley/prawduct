...
**Acceptance criteria**
- [ ] An agent-path worktree on branch `fix/real-thing` is NOT ephemeral; `critic-begin`,
      `test-evidence record`, `disposition` and `advisory dismiss` all proceed.
- [ ] An agent-path worktree on `worktree-agent-<hex>` is still ephemeral and still refuses all
      four, with the existing message.
- [ ] A `wf_*` workflow worktree is still ephemeral regardless of branch (workflow stages get no
      named branch; the path IS the identity there).
- [ ] An agent-path worktree with a detached HEAD is ephemeral (fail closed).
- [ ] An `EnterWorktree` session worktree under the same parent is unchanged — still governed.
- [ ] **The #649 decider:** two agent-path worktrees of one clone, each on its own real named
      branch, each record test evidence; each reads back its own counts and the clone's
      `.test-evidence.json` is untouched.
- [ ] Full suite green, no regressions against the baseline taken before this plan.
...
**Acceptance criteria**
- [ ] A fact recorded from an agent-path worktree on a real named branch does NOT read as
      ephemeral in `evidence status`, and is not counted in the wasted-cost sentence.
- [ ] A fact from a scratch-branch agent worktree still does.
- [ ] A pre-existing fact carrying no branch is not silently reclassified in either direction —
