...
- **Open against this chunk's design, raised 2026-08-19, awaiting owner response — recorded so none is silently dropped:**
  1. **`COMPLETE` has no scope.** "Complete of what?" A finished chunk in an unfinished plan will attract the label. Proposed rule: `COMPLETE` means nothing remains that this session knows to do; a known, unstarted next step is `YOUR TURN`. **Treat as required** — it prevents predictable misuse of the rarest label.
  2. **Step 1 of the agent procedure collides with `building.md` "Session Scope Discipline"** ("limit work cycles to 1-3 chunks"), which is an instruction to STOP where step 1 says keep going. Needs explicit subordination: step 1 yields to declared stopping points (scope discipline; a chunk boundary whose review has not run). **Treat as required** — shipping it unqualified erodes an existing gate, and the newer, more specific rule would win.
  3. **`RUNNING` makes a reliability promise the agent cannot keep.** It claims results arrive without the reader; a dead subagent, a timed-out monitor or a stalled remote queue produces no turn at all and strands them. Mitigation: `RUNNING` copy names its fallback (a bounded re-check), so the promise is bounded rather than absolute.
  4. **`RUNNING` ⇒ `DO NOT CLEAR`** is derivable — clearing destroys the thing being waited on — so a block claiming `RUNNING` + `SAFE TO CLEAR` is internally incoherent and a pin can say so. The converse does NOT hold: `COMPLETE` + `DO NOT CLEAR` is valid and common (finished but uncommitted).
  5. **Under the precedence rule the in-flight fact leaves the label**, so the copy must name what is running or that information is lost outright. Pin it as part of the precedence rule rather than leaving it to judgment.
  Also settled in that discussion and recorded here because it constrains the copy: a turn where something is running and a decision will be needed ONLY ONCE IT LANDS is `RUNNING`, not `YOUR TURN` — the next turn is genuinely machine-produced, and a summons with nothing yet to act on is what trains a reader to ignore the label. The forewarning belongs in the copy.
