...
governed_by:
  ...
  - artifact: observability-strategy
    dispositions:
      ...
      - "The governance ledger has a single writer (`ledger-append`); agents never hand-author it → inapplicable, because no chunk here writes, reads or reshapes a ledger event. Chunk 01 added `next_action` to the findings CACHE, which is a derived view regenerated by `critic-consolidate`, not a ledger line."
