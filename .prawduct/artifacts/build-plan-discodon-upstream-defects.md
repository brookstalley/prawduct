...
- [x] Chunk 01: CRT-M3F8 / CRT-4T7M — normalize file-less/blank `files` in critic-consolidate instead of fail-closing
- [x] Chunk 02: PDT-C6R4 / BLD-5J8N — verify-chunk-refs parses `## Chunk N (ID) — Name` and distinguishes parse-miss from ref-missing
- [x] Chunk 03: CRT-T9RX / CRT-7H2W — intent-aware verify-resolutions head anchor (structural) + dirty-tree diagnostic
- [x] Chunk 04: PDT-WT9K — critic-begin worktree visibility + mismatch-detection + refuse-on-unresolvable
...
- **Depends on:** none
- **Artifacts consumed:** discodon backlog CRT-M3F8 (local **CRT-4T7M**); verified sites `lib/critic_consolidate.py` (the `validate_partial` files check and `merge_findings`, which kept truthy `files` verbatim)
...
- **Artifacts consumed:** discodon backlog CRT-T9RX; local CRT-7H2W (recommended layered-pair fix, exact sites); `lib/coverage_algebra.py` (the two-target design preamble); `lib/critic_mode.py` (the `_committed_files_since` helper)
