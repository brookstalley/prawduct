...
- **Deliverables:**
  ...
  4. `methodology/building.md` — one concise pointer: when a chunk builds/changes an exposed API surface, the versioning decision is a day-one call (adding a version later is a breaking change for every consumer) — record it, don't default to silence. Keep within budget.
