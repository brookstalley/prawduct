...
governed_by:
  - artifact: architecture
    dispositions:
      ...
      - "every fact has one home; every other mention is a reference to it → conforms. Chunk 01's premise correction lands in `architecture.md` § What counts as a session boundary (the home) and `critic_marker.py`'s module docstring states the mechanism, not a second copy of the taxonomy. Chunks 02-03 keep the standing block's shape on its two ratified carriers and add no third"
...
- `[ASSUMPTION: the residual cost of not sweeping a fresh marker at /clear is a false "abandoned review" signal at the next Stop, bounded by the 30-minute TTL | MED impact | user can correct]` — verified that `critic-begin` does *not* refuse on an existing marker (`write_marker` overwrites), so a surviving marker cannot block a new review. `marker_present()` has no TTL, which is where the residual cost sits. Chunk 01 must state what it costs, not only what it buys.
...
- **Deliverables:** liveness-gated sweep on the `clear` source in `plugin/bin/prawduct-hook`, with `startup` unchanged; corrected premise in `plugin/lib/critic_marker.py`'s module docstring and in `architecture.md`'s boundary section — stating that the sweep's question is process death and that `clear` is where that diverges from the table's transcript test; the residual cost named where the next reader meets it
- **Tests:** boundary behaviour in the marker/session-boundary suite — fresh marker survives `clear`, expired marker is swept by `clear`, `startup` sweeps both, and a continuation still sweeps neither. Assert against a marker whose age is set by its embedded `started_at`, not by touching mtime, so the test exercises the real freshness signal rather than the fallback
- **Acceptance criteria:** suite green; the four boundary cases hold; reverting the dispatch change turns the fresh-marker case red
- **Type:** code
- **Done when:**
