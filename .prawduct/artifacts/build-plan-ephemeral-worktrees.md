...
governed_by:
  ...
  - artifact: nonfunctional-requirements
    dispositions:
      ...
      - "proportionality ratchets both ways — a control names the yield it expects and emits it observably → **conforms, and the yield is stated.** The guard's expected yield is refusals that would otherwise have been silent strands; each one prints its reason and its override, so a control that never fires is visibly removable later"
