...
governed_by:
  ...
  - artifact: data-model
    dispositions:
      ...
      - "a governance document reaches a terminal state, never deleted; live outranks archived → engaged at plan close, not by any chunk. The base is `develop`, so this plan is RETAINED live at the closing merge and archived at the `develop→main` release, where `plan-backfill` archives every plan whose scope the release tagged — archiving it in the closing PR would collide with that step, which refuses names already in `archive/`. Chunk 06 additionally strengthens the live-outranks-archived half — an archived plan's `branch:` frontmatter is ignored by the resolver, so moving a plan under `archive/` ends its claim"
