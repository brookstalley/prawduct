...
- **A governed product's content never leaves that product's own repository and owner. The backlog adapter reaches exactly the repo named in `backlog_service_repo`; the upstream bug channel is filesystem-local. Any cross-owner or public-plane filing surface is an owner decision, never an increment.**
  ...
  Ruling 2026-08-06 (owner) — **`documentation/prawduct-fable-audit-20260806.md` publishes cross-repo content into this public repo, and that is accepted.** Its §2c carries, from sibling repos that are private, verbatim `learnings.md` quotes with file:line citations, per-surface store sizes and review counts, a named governance-bypass narrative and a data-loss incident. Owner ruling: *"It's harmless… Nobody cares about these projects, they are not secret. All good. Accept."* Recorded rather than left implicit because this norm's own text makes a public-plane disclosure **an owner decision, never an increment** — so a decision is exactly what it needed, and this is it. The norm is **not weakened**: it still binds every *mechanised* surface, and `test_no_upstream_content_egress.py` is untouched.
  **What the incident changed — the jurisdiction, not the rule.** The document was committed and pushed before anyone noticed, and was caught by the PR reviewer rather than by any gate, because the mechanism inspects the plugin for a `file-upstream` **code** surface while the actual egress path was **an agent authoring prose**. That gap is a requirement input for the tracking item above, filed on **#194**: content minimization that sanitizes adapter payloads still leaves the author unguarded. Also recorded there, from this ruling rather than from principle, is the line the owner actually draws — this repo already names `hallucinote` and `discodon` and cites their metrics publicly in `.prawduct/archive/artifacts/`, so a redaction rule keyed on **repo names** would both over- and under-fire; what a future rule must distinguish is quoted-internal-content from aggregate metrics, and even that is subject to the owner's call rather than a substitute for it.
