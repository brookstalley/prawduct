...
- **A governed product's content never leaves that product's own repository and owner. The backlog adapter reaches exactly the repo named in `backlog_service_repo`; the upstream bug channel is filesystem-local. Any cross-owner or public-plane filing surface is an owner decision, never an increment.**
  ...
  **Scope clarification 2026-08-03 (owner ruling) — this norm is egress-scoped and does not reach same-tracker curation. No amendment: the norm's text, `Status:` and mechanism are unchanged.** Recorded here rather than only on the item, because this norm's `Status:` cites **BKL-7Q4M (#194)** and a reader following that pointer would never reach the ruling. #542 (`backlog adopt <id>`) rewrites the **title** of an untriaged issue a human filed **on prawduct's own tracker**, preserving the original verbatim in `original_title:`. That is not a departure from this norm and does not need one: what this norm governs is a governed product's content **leaving that product toward a foreign owner** — the outbound direction, which is precisely what makes the damage irreversible on someone else's public tracker. Adoption runs inbound, performed by the maintainer of the tracker the issue already sits on, and is ordinary maintainer curation. Precedent rather than a new posture: `documentation/backlog-service-issue-standard.md` §5 (the MG4 scrub) already does exactly this at batch scale — model proposes an `area:`-prefixed title, original preserved verbatim, owner approves, deterministic import writes. Consent for the **outbound** payload is untouched and remains XP5's per-report `ask-user` default (`documentation/backlog-service-upstream-filing.md` §4); the broad consent/attribution/taxonomy scope stays bounded on **#217**, where this clarification is also recorded. Stated so that a reviewer meeting the retitle at `adopt` build time does not flag a departure the owner already ruled out of scope.
