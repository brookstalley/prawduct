...
governed_by:
  - artifact: nonfunctional-requirements
    dispositions:
      ...
      - "review rigor is stage-keyed (inner reviews block only on the inner BLOCKING set; the boundary review runs everything) → conforms, added 2026-09-22 when the branch merged forward past the norm's ratification — Chunk 01's directive tells chunk and verify-resolutions reviewers how to LABEL a finding, which changes no severity and no stage's blocking set, so it composes with the norm rather than touching it; the branch's boundary review ran as a full cumulative"
  - artifact: architecture
    dispositions:
