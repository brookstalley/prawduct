...
- **Parallelization**: pytest-xdist via `pyproject.toml` (`-n 5 --dist loadfile`); `tests/conftest.py` groups same-directory tests onto one worker for fixture/state isolation. **Amended by owner ruling 2026-08-19**, `-n auto` → `-n 5`: `auto` takes every core, so a full-suite run owned the developer's machine for its whole duration — observed at a load average of 24 on a 10-core box with a run in flight. The worker count is now a **developer-ergonomics** setting, not a throughput one, which is why it is a fixed number rather than a fraction of `auto`. CI is unaffected in practice: GitHub's standard runners have 4 cores, so `auto` was already giving it fewer. Revisit if the suite's wall clock becomes the binding constraint on this machine.
