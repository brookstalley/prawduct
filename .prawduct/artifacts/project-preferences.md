...
| Backlog issues conform to the issue standard's §1 title rules on every write path | `data-model.md` | `issuefmt._lint_title` *(blocking once #614 lands; advisory today — all three write paths non-conforming)* | janitor | a run-on title fails silently and is the handle every later reader triages by |
