...
- **Backlog filing**: strong bias to **fix, not file**. File a new backlog item only when the finding is **both** (1) orthogonal to the current work *and* (2) medium or larger. Anything failing either test gets fixed — in this branch when it is in scope, or in its own small branch immediately after when it is orthogonal but small. *Why*: a filed item is a deferred decision that costs triage, re-reading and re-derivation later, and the backlog's value falls as it grows — 168 pending items make the next `pick` worse, not better. The two tests are the cases where filing genuinely beats fixing: orthogonal work would corrupt the current change's scope and review, and medium+ work needs requirements before code (Principle 6). Small-and-orthogonal is the trap this rule closes — it *feels* like a legitimate defer and is really just avoided work. Recording a finding as a **comment on an existing item** is not filing and is unrestricted. Retroactive: yes — an item already filed that fails these tests should be fixed and closed rather than left queued.
