...
- **Package manager**: pip. `pyproject.toml` holds pytest configuration and dev dependencies (`pytest`, `pytest-xdist`, `pytest-timeout`, `pyyaml`), and declares an empty package (`packages = []`) purely so the dev extra can be read in CI — the scripts themselves are standalone tools, and nothing here is published or importable-as-installed. **Install with `pip install -c constraints.txt ".[dev]"`, never the bare form.** The `-c` is not optional hygiene: `constraints.txt` is this repo's committed resolution, and clause 5 of its recorded intake policy binds *installs performed by an agent* as explicitly as it binds CI. This line is injected verbatim into every generated `.subagent-briefing.md`, so the unqualified form here was guidance telling an agent to do the thing the policy forbids
...
| Upstream intake terms (norm lives in `security-model.md` § Direction / § Upstream Dependencies) | Test | `tests/preferences/test_dependency_resolution_pinning.py` | CI (test) | the norm's real mechanism is the pinned resolution plus the guards over it; unlinked, deleting that test would de-enforce the norm with nothing to notice — which is the aspirational-rule shape the norm itself is about |
