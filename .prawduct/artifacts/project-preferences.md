...
- **Runtime dependency (opt-in): the `gh` CLI.** Required once a product sets `backlog_service_repo` (the backlog backend) **or** files a bug upstream — `file-upstream` rides the same `gh` seam to the plugin's pinned repo and is reachable with that scalar unset, so reachability is keyed on *a person acting*, never on the scalar. A product that does neither runs stdlib-only. `gh` owns the credential (`~/.config/gh`) and the adapter never manages a token — which is why opting in flips no structural characteristic. Rationale home is PRD O5; the norm that permits it is `architecture.md` § Direction *Local-first*, amended 2026-07-21 and again 2026-09-07 (admitting owner-approved upstream filing as the second surface). **This repo opted in on 2026-08-01** (v3.2.0 Chunk 06), so `gh` is a hard runtime dependency *here* and an optional one for the fleet.
...
- **Parallelization**: pytest-xdist via `pyproject.toml` (`-n 5 --dist loadfile`); `tests/conftest.py` groups same-directory tests onto one worker for fixture/state isolation. **Amended by owner ruling 2026-08-19**, `-n auto` → `-n 5`: `auto` takes every core, so a full-suite run owned the developer's machine for its whole duration — observed at a load average of 24 on a 10-core box with a run in flight. The worker count is now a **developer-ergonomics** setting, not a throughput one, which is why it is a fixed number rather than a fraction of `auto`. **CI inherits it and is therefore oversubscribed by one worker** on a 4-core runner — accepted deliberately, not overlooked. Overriding it in `tests.yml` would put a pytest flag in a workflow that carries none by norm, and no CI timing has been measured that would justify amending that norm; if runs begin timing out under contention, the fix is an environment-driven count *with* that evidence. Revisit also if the suite's wall clock becomes the binding constraint on this machine.
...
- **File organization**: `bin/` for entry-point scripts (`prawduct-hook`, `test-reference-verify`), `lib/` for the implementation package (a flat module per responsibility — `core.py`, `critic_mode.py`, `migrate_plugin.py`, `init_product.py`, `change_log.py`, `plan_index.py`, `plan_archive.py`, `plan_backfill.py`, `lifecycle_repair.py`, … — with `backlog/` the one sub-package, because it is the only surface large enough to earn one; the list here is illustrative and `ls plugin/lib/` is authoritative), `skills/` + `methodology/` + `hooks/` for the plugin governance surface, `templates/` for product artifact templates, `tests/` for tests
...
- **Backlog filing**: strong bias to **fix, not file**. File a new backlog item only when the finding is **both** (1) orthogonal to the current work *and* (2) medium or larger. Anything failing either test gets fixed — in this branch when it is in scope, or in its own small branch immediately after when it is orthogonal but small. *Why*: a filed item is a deferred decision that costs triage, re-reading and re-derivation later, and the backlog's value falls as it grows — 168 pending items make the next `pick` worse, not better. The two tests are the cases where filing genuinely beats fixing: orthogonal work would corrupt the current change's scope and review, and medium+ work needs requirements before code (Principle 6). Small-and-orthogonal is the trap this rule closes — it *feels* like a legitimate defer and is really just avoided work. Recording a finding as a **comment on an existing item** is not filing and is unrestricted. *Status*: steady-state. *Retroactivity*: **grandfather**, inventory ref `gh issue list --repo brookstalley/prawduct --state open --search "created:<2026-08-05"` (168 at birth) — every item filed before 2026-08-05 is exempt and will not be re-triaged against these tests. The query is the ref, not the count: once those issues change state the number stops reconstructing the exempt set; converging them is separate, unrequested work, and grandfather is the honest word for not intending to converge. The norm binds filings from 2026-08-05 forward. One site inside the birthing changeset is not exempt: #595, filed the same session under the old habit and failing test 2 (small), is to be fixed rather than left queued.
- **Model floor and coherence pass**: Sonnet is the minimum model for any prawduct work — main session or subagent; Haiku-class models are never used. Any work cycle whose substantive stages ran below Fable ends with a **Fable final-coherence pass** over the whole result before it lands. Both are **session-level operator choices** (`/model`, or an explicit subagent model set at spawn): reviewers still inherit the session model, and no machinery selects models at dispatch time — this preference chooses sessions, it does not reopen the "no intelligent model switching" pin (Enforcement row: reviewer model dispatch). *Why*: scaling cost down is legitimate for mechanical stages, but sub-frontier models miss exactly the cross-cutting incoherence prawduct exists to catch; a frontier pass at the whole-result view restores that check where it matters most. *Status*: steady-state (owner decision 2026-08-13, during purpose-and-cession Cycle 1). *Retroactivity*: none — binds cycles from 2026-08-13 forward.
...
- **Upstream filing**: ask-user (the default, kept deliberately rather than set to `never-file`. This repo **is** the pinned upstream target, so `file-upstream` refuses here on the no-self-file check whatever this row says — and `never-file` would only shadow that refusal, because `send()` consults the preference first. `self-file` is the better of the two diagnostics: it names the remedy, which is that prawduct's own bugs are filed with `/prawduct:backlog add`. The row is written anyway so the three states are discoverable in the repo that defines them.)
...
| Upstream filing holds its adapter contract — XP7's four guarantees plus XP5's consent gate (norm lives in `security-model.md` § Direction, steady-state) | Test | `tests/preferences/test_no_upstream_content_egress.py` | CI (test) | a private repo filing into prawduct's public tracker crosses a trust boundary irreversibly, so the five checks — pinned target, authentication, per-filing owner approval of the exact bytes, no self-file, preference honoured — are what make the surface safe to have, and each must stay independently falsifiable |
...
| No surface promises that a *continuation* (`resume`/`compact`/`fork`) resets session state | Test | `tests/preferences/test_session_boundary_prose.py` | CI (test) | the boundary/continuation split was re-asserted wrongly on four consecutive review rounds; the pin catches the copied *phrasing*, which is how every recurrence actually happened, and carries its own non-vacuity check |
| Every surface that tells a builder whether to spend a review round names the free-interval answer | Test | `tests/preferences/test_free_interval_prose.py` | CI (test) | `critic-begin` refuses a round the coverage gate would not require, but a builder reading an unconditional instruction spends it anyway — the measured waste was builders following these instructions correctly |
| Every primitive the code enumerates has a row in the registry artifact that owns it | Test | `tests/preferences/test_registry_completeness.py` | CI (test) | a stale registry is cheap to fix and expensive to *notice*, so each one costs a review round; the check is a set-difference against the code's own enumeration, and it deliberately covers only the registries with an enumerated left-hand side |
...
| Backlog issues conform to the issue standard's §1 title rules on every write path | `data-model.md` | `issuefmt.lint_title` — blocking on all three write paths (`core._title_refusal` for `file`/`update`, `migrate.preflight_titles` for `import`) | janitor | a run-on title fails silently and is the handle every later reader triages by |
...
| Whole-surface semver; internal CLI unversioned; evidence store schema-versioned (`api_versioning_approach`) | `api-contract.md` | Critic + `evidence status` exit 2 | janitor | one cache key; the CLI is internal **except the two-command "Published surfaces" group**, which is `stable` for third parties (ruling 2026-08-02) |
...
| Review rigor is stage-keyed — the inner stage blocks only on ships-broken; the boundary runs everything and is never skipped; unsure defaults to the cheaper inner review | `nonfunctional-requirements.md` | Critic (`stage` on the review manifest, once carried) + `tests/test_v5_methodology.py` (the retired-sentence grep) | janitor (`review-stats` by stage, once carried) | mid-chunk observations were buying rounds at boundary price; 80% of recent verify rounds returned nothing |
