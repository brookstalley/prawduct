...
| Proportionality ratchets both ways — a control that never catches anything is removed by default | `nonfunctional-requirements.md` | Critic *(no mechanical hook — the yield query is the janitor's)* | janitor | additions are each justified, so argument alone never reverses the drift |
| Gate cost scales with the diff, not the repo (repo-scale costs cached once) | `nonfunctional-requirements.md` | Critic | janitor | consumers run ~20x this repo and are where the benefit lands |
