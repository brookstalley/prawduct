...
**Chunk 01 built** on `feat/advisory-actionability` off `develop` at `596d761`. Suite green, with evidence recorded from the declared JUnit command (the count lives in `.test-evidence.json`, not restated here — a literal total in prose is the part that goes stale). The ordering algorithm changed once during the chunk and the reason is worth carrying forward: a ready-queue toposort was written first, and the fixture render of the four real advisories showed it demoting the `warn` below three unrelated `info` entries — one edge globally re-sorting the block. It is now a pull-up (each advisory emitted after its own prerequisites, otherwise in arrival order), pinned by `test_one_edge_does_not_demote_the_warn_below_unrelated_infos`. **Chunk 01 closed** at `99adc3b` — reviewed `cumulative` (1 blocking, 4 warnings), dispositioned in one commit, then `verify-resolutions` cleared all five and its own four prose findings were fixed in a second. Zero blocking outstanding.

Two things the review changed that Chunk 02 depends on. **`reconcile` now refreshes an active advisory's derived text on every re-fire** (`_DERIVED_DISPLAY_FIELDS`), preserving id, `triggered_at` and sticky dismissals — without it Chunk 02's copy would reach only repos that had not yet tripped each probe, i.e. never the ones that have been living with an advisory longest. That fix also closed a latent fleet-wide defect: probes are told to keep volatile counts out of `evidence` so the id stays stable, and the old no-op path then froze those counts at first trigger, so every "N items pending" in a briefing was a first-sighting number. **And the round-trip between `_new_advisory` and `briefing._declared_prerequisites` is now pinned by a real test** rather than by two fixtures agreeing with each other.

**Chunk 02 built** — all 18 sites carry `owner_action`, the four narrowed `recommended_action` values are discharged (one command, three empty), and `tests/test_advisory_actionability.py` is the lint. The fallback line no longer renders for any probe-raised advisory; it remains live only for advisories *stored* under the old schema, which is the case it was written for.

Three things worth carrying forward. **The lint's rule set is where the review pressure landed:** it shipped implementing two of §7.2's three banned forms, and the missing one — a bare shell invocation — was not hypothetical, because `git` is an allowed executable in the agent's field and the stale-base advisory's whole subject is a push. It now matches executable-followed-by-a-word, which also catches the incidental noun ("your git history"); that over-reach is deliberate, since owner copy describes an outcome rather than a tool and the fix is always a reword. **The no-commands-in-owner-text rule turned out to bind one field higher than the plan scoped it** — `trigger_summary` is relayed too, so three summaries ending in "run `<command>`" were rewritten; the strategy-artifact one was a named copy target whose *summary* still carried the defect its `owner_action` had just been written to fix. **And `recommended_action` empty is now a shipped shape** (dead-why, stalled-transition, checks-dormant): every reader guards on truthiness, so the `agent →` line is simply absent, which is correct for an advisory whose resolution is a decision or an artifact edit rather than a command.

Next: Chunk 03 — the read-side surfaces (`advisory_cmd.py` `list`/`show`, and `plugin/skills/advisory/SKILL.md`). Both still print `recommended_action` only, so a `dead-why` or `checks-dormant` advisory renders there with no action at all; the CLI is the surface where the new field is currently *least* visible.
