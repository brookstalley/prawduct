...
Next: Chunk 02 — 17 remaining `owner_action` sites and the four narrowed `recommended_action` values. The fallback line is currently visible on most advisories including this repo's live one; that is the intended interim state, not a defect, and it disappears site by site as Chunk 02 lands.
