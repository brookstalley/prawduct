...
governed_by:
  ...
  - artifact: data-model
    dispositions:
      ...
      - "facts are immutable and append-only / a fact written by a newer schema is a loud block / governance verdicts computed from the fact ledger → inapplicable because no chunk touches the evidence store or the fact schema"
...
- **Deliverables:**
  ...
  - new `tests/test_advisory_actionability.py` — sweeps every `AdvisoryCandidate(` construction under `plugin/lib/*_probes.py`: `owner_action` present and non-empty; no slash command, `prawduct-hook`, or shell invocation inside it; `recommended_action` either empty or a single command token.
