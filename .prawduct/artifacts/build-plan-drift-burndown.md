...
governed_by:
  ...
  - artifact: nonfunctional-requirements
    dispositions:
      - "proportionality ratchets both ways; a control added from 2026-07-29 names its expected yield **and emits that yield observably** → **binds Chunks 01 and 04, both of which add a control after the boundary date.** Chunk 01's yield is directly countable without any new mechanism: the check is a test, so its yield is the set of references it reddens, and the plan records the pre-build census (below) as the denominator. **Chunk 04's doctor check does NOT discharge the emission half, and an earlier draft of this line claimed it did** — it read 'its yield is the degraded-count that surface already prints', which is verbatim the thing the norm rules out ('a control whose findings are printed and forgotten satisfies the letter and defeats the point, since it can never be retired on evidence, only defended on principle'). Doctor's output is session prose: no fact, no counter, no ledger line, so a #13 firing leaves no trace the janitor's Norm Health sweep can query. What IS discharged is the *named* half, and the number is exact rather than qualitative: **every already-onboarded repo fires #13 exactly once and then never again** (the marker only ever reached repos scaffolded after `init_product` started writing it, and the repair is idempotent), so expected yield = the count of pre-marker repos in the fleet, and a sweep finding a *second* firing on one repo is evidence the repair did not hold. The emission half is **unmet, not carved out** — the norm's `in-transition` status is about the yield *query* not existing (LNG-5W8R carries the canary's enabling half), and it exempts nothing from emitting. What the norm's interim rule does ask of a control being reasoned about rather than measured — *state what evidence would have settled it* — is what the exact expected yield above supplies. Doctor has no fact-emitting path at all, and building one for a single check is the disproportion the norm exists to prevent, so the gap is filed as `#563`. **[DECISION: Health Check #13 ships WITHOUT the observable-emission half — a bounded exception to the observable-yield norm, not a conforming control | doctor has no fact-emitting path at all, so discharging the obligation means building a whole emission channel for one check, which is itself the ceremony-accumulation the norm was ratified to stop; the norm's own why is that removal must become a query rather than a debate, and the exception preserves that by pinning the number the query would return (every pre-marker repo fires once; a second firing on one repo falsifies the repair) so #13 stays *judgeable on argument with a stated evidence specification*, which is exactly what the norm's interim rule asks of a control it cannot yet measure | bounded: expires when #563 gives doctor a fact-emitting path, at which point #13 is the worked first case; user can veto/override — vetoing means #13 does not ship until #563 does]** Naming this as a rationale alone would have been a silent departure: `docs/norms.md` § The Authority Rule offers four resolutions and rules that a builder-authored sentence, however long, is not one of them. Neither control needs a stable-token workaround because neither rides a Critic finding title"
