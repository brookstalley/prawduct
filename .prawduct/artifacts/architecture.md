...
- **Local-first: *governance* coordination is process-spawn + atomically-written files + the git object database — no network, no daemon, and the governance runtime carries no third-party dependencies (dev/test tooling excepted). Two network surfaces are admitted, both off unless a person acts and neither carrying a governance verdict: an *opt-in backlog backend*, which must degrade to the markdown backend when unconfigured, and *owner-approved upstream filing*, which must send nothing without an owner approval of the exact outbound bytes. **A *surface* here is a path by which a governed product's own content leaves it; that is what the count of two binds. It is not the count of egress *sites*, which is larger and lives in exactly one place — `security-model.md` and `project-state.yaml`'s `egress_boundary` — and which includes calls that carry no product content and are therefore outside this count (`check-released`, operator/CI-invoked; `cmd_stop`'s `gh pr list`, which does run on the Stop hook). Clarified 2026-09-07 alongside the same-day amendment: it states what the count already meant and admits nothing new — a third *surface* remains a fresh decision, not an increment on these.**
  ...
  Clarified 2026-09-07 (same day, separate act, **open to owner veto**): the statement now distinguishes an admitted *surface* — a path by which a governed product's own content leaves it, which is what the count of two binds — from an enumerated egress *site*, of which there are more, and which keeps its single home in `security-model.md` and `project-state.yaml`'s `egress_boundary`. `[DECISION: state the surface/site distinction in the norm rather than leave the count to be read against a longer inventory | a reader consulting this sentence to decide whether a new call needs an owner ruling could not tell whether `check-released` and `cmd_stop`'s `gh pr list` were exempt non-surfaces, admitted surfaces the count omits, or departures the sentence now forbids | admits nothing new and blesses no code in this bundle — a third SURFACE remains a fresh decision | user can veto/override]` Written by the builder, not by the owner ruling above, which is why it is flagged: the widening was the owner's, this sentence is not.
