...
3. **Local-first, no network, no daemon — in governance.** Coordination is process-spawn +
   ...
   not survive contact with "I just want to code." **Two network surfaces are admitted, and neither is
   reached unless a person acts** — that property, not a configuration scalar, is what carries this
   constraint (§ Direction *Local-first*, amended 2026-09-07). The **principal** one is the
   **opt-in** backlog backend (`backlog_service_repo`), which reaches GitHub Issues through the
   ...
   depends on it. The second is **owner-approved upstream filing** (`file-upstream`), which rides
   the same `gh` seam to the plugin's pinned upstream repo and is therefore reachable **with
   `backlog_service_repo` unset** — its default state files nothing, so a product that never reports
   a bug upstream never opens a socket. Neither is the whole list — the full one, including one call
   that does run on a hook path, is enumerated in `security-model.md` and `project-state.yaml`'s
   `egress_boundary`.
...
- **Local-first: *governance* coordination is process-spawn + atomically-written files + the git object database — no network, no daemon, and the governance runtime carries no third-party dependencies (dev/test tooling excepted). Two network surfaces are admitted, both off unless a person acts and neither carrying a governance verdict: an *opt-in backlog backend*, which must degrade to the markdown backend when unconfigured, and *owner-approved upstream filing*, which must send nothing without an owner approval of the exact outbound bytes. **A *surface* here is a path by which a governed product's own content leaves it; that is what the count of two binds. It is not the count of egress *sites*, which is larger and lives in exactly one place — `security-model.md` and `project-state.yaml`'s `egress_boundary` — and which includes calls that carry no product content and are therefore outside this count (`check-released`, operator/CI-invoked; `cmd_stop`'s `gh pr list`, which does run on the Stop hook). Clarified 2026-09-07 alongside the same-day amendment: it states what the count already meant and admits nothing new — a third *surface* remains a fresh decision, not an increment on these.**
  Why: a governance layer that *required* infrastructure would not survive contact with "I just want to code," and a zero-dependency stdlib runtime shrinks the supply-chain surface to prawduct's own code plus git. Both rationales survive, and the property that carries them is **not** that the surfaces are confined to one subsystem — it is that neither is ever reached without a person acting. The backlog backend is opt-in per product (`backlog_service_repo` unset ⇒ the markdown backend); upstream filing needs an owner approval per filing, so its default state files nothing and a product that never reports a bug upstream never opens a socket. A repo that does neither runs exactly the substrate this norm has always described. The zero-dependency half is untouched by both — `gh` owns every network call and the governance runtime still imports nothing outside the stdlib — and no gate, evidence fact, or review verdict crosses the network on either surface.
  Status: steady-state.
  Amended: 2026-09-07, owner ruling in-session, admitting owner-approved upstream filing as the second network surface. `[DECISION: widen the admitted network surface from "opt-in backlog storage" to "opt-in backlog storage AND owner-approved upstream filing", rather than gating `file-upstream` on `backlog_service_repo` | this norm's why is that a governance layer requiring infrastructure does not survive contact with "I just want to code" — the load-bearing property is that nothing reaches the network unprompted, which `file-upstream` satisfies through per-filing owner approval rather than through a configuration scalar. The rejected alternative reads as the smaller change and is not: keying a bug-reporting capability on an unrelated storage choice re-introduces exactly the `backlog_service_repo`-keyed fail-open that the filing design's own §5 check-3 amendment removed, and it would leave a product that never adopted the backlog backend unable to report a bug upstream for a reason with no relation to reporting bugs | user can veto/override]` The prior scope sentence claimed the surface was "confined to backlog storage", which `file-upstream` breaks literally — it targets the plugin's pinned upstream repo, not the product's own, so it is reachable with `backlog_service_repo` unset. Naming the surface is the honest resolution; the `Why:` above is repaired to rest on the guarantee that actually holds. No structural characteristic flips: `gh` still owns the credential and the adapter still manages no token.
  Clarified 2026-09-07 (same day, separate act; **ratified by the owner the same day** — the veto was offered and withheld): the statement now distinguishes an admitted *surface* — a path by which a governed product's own content leaves it, which is what the count of two binds — from an enumerated egress *site*, of which there are more, and which keeps its single home in `security-model.md` and `project-state.yaml`'s `egress_boundary`. `[DECISION: state the surface/site distinction in the norm rather than leave the count to be read against a longer inventory | a reader consulting this sentence to decide whether a new call needs an owner ruling could not tell whether `check-released` and `cmd_stop`'s `gh pr list` were exempt non-surfaces, admitted surfaces the count omits, or departures the sentence now forbids | admits nothing new and blesses no code in this bundle — a third SURFACE remains a fresh decision | user can veto/override]` Drafted by the builder rather than by the owner ruling above, which is why it was put up for veto separately: the widening was the owner's, this sentence was not. It is theirs now — the clarification stands on the same authority as the amendment it sits under, and reverting it is an ordinary amendment rather than the withdrawal of an unratified line.
...
- **Prawduct is written in Python and must never be specific to Python. Gates and canaries dispatch per *file* by language, never per repo; no gate may assume the governed product shares the runtime's language; and a language with no populated rules is reported as *unchecked*, never silently passed.**
  ...
  Stopgap: recorded 2026-09-01, expires 2026-12-01. The interim rule above stands as a **bounded exception** for the duration. `[DECISION: the per-suffix dispatch interim rule is extended as a bounded exception until 2026-12-01, rather than accelerating the migration or improvising around it | the interim rule already holds the line this norm exists to defend — new gate code delegates, an unknown suffix reports **unchecked** rather than passing, and no gate acquires a language parser — so nothing new is being built Python-specific while the transition runs; what is stalled is the *enumeration sweep* of existing violating sites, which LNG-5W8R's own first acceptance criterion makes a multi-chunk plan rather than a fix, and a stall advisory cannot be answered by pretending otherwise | user can veto/override]` Owner ruling, 2026-09-01 (#732). Clock: the `expires 2026-12-01` bound above IS machine-read — `norm_probes._expired_stopgaps` parses it and the stall advisory fires on 2026-12-02 without anyone walking it. (An earlier revision said the opposite, citing #564's missing `revisit:` write path; #564 shipped 2026-09-02 and the stopgap arm never used that field. Corrected at the 2026-09-16 Norm Health sweep.) LNG-5W8R additionally carries the expiry as prose on the item.
  ...
  Retroactivity: migrate: LNG-5W8R — inventory 2026-07-29, **partial by declaration, not exhaustive**. Confirmed compliant: `gates.py`, `coverage_algebra.py` (no language literals; path-based). Confirmed violating: `compliance.py` (`_is_test_file`, `_is_dependency_file`, `_check_broad_exceptions`); `plugin/bin/test-reference-verify` (`_PY_SYMBOL_RE`, `_is_python_file` — and it feeds `verify-coverage`, a **BLOCKING** Goal 1 check that therefore passes silently on Swift/Rust/C#, the most serious instance found); `gitstate.py` `_PRODUCT_CODE_SUFFIXES` (12 suffixes, missing `.cs`, fails open rather than reporting unchecked); ~~`release_verification.py`~~ — **discharged 2026-08-04**, both grounds, before the module's first release. `_VERSION_FILES` was a prawduct-layout table and `_version_from`'s `toml` branch a hand-rolled parser for one language's manifest format — the "no gate acquires a language-specific parser" clause exactly. Products now declare `release_version_files:` (path + format + key path), which is the remedy as owner-ruled — **declaration** rather than broader parsing — and the TOML read **delegates to stdlib `tomllib`** per this norm's own interim rule ("new gate code delegates first"), so the parser was deleted rather than taught about tables. The layout table survives only as an explicitly-labelled fallback *guess* that cannot produce a failure. ~~`audit_learnings_cmd.py` `run_sentinel`~~ — **discharged 2026-08-26**, and it is the warning the "not closed until the sweep runs" sentence below was written for: found by triaging an upstream bug report, not by the inventory. It hardcoded `sys.executable -m pytest`, so in any non-Python product the learnings sentinel mechanism was inert *and* reported green tests as failing — and because a failing sentinel argues for retiring the rule it guards, the fail-open clause was inverted into something worse than silence: a gate arguing *against* a live rule on evidence it never gathered. Remedied by **declaration** on the `release_verification.py` precedent, and **as owner-ruled** (2026-08-26, stated directly: *"NEVER python specific, never requiring a specific implementation of testing or anything else. We provide absolute business requirements, consuming repos decide the best way to implement for their project"*) — products declare `sentinel_command:` with a `{sentinel}` placeholder — and the ungradeable case now reports **ungraded**, distinct from failed, exactly as this norm's fail-visible clause requires. No default survives: a pytest fallback would be the same violation wearing a default's clothes. That the fallback was deleted outright rather than held through a deprecation window is recorded where the clause it departs from lives — `api-contract.md` § Direction, case law [[inert-retention-cannot-be-extended-across-norms]]. Kept in this list, struck rather than removed: the enumeration is the migration's record, and a reader checking whether a named site was ever addressed needs the answer, not a gap. Completing this enumeration is LNG-5W8R's first acceptance criterion — an earlier revision of this line claimed `compliance.py` held *every* violation, which was false, and no reader should treat this list as closed until that sweep runs.
- **Prawduct guides and reviews; it never implements. It writes no product code, no config, and no tooling — a best practice enters as a *requirement* captured at discovery, Claude Code implements it, and the Critic blocks on a requirement that went unimplemented. The same line one level down: prawduct never re-implements what a product's own tooling already does. A bare-except check belongs to ruff or clippy, not to a prawduct gate.**
  ...
  Stopgap: recorded 2026-09-01, expires 2026-12-01. The interim rule above stands as a **bounded exception** for the duration. `[DECISION: the no-new-duplicate-checks interim rule is extended as a bounded exception until 2026-12-01 | the corollary's live edge is *new* checks, and the interim rule already closes it — a check duplicating an existing linter rule is not written, the toolchain standard is captured as a requirement instead — so the framework is not accumulating the re-implementations this norm forbids; what is stalled is retiring the two existing `compliance.py` checks, and that cannot honestly precede configuring ruff for prawduct itself, or coverage is lost rather than delegated | user can veto/override]` Owner ruling, 2026-09-01 (#732). Clock: the `expires 2026-12-01` bound above, machine-read by `norm_probes._expired_stopgaps` (fires 2026-12-02); LNG-5W8R also carries it as prose.
...
- **Goals and verification bind; prescribed method is advice.** A governing artifact says what must be true and how it will be checked, and those bind. Where it also prescribes *how* — a chunk's `Deliverables`, a named call site or file, a skill's ordering of steps — that is the author's best guess, made before the code was read, and a builder who finds a better route **takes it and records why** rather than conforming to a guess. Verification structure is not method and is untouched by this: independent review, mutation-proof, mechanical enumeration, and the gates all bind exactly as before, because they constrain the *output contract* rather than the route to it. Neither are the load-bearing invariants stated elsewhere in this section — reviewer non-mutation, authority-fails-closed, least-authority writes — which state what must be true, not how to get there.
  ...
  Stopgap: recorded 2026-09-01, expires 2026-12-01. `[DECISION: the incomplete GOV-4T9P sweep is recorded as a bounded exception until 2026-12-01 rather than left as an open stall | this norm changes how a prescription is *read* rather than forbidding it from existing, so an unfinished sweep blocks no new work and leaves no reader without an answer — the Direction entry itself states the binding reading; what is stalled is migrating the two named sites whose phrasing propagates, `templates/build-plan.md`'s worked examples and `methodology/planning.md`'s teaching block, neither of which a grep for the field name finds | user can veto/override]` Owner ruling, 2026-09-01 (#732). Clock: the `expires 2026-12-01` bound above, machine-read by `norm_probes._expired_stopgaps` (fires 2026-12-02); GOV-4T9P also carries it as prose.
...
- **Every fact has one home; every other mention is a reference to it.** A fact is anything checkable that can change — a number, a behaviour claim, a path, a rule statement, a schema. Its home is the place closest to the mechanism that owns it. Elsewhere: cite the symbol, the heading, or the command that yields it. Never restate it. **If changing a fact requires editing N places, N−1 of them are already wrong** — the only question is when someone notices. **A fact is the whole predicate, not a token inside it:** sharing a constant shares *syntax*; sharing the constant AND the reader that feeds it is what shares the *definition*.
  ...
  Stopgap: recorded 2026-09-01, expires 2026-12-01. `[DECISION: the not-yet-enumerated GOV-2R8K retroactivity is recorded as a bounded exception until 2026-12-01 | this norm's Enforcement row is Critic-judgment-only — GOV-2R8K records why, that the question has no reliable mechanical hook for prose — so the norm binds every review today whether or not the enumeration is finished, and the sweep buys back-catalogue coverage rather than forward authority; enumerating three kinds across the corpus, one of which (a shared symbol with an unshared reader) presents as proof of agreement and evades text search entirely, is a plan and not a pass | user can veto/override]` Owner ruling, 2026-09-01 (#732). Clock: the `expires 2026-12-01` bound above, machine-read by `norm_probes._expired_stopgaps` (fires 2026-12-02); GOV-2R8K also carries it as prose.
  ...
  Decision: `[DECISION: a fact is the whole predicate — sharing a constant shares syntax, sharing the constant AND the reader that feeds it shares the definition | the norm was FOLLOWED (one home, imported symbol) and still produced two answers, because a token-level reading satisfies its letter; the shared symbol then reads as proof of agreement, so nobody re-checks it | user can veto/override]` Owner amendment, 2026-08-11 (#643), on the branch whose own code the stricter reading had already been fixed to (`b0a96583`) — the norm tightened against the code, not the code blessed by the norm. Case that produced it: [[one-home-is-the-predicate-not-the-token]]. **This DOES forbid something previously permitted**, and the earlier draft of this line wrongly claimed otherwise. Sharing a constant while leaving its input unshared satisfied the norm as written and does not now; #643's own problem statement says so ("any product following the norm as written will share a constant and believe the job is done"), and this branch is the proof. The patch-release argument does not rest on the amendment being toothless — it rests on the amendment being small, its only known violating site being already fixed, and — the half that matters for governed products rather than this tree — this norm's Enforcement row being Critic-judgment-only (GOV-2R8K records why: the question has no reliable mechanical hook for prose). No consumer's gates change behaviour on upgrade; what changes is what a reviewer asks.
  Decision: `[DECISION: a rule whose readers load DISJOINT payloads may carry one statement per carrier, provided a construction pins their agreement | the norm's why is that N copies means N−1 are already wrong and nobody notices; a pinned map removes the second half — the copies cannot disagree without a test going red — while a single home would put the rule behind a pointer the reviewer is forbidden to follow, which is not one home but zero for half its audience | user can veto/override]` Recorded 2026-08-20 at the CATEGORY level, because the edge had already been decided twice case-by-case (`RESOLUTION_IS_A_CLAIM_DIRECTIVE`, `VERIFY_RATES_BLOCKING_ONLY_DIRECTIVE`) and a third case still re-derived it from scratch against four constraints — which is the cost this norm's own rationale predicts for an unstated rule. Scope of the exception, and it is narrow: the carriers' audiences must be disjoint (a reader who could open the other carrier has a pointer available and takes it), and the construction must fail when they disagree — `tests/test_prose_severity_ceiling.py` and `tests/test_finding_scope_rule.py` are the two worked examples. Absent that pin this is an ordinary second home and the norm applies unchanged.
  Retroactivity: migrate: GOV-2R8K — **not yet enumerated.** **A THIRD KIND, added 2026-08-11 with the granularity amendment and stated here rather than discovered mid-pass — GOV-2R8K's own practice:** a *shared symbol with an unshared reader* — two call sites importing one constant but normalizing their input differently. **What separates it from the copied-derivation kind is not invisibility** — GOV-2R8K already records that a derivation has no shared string and a prose sweep walks straight past it, so both evade text search. It is that **a shared symbol reads as proof of agreement**: the norm's operative review question is *why are there two?*, and a shared symbol answers it in the negative by presenting one site. A copied derivation at least presents N sites for that question to land on — though a reviewer may not recognize them as one predicate, so the difference is degree, not kind, on the review axis. It survived this branch's own chunk review on exactly that reading (a commit message asserting `record_lint` "needs to know nothing about blockquotes" because the regex bytes matched) and was caught only at the cumulative, by two reviewers independently. Known site: `record_lint._norm_field_re` / `norm_probes._FIELD_MARKER_RE`, fixed in `b0a96583`. Known duplications from the review that prompted this: the restamp claim (4 copies), "sentinel tracker" (2), the empty-pointer promise (3), `changes_referenced`-means-judged-code (3). Enumerating the rest is the item's first acceptance criterion.
...
- **`critic-begin` (code).** Captures the working tree into the shared git object DB via a
  *temporary* index (never touching the session's real index or working tree), derives the review
  ...
  added lines. It also **hands the reviewer the rules its payload cannot carry** — the dispatch
  directives (`critic_consolidate`'s `FINDING_SCOPE_DIRECTIVE`, `VERIFY_RATES_BLOCKING_ONLY_DIRECTIVE`,
  `RESOLUTION_IS_A_CLAIM_DIRECTIVE`), printed per mode. They are instruction text, never data: no
  gate reads them, and a mode's total load — payload file plus whatever its dispatch prints — is
  what the unit-cost norm governs and what `tests/test_finding_scope_rule.py` meters.
...
| Upstream bug intake (this repo) | GitHub Issues on this repo | bug reports products file about prawduct itself. `/prawduct:report-bug` files them as issues through the backlog adapter's `file-upstream`, and `untriaged-upstream-reports` counts the open prefixed ones nobody has staged. Triage happens on the issue — it is already the durable record. The `incoming-bugs/` drop-box that preceded this retired with the repoint (2026-09-08); reports written before it are untracked files on one machine, with no channel behind them | issues are public and permanent, which is what the filing side's per-report owner approval exists for |
...
5. **`gh` subprocess → GitHub Issues REST** — **off unless a person acts, which is not the same as
   keyed on `backlog_service_repo`.** `lib/backlog/transport.py` carries TWO callers: the opt-in
   backlog backend, present when that scalar is set, and the owner-approved `file-upstream` op,
   which targets the plugin's pinned upstream repo and so is reachable **with the scalar unset**.
   Reading this channel as gated on the scalar is what let the previous phrasing stand; the counts cache under
