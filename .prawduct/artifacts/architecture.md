...
| Upstream bug intake (this repo) | GitHub Issues on this repo; `incoming-bugs/` (gitignored) for pre-cutover reports only | bug reports products file about prawduct itself. `/prawduct:report-bug` files them as issues through the backlog adapter's `file-upstream`; the drop-box holds only what was filed before that cutover and nothing writes to it. Both are triaged into the backlog | issues are public and permanent; the drop-box is per-clone and retires with the advisory repoint |
