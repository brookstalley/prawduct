...
4. **The reason I supplied.** "A loser that was itself an earlier survivor carries several ids, and `merge` never moves them" — self-contradictory in one sentence, and false: *because* merge never moves aliases, an earlier survivor is precisely the item that accumulated none.

**The instruction was right every time.** What kept failing was the sentence explaining why.
