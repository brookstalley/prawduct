...
- **[COV-7K4N]** check-cumulative-critic false-`uncovered` with a misleading remedy when origin/<base> is stale (feature built on unpushed local integration commits)
  `effort: S · impact: M · area: coverage · source: reflection · added: 2026-07-14 · reviewed: 2026-07-14 · status: shipped · stage: design · closed-by: stale-remote-base-diagnostics · related: COV-5H3N, ENV-2W7K, PR-2H8N, PR-7T2K, COV-9B4T · refs: lib/coverage.py (_resolve_base_branch prefers origin/<b> for a "stable remote-tracking merge-base"), lib/gates.py (check_cumulative_critic uncovered path), docs/release-process.md`
