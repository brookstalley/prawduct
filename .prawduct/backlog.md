...
- **[BKL-5N9W]** `skills/backlog/SKILL.md` pairs `disable-model-invocation: false` with a wildcard `Bash(prawduct-hook backlog *)` grant — narrow it to the ops the skill drives
  ...
  **[STATUS POINTER 2026-07-24 — the body above reads as unfixed; it is BUILT.]** The work is built on `feature/v3.2.0-c02-adapter-safety` (Chunk 03); the `status=shipped` flip is deferred to **Chunk 09** per this release's `[ ]`-until-release convention, so the item correctly stays `open`. The metadata test shipped, in both invocation forms. **Two follow-ons from the 2026-07-24 cumulative Critic, filed rather than folded here:** **GOV-5J9Q** — the narrowing did not ripple to `janitor`/`pr`/`runbook` SKILL.md, which still carry the wildcard; and a warning that the negative assertions are exact-string rather than parsed, so `Bash(prawduct-hook *)` or `Bash(prawduct-hook backlog*)` would re-grant the withheld ops with all three tests green. (Prose left intact per BKL-8Q3D.)
...
- **[BKL-8K2N]** Pacer telemetry is discarded on the real migration path — `cli._run_import` passes no `pacer`, and both sleep paths are silent, so a throttled ~900-issue run is indistinguishable from a hang
  `effort: S · impact: L · area: backlog-service · kind: bug · source: critic · added: 2026-07-24 · reviewed: 2026-07-24 · status: open · stage: ready · related: BKL-6X5D, BKL-6M4T · refs: plugin/lib/backlog/cli.py:507 (_run_import — the production entry point, passes no pacer), plugin/lib/backlog/migrate.py:171-184 (Pacer.before_points — sleeps with zero output), plugin/lib/backlog/migrate.py:251 (RateLimitBackoff.pause — sleeps with zero output), plugin/lib/backlog/migrate.py:142-144 (points_charged / point_waits — accumulated then dropped), tests/spikes/s2_migration.py::_record_pacing (the only reader in the tree)`
