...
- **[TST-6H2Q]** `test_pr_reviewer.py::TestStopPrReviewGate` stop-gate "blocks" tests flake under xdist cross-FILE pollution
  `effort: M · impact: M · area: test-isolation · source: builder · added: 2026-07-09 · status: open · stage: ready · related: TST-4P8H`
...
- **[COV-2P7F]** Unify the "`.prawduct/**` is governance-metadata, not code" predicate across ALL PR fast-paths (not just `.md`)
  `effort: M · impact: M · area: coverage · source: user · added: 2026-07-09 · status: shipped · closed-by: gate-friction-batch · reviewed: 2026-07-09 · stage: design · related: CRT-5D8Q, COV-5H3N, COV-8R2K, PR-5K8D · refs: lib/gates.py (_record_covers_head, _compute_verify_resolutions_scope), lib/coverage.py (cmd_check_pr_doc_only), bin/prawduct-hook, incoming-bugs/archive/2026-06-13-governance-metadata-fix-triggers-full-code-pr-gates.md`
...
- **[CRT-5D8Q]** PR-gate coverage vs verify-resolutions scope disagree on the metadata exemption — deadlock when the ledger fallback window has lapsed
  `effort: S · impact: M · area: governance/critic-gate · source: critic · added: 2026-07-02 · status: shipped · closed-by: gate-friction-batch · reviewed: 2026-07-09 · stage: ready · related: CRT-8H3R, CRT-2K9F, CRT-4J8W, STH-6T9W · refs: lib/gates.py (_record_covers_head L972, _compute_verify_resolutions_scope L447)`
...
- **[PR-7T2K]** PR gates validate local HEAD, not the pushed origin/<branch> that squash-merge uses — post-push commits silently dropped
  `effort: M · impact: M · area: pr · source: user · added: 2026-07-09 · status: shipped · closed-by: gate-friction-batch · reviewed: 2026-07-09 · stage: ready · related: PR-2H8N`
...
- **[CRT-9K7T]** Forked `/prawduct:critic` coordinator never writes `.critic-findings.json` and leaves a stale `.critic-active` marker
  `effort: M · impact: M · area: critic · source: critic · added: 2026-07-09 · status: shipped · closed-by: gate-friction-batch · stage: ready · reviewed: 2026-07-09 · related: CRT-7Q2T, CRT-2K9F, STH-6T9W, CRT-6F2N · refs: skills/critic/SKILL.md, .prawduct/.critic-findings.json, .prawduct/.critic-active, lib/ledger.py, lib/critic_mode.py, bin/prawduct-hook (critic-end, check-cumulative-critic)`
  ...
  **Corroborated + broadened 2026-07-09 (discodon, cross-repo upstream corroboration of CRT-8F3K).** Three new facts sharpen scope and root cause: (1) the defect ALSO affects the /prawduct:critic **CUMULATIVE** variant, not just forked final+verify-resolutions. (2) A **SECOND write surface** is skipped — the `review.critic` event is never appended to `.governance-ledger.jsonl`, and that event is the anchor `prawduct-hook check-cumulative-critic` gates on; missing it, the gate reports `chain-missing-anchor` and **deadlocks the PR gate**, with recovery being a full ~3-reviewer-agent re-run. (3) **Non-deterministic race confirmed by repro:** the first cumulative did NOT persist while a second identical invocation persisted correctly (`commit_reviewed=adf41134` + ledger event) — a silent failure the caller only learns about via the failing gate; independently seen 2026-07-09T16:28 on a `review.pr` on branch `fix/discord-identity-optional-token`. **Likely cause:** fork-and-return hands control back to the caller BEFORE the monitor's all-reviewers-complete consolidation/writeback runs. **Refined fix-shape:** an exit-time assertion (in `critic-end` / skill exit) that BOTH `.critic-findings.json` `commit_reviewed==HEAD` AND a `review.critic` ledger event for HEAD are present, failing loud otherwise; and always clear `.critic-active`. Now confirmed across 2 consumers, spanning BOTH critic variants (forked + cumulative) and BOTH write surfaces (findings JSON + governance ledger).
...
- **[BRF-6K2D]** Session-briefing "delete the plan" nudge isn't merge-aware — fires on develop while the plan's feature branch is unmerged
  `effort: S · impact: M · area: briefing · source: reflection · added: 2026-07-09 · status: shipped · closed-by: gate-friction-batch · reviewed: 2026-07-09 · stage: ready · related: STH-3K7M, STH-3R8K, DOC-5T8N`
