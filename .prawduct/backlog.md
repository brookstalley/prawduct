...
- **[BLD-7K3Q]** `verify-chunk-refs` grades the WRONG chunk on a `views_enabled` branch — it reads the first *unchecked* Status item, but checkboxes only flip at release
  ...
  **RECURRENCE 2026-07-24 — hit again, live, on the v3.2.0 release itself (cumulative Critic rev-20260724T205224Z-4a1c2528).** This release adopted a `[ ]`-until-release convention (`build-plan-v3.2.0-golive.md:541` — chunks build and verify but defer their checkbox flip to Chunk 09), which is the *same* "checkboxes only flip at release" condition by a different route: not `views_enabled`, but a deliberate release convention. Effect is identical and now measured across a whole release — `verify-chunk-refs` stays pinned to **chunk 01 for the entire release**, so **chunks 02–05 received no ref verification at all**; the gate ran green while scoped to a chunk that has no refs.

  Two things this recurrence changes about the item. First, the trigger is **broader than `views_enabled`** — any convention that defers checkbox flips reproduces it, so a fix keyed only on `is_views_enabled` would not have caught this release. Second, it is no longer a single-branch observation: it is the standing condition for a multi-chunk release, which is when ref drift is most likely and least visible. Cross-filed from **CRT-2P8V**, which carries the sibling `_GIT_REF_PREFIXES` gap from the same review and points here rather than duplicating this. (critic — rev-20260724T205224Z)
