...
- **[STH-6D4Q]** `tests_are_current` fails OPEN when `.session-start` is absent — with no tree check on that path — and this bundle widened the reachability by removing `resume`'s accidental re-anchoring; the reflection file now survives a boundary with no freshness check either
  ...
  **Half two — the same theme on the reflection file.** Gating the `.session-reflected` unlink on `reflection_preserved` is a genuine improvement (it closes the one silent permanent-loss path in the boundary: a reflection that could not be archived used to be deleted three statements later). But the file now **survives** a boundary, and *both* readers of it check only presence and `len(content) >= 50` — `prawduct-hook:1405` for the blocking reflection gate and `briefing.py:1491` for the warning. Neither checks freshness. So the next session's blocking reflection gate can pass on the **previous** session's text. The old unconditional deletion was what carried that invariant; nothing replaced it.
...
- **[TST-9M2X]** A prose pin that passes vacuously: `test_session_boundary_prose.py`'s regex cannot match a past participle, so both sites it exists to catch survive it — and both are now false
  `effort: S · impact: M · area: tests · kind: bug · source: critic · added: 2026-07-27 · reviewed: 2026-07-27 · status: open · stage: ready · related: SCN-5B8Q · refs: tests/preferences/test_session_boundary_prose.py:48-51 (`_BARE_NEXT_SESSION`) and :36-46 (the `SURFACES` roster), plugin/methodology/building.md:94 ("Auto-cleared next session"), plugin/bin/prawduct-hook:1374 ("(auto-cleared at session start)"), plugin/bin/prawduct-hook:586 + :667 (`_boundary_close_session` — the only `.gates-waived` deletion), .prawduct/artifacts/project-preferences.md § Enforcement`
  ...
  **The defect, verified at the source.** The pin's regex is `(?:on|at|the)\s+next\s+session\s+start\b|auto-?clears?\s+next\s+session\b`. `auto-?clears?` matches `auto-clear` / `auto-clears` and nothing else, so **neither** of the two live claim sites can match:
  ...
  Both sentences are **now false**: `.gates-waived` is deleted only inside `_boundary_close_session`, so a waiver outlives an unbounded number of continuations and clears at a boundary, not at "next session start". That new lifetime is deliberate and is stated in `architecture.md`, the `cmd_clear` docstring and six runtime strings — these two sites are the ones the sweep missed, and the pin that was supposed to catch them reported green.

  **`building.md` is in the pin's own `SURFACES` roster.** So the pin does not merely fail to catch these — it **reports coverage it does not provide**, over a file it explicitly claims. Widening to `auto-?clear(s|ed|ing)?` closes both.
...
- **[DOC-4T6P]** Three documentation-accuracy defects from the cumulative: change-log names the wrong function, the Enforcement table skips one of three new prose pins, and a concerns cell cites a plan that will be deleted
  `effort: S · impact: S · area: docs · kind: bug · source: critic · added: 2026-07-27 · reviewed: 2026-07-27 · status: open · stage: ready · related: GOV-3P8K · refs: .prawduct/change-log.md:228 (the Chunk 02 function list) and :264 (the DECISION that reverses it), .prawduct/artifacts/project-preferences.md § Enforcement (~:78-83), tests/preferences/test_session_boundary_prose.py (the unrostered pin), .prawduct/cross-cutting-concerns.md:44 (the citing cell) and :40 (the ephemeral-ref firewall row), plugin/lib/gates.py:744, plugin/lib/briefing.py:442 + :457, plugin/lib/buildplan_refs.py:445 + :958`

  Filed from cumulative Critic review fact `rev-20260727T164842Z-1b70981d` (branch `feature/session-handoff-continuity`). Grouped as one small item because all three are single-line corrections; each is independently verified at the source.
