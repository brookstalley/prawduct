...
- **[BKL-9XQ2]** Consuming repos filing issues against prawduct is critically underspecified — needs discovery before any consumer files upstream
  ...
  **Therefore TWO SEPARATE consent obligations** (sub-concern (1) previously conflated them into one):

  - **(1a) CONSENT-AT-INSTALL / DISCLOSURE.** Users must **understand that prawduct can file upstream at all.** This is an **onboarding-time** obligation and **cannot be satisfied by a per-file prompt** — a user who does not know the capability exists cannot meaningfully evaluate the prompt when it appears. Likely surfaces: `/prawduct:onboard`, the committed install reference, `project-preferences.md`.
  - **(1b) CONSENT-AT-FILE.** The three-state preference from the original sub-concern (1): `always-file` / `ask-user` / `never-file`, **default `ask-user`**, honored without exception. **Consequence of (1a):** even `always-file` should require a **one-time informed opt-in** — not merely a config value someone set without understanding what it authorizes. A blanket standing authorization has to be *granted*, not *defaulted into*.
  ...
  **Unchanged by this refinement:** the structural finding above still holds in full — the policy must bind at the **adapter (data plane)**, never in skill prose, since the prose is what the drop-box retirement rewrites. (1a) is the one part that necessarily *also* lives outside the adapter — disclosure is an onboarding surface — but that is an **addition** to the adapter guard, never a substitute for it.
...
- **[BKL-6X5D]** Archive window never quantified (binary `{all,open}` is the only shipped lever); Pacer doesn't meter 900 REST pts/min for the create+close archive stretch — gating for any `all`-scope migration at discodon size
  `effort: S · impact: M · area: backlog-service · source: user · added: 2026-07-17 · reviewed: 2026-07-20 · status: open · stage: design · related: BKL-6M4T · refs: backlog-service-prd.md#89, backlog-service-nfr.md`
  ...
  **Still open — and part (b) re-scoped from adopter-scale to gating.** Remaining: (i) the window **quantification** (an N-months / throughput formula between the `{all,open}` poles — still the adopter-scale refinement, unbuilt); and (ii) **part (b)**, the Pacer metering *total* REST points (5/write, 1/read) against 900/min. Part (b) was filed "not gating the dogfood," which held while the dogfood was small.

  The escalation to **gating** rests on a **structural** argument that needs no item count. Under `--archive-scope all`, each archived item costs a **create *and* a close**, while `pacer.before_create()` is annotated "the only paced call" (`lib/backlog/migrate.py:787`) — so the archive stretch runs create-then-close with **only half of it metered**, which is exactly the >900 REST pts/min window part (b) describes. That holds at 317 items and at 389 alike: the ratio, not the volume, is what breaches the burst ceiling. Its only mitigation today is *incidental* `gh`-subprocess latency, explicitly "not designed-in" — and forfeited by the raw-HTTP fast-path (D2/W1). New scope: **gating for any `--archive-scope all` migration at discodon's size**; under `--archive-scope open` there is no archive stretch and the gap stays theoretical, so it remains deferred for `open`-scope runs (including prawduct's own dogfood). Impact raised S→M accordingly. Part (b) now has a **named prover**: NFR §9 (:287) makes it an S2 obligation — measure the burst with `--archive-scope all` (volume reduction disabled, so the run proves the Pacer and not a small input) and answer whether the create-then-close archive stretch breaches 900 pts/min.
