...
- **[STH-7W9K]** Skill/subagent forks silently write `.prawduct/` state to the launch dir instead of a mid-session-entered worktree (cross-worktree pollution)
  `effort: M · impact: L · area: worktree · source: user · added: 2026-07-16 · status: open · stage: requirements · related: CRT-6W2N, STH-4K7N, STH-3R8K, CRT-3X9D · refs: bin/prawduct-hook (get_project_dir), lib/gitstate.py (resolve_project_dir), skills (fork skills that mutate .prawduct/ — backlog/advisory/operator-verification), methodology/building.md (mid-cycle worktree-entry edge)`
...
- **[VWS-2W6H]** regen-views plan-discovery mis-classifies scope-tagged design artifacts as duplicate build plans
  `effort: S · impact: M · area: views · source: builder · added: 2026-07-16 · status: open · stage: ready · related: REL-3B7Q · refs: lib/views.py (build_scope_to_plan_map, diagnose_scope_plan_coverage)`

  `build_scope_to_plan_map` and `diagnose_scope_plan_coverage` in lib/views.py glob all `artifacts/*.md` and treat any file with a frontmatter `scope:` key as a build plan. When design artifacts (data-model, api-contract, security-model, api-notes) declare the same `scope:` as their build plan (as the backlog-service Chunk 01 artifacts do), regen-views errors with "duplicate scope ... one plan is malformed" and writes no views — silently disabling Status derivation for the whole branch. Fix: filter plan-discovery to files whose frontmatter declares `artifact: build-plan` (design artifacts legitimately carry `scope:` for traceability and must not be mistaken for plans). (builder — backlog-service Chunk 02)
