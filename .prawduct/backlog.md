...
- **[CRT-5W3P]** The two fastest-growing reviewer payload files carry no token budget — `SKILL.md` +16% and `review-cycle.md` +14% in the bundle that ratified payload as a P0 wall-clock lever
  ...
  **Measured, not estimated** (bytes against the branch base `3a8eb4d`, 2026-07-30): `SKILL.md` +16.4%, `review-cycle.md` +14.4%. Both grew one commit after reviewer payload was ratified as a **P0 wall-clock lever**, and neither is in `LAST_MEASURED_TOKENS` or under a `test_token_budget` ceiling — so the growth is invisible until someone measures by hand, which is what happened here. The mechanism already exists and is already applied to three files (including `goals-1-3.md`, created this same bundle and budgeted from birth); these two simply aren't in it.

  **The two files need different answers — don't fold them into one ceiling.** `SKILL.md` is on the payload path of **every** review, so a byte added there is paid by every dispatch; that is a straightforward budget case. `review-cycle.md` is **not** on the `chunk`-mode payload path — which is precisely why content was deliberately relocated into it this session — so its budget question is about *reader* cost and drift, not per-dispatch wall clock. A single shared ceiling would either over-constrain the relocation target or under-constrain the hot file. Note also that the byte deltas above are a proxy: the existing table is in estimated **tokens**, so whoever lands this should record token figures, not these percentages. (critic — `rev-20260730T200123Z-1a9cce53`)

- **[GOV-3M9T]** The reconciled-files norm amendment did not cascade — `security-model.md` and `api-contract.md` still enumerate the carve-out without `CLAUDE.md`, and both were edited in the same bundle
  `effort: S · impact: M · area: governance · kind: bug · source: critic · added: 2026-07-30 · status: open · stage: ready · related: GOV-4B9N, GOV-7W3D · refs: .prawduct/artifacts/architecture.md:67,69,80 (the amended norm — `[DECISION: name CLAUDE.md in the reconciled-files carve-out …]`, dated 2026-07-30, plus the Retroactivity line that spells out "**and `CLAUDE.md`**"), .prawduct/artifacts/security-model.md:58-60 (§ Least authority over the machine — enumerates `.prawduct/`, the evidence store, `.gitignore`, `.claude/settings*.json`; **no `CLAUDE.md`**), .prawduct/artifacts/api-contract.md:235-237 (§ Least authority — the same enumeration, same omission), plugin/lib/migrate_plugin.py:60 (`_EDIT_IN_PLACE = frozenset({"CLAUDE.md", ".claude/settings.json"})` — the code proving the enumeration incomplete)`

  **Verified against the tree 2026-07-30.** `architecture.md` was amended this bundle to name `CLAUDE.md` in the reconciled-files carve-out, on the reasoning that the enumeration was *incomplete at ratification* rather than permissive-by-omission — `init_product.py` has created-or-anchored `CLAUDE.md` and `migrate_plugin.py` has edited it in place since before the norm was written. Two sibling artifacts restate the same enumeration and neither was updated. Both were edited in this same bundle, so the omission is a missed cascade rather than an untouched file.

  **Why it is worth an item rather than a silent patch.** As it stands, three governing artifacts describe the plugin's write authority and two of them describe an authority the installer demonstrably exceeds — a reader who reaches `security-model.md` first concludes the plugin violates its own least-authority norm by anchoring `CLAUDE.md`. This is the exact class the repo already has a learning for (*"when a scope narrowing is recorded, it is a CASCADE, not an annotation"*) and the exact shape of **GOV-4B9N**, where a ratified amendment sat in one artifact while another kept the pre-amendment form. Fix is mechanical — restate both enumerations to match `architecture.md` — but the fix should also carry the falsifying grep so a fourth copy can't hide. (critic — `rev-20260730T200123Z-1a9cce53`)

- **[CRT-8K6M]** `risk_surfaces:` changes review depth for every onboarded repo but has no discovery question and no concerns-registry row — the only path to opting in is reading a template comment
  `effort: S · impact: M · area: governance/critic · kind: task · source: critic · added: 2026-07-30 · status: open · stage: ready · related: CRT-2W8J, GOV-8C3W · refs: plugin/templates/project-state.yaml:253-274 (the `# RISK SURFACES (opt-in)` block — the product-facing config, including the `risk_surfaces: []` explicit opt-out), plugin/methodology/building.md:182 (the Builder surface — points at review-cycle.md for the roster rule "which depends on whether the repo declares `risk_surfaces:`"), plugin/skills/critic/review-cycle.md:46 (the definition — "a changed path matching this repo's `risk_surfaces:`"), plugin/lib/critic_consolidate.py (`_derive_roster` — the Critic surface, code-derived), plugin/methodology/discovery.md (**the gap** — verified 2026-07-30 to contain no risk-surface or cost-of-a-missed-defect question), .prawduct/cross-cutting-concerns.md:36-37 (the two rows this bundle *did* add, for the record-lint concerns — no row for this one)`

  **Pipeline coverage as shipped, checked surface by surface (2026-07-30):** Artifact **present** (the template block), Builder **present** (`building.md:182` → `review-cycle.md:46`), Critic **present** (`_derive_roster`, code-derived), Discovery **absent**. Nothing in `discovery.md` asks the question that would produce the answer — *where would a missed defect cost you most?* — so the only route to declaring `risk_surfaces:` is a product owner reading a comment block in a template they may never open.

  **Why the absent leg is the load-bearing one.** An unasked question is an unanswered one, and the fallback is silent by design: a repo that declares no risk surfaces keeps the older file-count rule **forever**, with nothing ever prompting it to reconsider. That fallback is correct (it is what makes the roster change safe to ship on this repo's evidence alone — see **CRT-2W8J**), but it converts a missing discovery question from a documentation nicety into the thing that decides review depth for every onboarded product. The registry row is the cheaper half: `cross-cutting-concerns.md` gained rows for both record-lint concerns in this bundle and none for this one, so the concern is not discoverable through the registry either. (critic — `rev-20260730T200123Z-1a9cce53`)
...
- **[CRT-8L3Q]** The ledger anchor's residual **overlap** window needs a **lock**, not just a probe — `review_event_exists` is read-then-write, so two consolidations can both probe before either appends
  ...
  <!-- refs are NAME-anchored, not line-anchored, per the convention CRT-R4Z2 sets: the probe landed
       2026-07-29 and further consolidate edits will shift these line numbers again. -->

  `review_event_exists` (shipped 2026-07-29) closes the **REPLAY** path completely — the same
  manifest and partials re-materializing after success, or a crash between the fact append and
  `remove_partials`. It only **NARROWS** the **OVERLAP** path: the probe is read-then-write with
  **no lock anywhere** in `critic_consolidate.py`, `evidence.py`, or `ledger.py`, so two
  consolidations running past the manifest-existence check at once can both probe before either
  appends. The window shrank from the whole consolidate body to the microseconds between probe and
  append — narrowed, not closed, and not "exactly once."

  **Verified 2026-07-29** (this filing, not inherited): no `flock`/`fcntl`/`O_EXCL`/`os.rename`/
  `os.replace` appears in any of the three modules — the only `mkdir` calls are `parents/exist_ok`
  directory setup, not claims. `aggregate_review_stats` builds one row per event and reports
  `events_total: len(rows)` with no dedupe on `review.fact_id`, so a surplus anchor inflates the
  report directly.

  **Trigger for doing the work:** a *second* surplus `review.*` event appearing in the ledger.
  Today there is **exactly one**, and it is nameable — `rev-20260729T233201Z-d91acd9e`, anchored
  twice a second apart (the live observation `review_event_exists`'s docstring records). Naming it
  matters: the trigger is "a second surplus," so the known one must be distinguishable from a new
  one. Count with:
      ...
      for line in open('.prawduct/.governance-ledger.jsonl'):
          ...
          try:
              ...
              continue
          if not str(e.get('event', '')).startswith('review.'):
              continue
          fid = (e.get('review') or {}).get('fact_id')
          if fid:
              c[fid] += 1
      print('surplus:', sum(c.values()) - len(c))
      print({k: v for k, v in c.items() if v > 1})
      PY

  As of filing: 283 `review.*` events, 143 carrying a `fact_id` over 142 distinct ids (surplus 1),
  plus 140 pre-field events with no `fact_id` at all — those predate the key and are invisible to
  both the probe and any count, which is itself a reason to key the trigger on *duplicated ids*
  rather than an events-vs-reviews subtraction.

  **Why it matters now rather than later.** Two reasons, both from the same release: (a) v3.2.1
  mandates **concurrent** coordinator reviewer dispatch, which makes overlap *more* reachable, not
  less — `review_event_exists`'s docstring says so explicitly; and (b) `review-stats` is the
  instrument the record-mechanization plan's own success criterion reads ("median rounds per
  logical change at or under 2"), with Chunks 03 and 05 leaning on those numbers. A surplus anchor
  inflates the very measure review proportionality is judged on.

  **Candidate fix** (per the review): have `consolidate` **CLAIM** the manifest — atomic rename —
  *before* doing work, rather than gating only on its existence. That converts the manifest check
  from a read-then-write test into the mutual-exclusion primitive it is standing in for, and makes
  the probe a belt-and-suspenders backstop rather than the whole guard. Note this resurrects the
  original fix-shape of the archived CRT-4B7X ("a lightweight lock / rename-guard around the
  consolidate merge so exactly one firing wins") — that item was archived "resolved by design"
  because kernel-v3 made the *store fact* idempotent, which killed the store race but never
  supplied the lock. (critic)

- **[CRT-R4Z2]** Reviewers file **meta-notes as findings** — a note *about* another finding (a Learnings Cross-Check entry restating a finding, a note restating another finding's consequence) inflates the count and the disposition workload. A protocol/severity question, not a merge-key one. *(Reporting half shipped 2026-07-29 — see Status.)*
  `effort: S · impact: M · area: critic · kind: bug · source: critic · added: 2026-07-29 · reviewed: 2026-07-29 · status: open · stage: design · related: CRT-4T7M, CRT-8N5V, CRT-3W6P, CRT-4X2N, CRT-2W8J · refs: plugin/skills/critic/review-protocol.md (§ Severity Levels and § "Learnings Cross-Check and Backlog Reconciliation" — where the remaining meta-note half lands: what a reviewer does when its observation is *about* another finding rather than an independent defect), plugin/skills/critic/review-protocol.md (§ Coordinator Pattern — the per-role dispatch prompt "Review ONLY <GOALS>", which guarantees two reviewers seeing one defect file it under different goals), plugin/lib/critic_consolidate.py (`likely_duplicate_groups` + `distinct_finding_count` + `DUPLICATE_SIMILARITY` — the SHIPPED advisory half; `merge_findings` and its `(goal, name, files)` key, deliberately unchanged; `consolidate`'s stdout duplicate note; `fact_to_cache_record`'s `likely_duplicate_groups` key; `COORDINATOR_ROSTER` — the three disjoint goal sets), tests/test_critic_consolidate.py (`TestLikelyDuplicateGroups` incl. `test_the_real_cross_goal_duplicate_is_grouped`, the live R-7/R-13 anchor; `TestDeterministicCycleEndToEnd::test_consolidate_reports_a_likely_duplicate_group` and the clean-review assertions in `test_begin_partials_consolidate_full_cycle`)`
  <!-- refs are deliberately NAME-anchored, not line-anchored: the 2026-07-29 commit shifted every line number the previous refs cited. -->

  **Status — the reporting half SHIPPED 2026-07-29; the meta-note half remains.** Shipped on branch scope `record-mechanization`, outside every chunk's scope: `likely_duplicate_groups` (union-find over cross-goal findings with non-disjoint file attributions whose significant title words pass a `DUPLICATE_SIMILARITY = 0.4` Jaccard overlap; transitive, so one defect found by all three reviewers reports as one group) and `distinct_finding_count`; a `consolidate` stdout note (`(~M distinct; K likely-duplicate group(s): R-7+R-13)`); a `likely_duplicate_groups` key in the derived `.critic-findings.json` view; 10 tests, including `test_the_real_cross_goal_duplicate_is_grouped` (the actual R-7/R-13 pair from the observation below — 4/9 words → 0.444, a genuine regression anchor rather than a tautology) and two end-to-end integration tests (the grouped case and the clean-review no-note case).

  **Caveat on the derived-view key:** `likely_duplicate_groups` in `.critic-findings.json` has **no programmatic reader** today — it is there for a human (or a JSON consumer) reading the view. A test asserts it is always written (empty list when nothing looks duplicated), so it cannot silently stop being written; but nothing downstream consumes it, so don't assume a reader exists when extending it.

  **Decided 2026-07-29 — all three questions this item left open are now answered.**
  1. **The judgment lives in deterministic code**, not a model-side consolidation step. Kernel-v3 forbids a model in the fact write path, and this feeds the recorded fact and its derived view.
  2. **It is advisory reporting — never a collapse or a discard.** Nothing merges, drops, or reorders a finding. A fuzzily-dropped real finding is *invisible* in the merged output, which is strictly worse than over-counting; so the code reports and the builder decides. (This settles the `goal`-attribution question too: no finding is collapsed, so nothing loses its goal.)
  3. **It IS two problems, and only the first shipped.** Cross-reviewer duplication of one defect → the shipped advisory grouping. Reviewers filing meta-notes as findings → the remaining half, below.

  **Remaining scope (`stage: design`) — the meta-note class.** A reviewer files an observation that is *about another finding* rather than an independent defect: a Learnings Cross-Check entry that restates a finding (observed: R-23 about R-20), or a note restating another finding's consequence (observed: R-25 restating R-1). This is **not** a merge-key problem — the two findings are genuinely different text about genuinely different subjects, so no similarity threshold should or does group them. It is a **protocol/severity** question for `review-protocol.md`: what should a reviewer do when its observation is bookkeeping about, or a consequence of, another finding — fold it into that finding, file it at a lower severity, or not file it at all? Design must also say how a reviewer *knows*: within one reviewer's own partial this is self-evident, but a cross-reviewer meta-note (the Learnings Cross-Check reviewer commenting on a correctness finding) is written before the other partial is visible. Sequencing note: this half changes reviewer *instructions*, so it interacts with any dispatch-path change (CRT-3W6P) but not with `merge_findings`, which this item no longer proposes to touch.

  ---

  *History below — the original filing. Preserved verbatim; the merge-key analysis is what the shipped advisory half acted on.*
...
- **[CRT-3W6P]** Converge-by-construction re-review policy enforced in `critic-begin` — round 2+ on the same chunk should be verify-resolutions only, and a full re-round should be refused when zero blocking findings remain
  ...
  **Reproduced independently in a consuming repo (v3.2.1), 2026-07-30 — the evidence is now cross-repo, not single-repo.** Four rounds where two would have done, on a diff the reporter considered ordinary. Round 1 (`chunk`): 1 blocking + 3 warning, from the actual work. Round 2 (`verify-resolutions`): 2 warning + 1 note, **all three written while fixing round 1**. Round 3 (`verify-resolutions`): 1 note, written while fixing round 2. So **100% of rounds 2–3's new findings were self-inflicted by the fixing**. Same pump as the filing evidence, on a *released* version, in a repo that is not this one.

  **Round 4 is the reporter's own counter-example, and it is the sharpest design constraint this item has acquired.** Round 4 (`cumulative`, run after merging develop) was **not** loop waste: the merge brought ~4,300 lines into functions the branch had already changed, and no prior review had seen that combination. A policy keyed only on *"same chunk, round 2+"* would have wrongly refused it. So the refusal must discriminate **"extra round caused by fix churn"** (the target) from **"new round because the tree genuinely changed for a reason outside the review loop"** (must stay allowed). This sharpens the first open design question above: the discriminator is not a round counter but *what moved the tree between rounds* — and note the evidence store already distinguishes these, since a fix-churn round's delta is exactly the files the previous round's findings named.

  **Every self-inflicted finding was one defect class: unverified factual claims in fix-time prose.** The reporter's four instances — a waiver reason stating how a library raises errors (wrong); a comment saying a set was "derived from the enum" when the next line was a literal; a comment claiming two code paths shared a function when one never called it; a claim true prospectively but false for existing data. The mechanism the reporter names: *while fixing a finding you write prose explaining why, and that prose asserts things you have not verified, because you are in fix-mode rather than verify-mode.* That is a **generative** account of the pump rather than a tally of it — it explains why round N+1 reliably finds something: the fixing itself manufactures the next round's findings.

  **The learning-loop datum, and the strongest single argument for building this.** The reporter had already written the governing rule — *"a comment asserting how a dependency behaves is a factual claim: cite the read or don't write it"* — **hours before** committing two more instances of exactly that. Writing the rule did not install it; independent review caught the instances and self-discipline did not. Direct evidence for preferring a **structural** guard to a documented one for this class, and it argues that CRT-8N5V's remaining meta-note/severity half cannot be closed by reviewer *instructions* alone.

  **Reporter's proposed mitigations — evaluate during design, do not adopt wholesale.**
  1. Freeze the tree before invoking, and batch fixes: collect findings → fix all → review once.
  2. Treat fix-time prose as new code needing review.
  3. Prefer a structural guard over a comment promising one. Their best fix replaced a comment (*"a transport added later must decide which side it falls on"*) with a `require_total(...)` that fails at import — a promise the compiler keeps needs no re-review. This is the same argument this item makes about prose, one level down.
  4. Document which files are free to write during an active review: gitignored files (handoff notes, reflection) don't move the tree hash and can be written mid-review; tracked files can't, and doc-only edits are meant to be skipped by the gate. **Already true of the mechanism, but undocumented where the operator meets it** — verified 2026-07-30 that nothing in `plugin/skills/critic/` or `plugin/methodology/building.md` states it, and no open backlog item covers it. Cheapest actionable piece here; **filed separately as CRT-9B4K** (2026-07-30) pointing at `plugin/skills/critic/review-cycle.md` rather than absorbed into this one — different surface, doc-only, no dispatch-path change.
  5. Two rounds is the floor, not one: review → fix → `verify-resolutions` is the minimum. **This must be reconciled with rule 2 of the policy above**, which as written refuses a re-round whenever zero blocking findings remain — a round 1 that finds warnings-but-zero-blocking would be refused by rule 2 while mitigation 5 would still run the verify pass. Which is right is a design call this item now has to make explicitly rather than inherit.

  **Corroborated in this repo the same week, independently.** Chunk 03 of `record-mechanization` took **six** rounds; rounds 4–6 found only record-prose defects in the change-log entry narrating the previous round's fix — same pump, same defect class. See `.prawduct/reflections.md` § "The round pump, observed on itself" (not restated here), which reaches the same conclusion from the inside and already names this item as the structural answer.

- **[CRT-9B4K]** `review-cycle.md` never says which files are free to write while a review is in flight — the operator meets this exact question mid-review, and guessing wrong costs a full extra round
  `effort: S · impact: M · area: critic · kind: chore · source: user · added: 2026-07-30 · status: open · stage: ready · related: CRT-3W6P, CRT-8N5V · refs: plugin/skills/critic/review-cycle.md (§ "The review loop terminates — and the builder is what terminates it" — the natural home; the section a reader in this situation is already in), plugin/lib/coverage_algebra.py (`is_judgeable_path` — the doc-only carve-out's classifier), plugin/lib/coverage.py (`_pr_diff_is_doc_only` / `check_pr_doc_only` — the doc-only fast path), .gitignore:7,11,12 (`.prawduct/.handoff-notes.md`, `.prawduct/.session-handoff.md`, `.prawduct/.session-reflected` — the ignored session files, verified ignored 2026-07-30), tests/test_v5_methodology.py (`TestBuildingMd` token-budget assertion — the `< 4660` ceiling that rules out the building.md half), CRT-3W6P § "Reporter's proposed mitigations" item 4`

  **The rule to document.** *Which files are free to write while a Critic review is in flight.* Gitignored files do not move the tree hash, so `.prawduct/.handoff-notes.md`, `.prawduct/.session-reflected` and the other ignored session files can be written mid-review **without voiding coverage**. Tracked files cannot: any tracked write means no review fact covers the current tree, and the gate blocks.

  **Name the doc-only carve-out as a *separate* thing, or the two will be conflated.** Doc-only edits are also meant to be skipped by the gate — but by a different mechanism and for a different reason (`coverage.py`'s doc-only allowance, classifying through `coverage_algebra.is_judgeable_path`). That is a judgeability carve-out on *tracked* files; the rule above is about files the tree hash never sees. A reader who fuses them will conclude that tracked doc edits are safe mid-review, which is the wrong lesson in the wrong direction.

  **Why document it rather than leave it as folklore.** The operator meets this question at exactly one moment: a review is running and they want to bank progress. Getting it wrong costs a full extra review round; getting it right converts otherwise-dead waiting time into finished work. It is the cheapest of the five mitigations in CRT-3W6P's external v3.2.1 report — no dispatch-path change, no policy design, just **stating a property the mechanism already has**.

  **Destination, and the budget answer pre-computed so nobody burns a round rediscovering it.** `plugin/skills/critic/review-cycle.md` should carry this **alone**. A sentence in `plugin/methodology/building.md`'s Critic section would be natural, but that file is at **4648 estimated tokens against a `< 4660` ceiling — 12 tokens of headroom** (verified 2026-07-30), under a standing trim-or-relocate rule that requires any addition to fund itself. It will not fit; don't try, and don't raise the ceiling for it.

  **Caveat — the doc is the floor, not the fix.** CRT-3W6P's own evidence is that writing a rule down does not install it (its reporter authored the governing rule hours before committing two more violations of it). So if this mistake keeps happening after documentation, the answer is **not more prose**: it is a dispatch-time warning from `critic-begin` naming the ignored-file set, so the operator is told at the moment the question arises rather than in a guide read earlier. File that separately if it comes to it — this item is deliberately the cheap half.

  Doc-only, but governance-protected (`plugin/skills/`) → full Critic + PR review. (user — 2026-07-30, from CRT-3W6P's external reproduction)
...
- **[REL-6Q4M]** `check-releasability` is blind to a release-pending change-log entry that carries no `scope=` — it enumerates *scopes*, so a scopeless entry contributes nothing to enumerate and Phase 0 certifies `releasable` while unclassified work exists
  ...
  **The defect.** The gate enumerates *scopes*, not entries. `release_pending_scopes` collects statusless (release-pending) entries and extracts `scope=` from each; an entry with no `scope=` key contributes no scope, so it is never enumerated, never appears in the "N scope(s) are release-pending" list, cannot be classified against the release plan's `## Release classification` table, and cannot be withheld. Phase 0 therefore reports `releasable` while unclassified release-pending work exists — **the gate certifying a release while blind to part of it, which is the exact failure class the gate was built to prevent** (REL-8P6M part (f)).

  **Found live during the v3.2.0 release.** The scope enumeration accounted for 40 statusless entries while the file held 43. The three invisible ones (headings "the backlog service came back, laid out under plugin/…", "`--archive-scope open` stops promising a backup that cannot exist…", "`--archive-scope` becomes discoverable…") were all code-tested as `plugin/lib/backlog/` work absent from every release tag — genuinely unshipped, and genuinely unclassifiable by the gate. Mitigated for those three **by hand** (given `scope=backlog-service-v1` during Phase 1 tagging), but the gate hole is unfixed and recurs the next time anyone writes a tag line without a scope.

  **Why an absent scope is an ERROR, not an absence.** Unlike a missing *classification* — which the gate names and refuses on — a scopeless release-pending entry cannot even be *named* in the plan: the classification table is keyed by scope, so there is no row shape that could describe it. The gate's fail-closed posture therefore has a gap it cannot express, only count.

  **Fix shape.** `check_releasability` should count statusless *entries* as well as scopes and refuse when the two disagree. One open decision: **refuse (exit 1) vs. report**. Refusing is consistent with the rest of Phase 0 (un-evaluable state fails closed), and the remedy is a one-line change-log edit, so refuse looks right — but it makes a previously-passing gate fail on any repo with a legacy scopeless entry, so confirm before implementing. Note the code *already knows*: the `if not pending:` branch (`release_readiness.py:288-294`) documents "entries that carry no `scope=` key at all and are therefore invisible to this gate" — and still returns 0, printing only a `N entries scanned, M tagged` detail string. The accounting exists in the branch where nothing is pending and is absent from the branch where something is; that asymmetry is the whole bug in one place.

  **Correction to the filing note, verified against the code.** The report added *"the same blindness affects `regen-views` release-notes grouping, which groups by `release=` but reports rollups per scope."* Read: **release-notes grouping is NOT blind** — `_group_release_entries_by_scope` (views.py:951-966) explicitly keeps scope-less entries as their own group keyed by title, and `build_release_notes_view` renders `entry['scope'] or entry['title']` (views.py:1020), both documented as deliberate. What *does* share the guard is (a) `collect_release_pending_scopes` (views.py:593), so a scopeless release-pending entry flips no plan status, and (b) `_collect_scope_rollups` (views.py:802), so a scopeless *shipped* entry never lands in `scope_rollups:` in project-state.yaml — the latter documented as intentional ("Only entries with `status=shipped` AND a non-empty `scope=` tag contribute"). So the sibling exposure is real but sits in the plan/rollup path, not the release-notes path. Fixing this item should decide whether (a) deserves the same entry-vs-scope reconciliation. (builder — v3.2.0 release, Phase 0/Phase 1 walk)

- **[REL-3K9P]** The pruned-promotion runbook is unverified and nothing scheduled can clear it — its named verifier is a *version* (v3.2.0's run), but only a promotion *shape* opens the runbook, so the trigger turns on a fact it never states
  `effort: S · impact: M · area: docs · kind: fix · source: critic · added: 2026-07-29 · reviewed: 2026-07-29 · status: open · stage: ready · related: REL-8P6M, REL-7D4X, MET-7B3X · revisit: the next pruned promotion, whenever one occurs — the only run that opens this runbook; verify it end-to-end during that run and stamp `last_verified`/`verified_by` · refs: .prawduct/runbooks/promote-a-pruned-release.md (front matter `last_verified: null` / `verified_by: null`; tier 3, 14 steps), .prawduct/backlog.md § Archive → REL-8P6M, Disposition 2026-07-29, final bullet "Not verified end-to-end" (the same wrong-shape trigger asserted there — struck and annotated in place by this item on 2026-07-29, so the two records point at each other instead of contradicting 3,400 lines apart), .prawduct/artifacts/build-plan-release-readiness.md (Chunk 02 "Done when" 5 — the corrected trigger, which names this item as the tracker), .prawduct/.pr-reviews/feature--rel-8p6m-releasability-gate.json (finding W-2, where this was surfaced), .prawduct/runbooks/cut-and-publish-a-plugin-release.md (Phase 0 routing between shapes — the reason a *shape*, not a version, is what decides whether the pruned runbook opens; Phase 2's whole-develop path is the other branch), plugin/templates/runbook.md (`last_verified:` defined as "date this was EXECUTED or rehearsed — not edited"), plugin/skills/runbook/SKILL.md (the review question "is `last_verified` a date it was executed, and how stale?" — the field's only reader, and it is model-side on demand, not an advisory)`

  **The gap.** `.prawduct/runbooks/promote-a-pruned-release.md` shipped (REL-8P6M, Chunk 02) with `last_verified: null` — correctly, since that chunk *wrote* the procedure rather than exercising it. The problem is the **shape** of the verifier it named. Chunk 02's "Done when" 5 said *"v3.2.0's actual run is what verifies it."* A version number cannot be a verifier for this runbook, because whether the runbook opens at all is decided by the promotion's **shape**: Phase 0 of `cut-and-publish-a-plugin-release.md` routes a whole-develop promotion down Phase 2 (which never opens the pruned runbook) and a pruned one into it. So the trigger turns on a fact it never states, and reading it tells you nothing about whether it will fire. That is the defect — and it holds whatever shape v3.2.0 lands on.

  **Confidence boundary (Principle 5) — overclaim withdrawn 2026-07-29.** An earlier version of this item asserted that v3.2.0 "is planned **non-pruned**" and concluded the old trigger is *"not merely unlikely — it is **structurally unreachable**."* Both are withdrawn as unsupported. The planned-shape premise has **no source in the tree**: no `release-plan-v3.2.0*.md` exists; `project-state.yaml` carries only `active_build_plan: artifacts/build-plan-v3.2.0-golive.md`; `operational-spec.md` names v3.2.0 once, as an *unratified* minor-tier example ("planned for the backlog-service go-live"), and says nothing about its promotion shape; and the owner's intent for that shape lives only in untracked session handoff notes. It is a **plan, not a structural fact** — if v3.2.0 withholds any scope then it *is* pruned, this runbook *does* open, and the old trigger fires after all. "Structurally unreachable" was therefore doing work the evidence does not support: at most the trigger is *contingent on an unrecorded decision*, which is a weaker and truer claim. **The defect does not depend on the overclaim** — a *version* is the wrong kind of trigger for a *promotion shape*, and a trigger whose firing hinges on an undocumented plan is exactly the failure being corrected. (Credit: delta review of `c68443d`.)

  **And nothing else tracked it.** Three surfaces that could have caught the dangling flag all miss it: REL-8P6M is archived `status: shipped` (an archived item tracks nothing); no advisory fires on `last_verified: null` (verified — no code under `plugin/lib/` reads the field at all, its only consumer is a model-side review question in `skills/runbook/SKILL.md`); and until this item, no open backlog item referenced the runbook. So a tier-3 procedure entered the operational canon with an unverified flag that had no scheduled clearing event and no owner. That absence *is* the defect this item closes — the item is the tracker.

  **The trigger, restated soundly:** the next pruned release, whenever one occurs. Verify the runbook end-to-end during that run and stamp `last_verified`/`verified_by` from the real execution. Do not name a specific future version as the verifier again — that is the mistake being corrected; the trigger is a *promotion shape*, not a version number. (prawduct has pruned twice, v3.1.1 and v3.1.2, so this will fire; it just cannot be scheduled.)

  **Scope — the publish half is what is actually unverified.** Worth keeping straight so this isn't over-read. The command set was already **rehearsed against the real v3.1.2 candidate** (rebuilt from `v3.1.1` with `git diff … --3way`): it reproduced that release's three recorded conflicts and caught a missing `import sys` in the prune. So the candidate-tree-build half of the runbook is evidence-backed. What no one has executed is the **publish** half — publish-by-ref, tag, and the per-shape `Done when` checks. Verification effort is therefore small (`effort: S`); the value is in closing an untested step in an irreversible procedure. (critic — PR reviewer W-2 on `feature/rel-8p6m-releasability-gate`)

- **[CRT-5H2D]** A BLOCKING finding recorded on a zero-length review interval (`base_tree == head_tree`) is structurally invisible to every composition gate — `review_edges` drops the self-loop, so the finding can never even produce `blocked`
  `effort: M · impact: L · area: critic · kind: bug · source: user · added: 2026-07-29 · reviewed: 2026-07-29 · status: open · stage: design · related: CRT-9T6M, CRT-6D2N, CRT-7H2W, CRT-5D8Q, CRT-4J8W, CRT-7V4N, GOV-2R8W · refs: plugin/lib/coverage_algebra.py:178-179 (`review_edges` — `if src == dst: continue  # degenerate`), :280-281 (`coverage_verdict` — `base_tree == target_tree` short-circuits to `covered` with an empty path), :285-293 (the two-phase clean-edges / all-edges search), plugin/lib/critic_consolidate.py:401 (the empty-diff refusal that EXEMPTS `verify-resolutions` — where the degenerate interval is minted), :220 (`begin_review`), plugin/lib/gates.py:983 (`check_cumulative_critic`'s `coverage_verdict` call), :666 and :689 (the other two composition gates through the same predicate)`

  **Observed live 2026-07-29.** Review `rev-20260729T194336Z-5753c20e` recorded an R-1 **BLOCKING** finding on the interval `04b290eba235 -> 04b290eba235` after a re-dispatch against an unchanged tree. `prawduct-hook check-cumulative-critic` then reported `satisfied … 0 unresolved blocking`. A real blocking finding existed, was correctly recorded in the evidence store, and gated nothing.

  **Mechanism — VERIFIED by reading the code, and stronger than originally reported.** The filing inferred that `compose` "finds a path over clean edges first and never needs the self-loop" (`coverage_algebra.py:285-291`). That is true but is the *mild* reading. The code is worse in two independent ways:

  1. **The self-loop is never an edge at all.** `review_edges` (`coverage_algebra.py:178-179`) ends with `if src == dst: continue  # degenerate; contributes nothing a node doesn't`. So the fact is excluded from the graph before either search phase runs — not just from `clean_edges`, but from the second-phase `_find_path(edges, …)` (`:293`) whose entire job is to return `blocked` and enumerate unresolved findings. **A blocking finding on a self-loop can therefore never produce a `blocked` verdict under any evidence configuration** — it is not "shadowed by a cleaner path," it is unreachable. The comment's premise ("contributes nothing a node doesn't") is true of *connectivity* and false of *findings*: the edge carries a payload the node does not.
  2. **A zero-length gate span is unconditionally `covered`.** Separately, `coverage_verdict` (`:280-281`) returns `{"status": "covered", "path": []}` whenever `base_tree == target_tree`, **before** `review_edges`/`resolution_index` are even built. So when the gate's own span is degenerate, every blocking finding recorded at that tree is ignored by construction. This is a second door to the same outcome and any fix touching only `review_edges` misses it.

  **Where the degenerate fact comes from (the mint site).** `critic_consolidate.begin_review` refuses an empty diff — *except* for one mode: `if not files_changed and mode_token != "verify-resolutions"` (`critic_consolidate.py:401`). `verify-resolutions` is deliberately exempted (it must be able to run when only findings, not files, changed), and nothing downstream validates `base_tree != head_tree`. So a `verify-resolutions` re-dispatch against an unchanged tree mints a manifest with `base_tree == head_tree`, consolidation records a normal review fact on it, and the algebra silently discards it. `validate_manifest` does not reject the degenerate interval either.

  **Why this is severe rather than cosmetic.** This is the one failure mode the kernel-v3 design is supposed to make impossible: the gate does not merely fail to *find* evidence (which is loud — `uncovered`, with a remedy), it reports **`satisfied`** while holding an unresolved BLOCKING fact in the store. Fail-open, silent, and indistinguishable from a genuinely clean branch. It also inverts the store's stated invariant that "a partial or malformed fact must weaken coverage, never strengthen it" (`review_edges` docstring) — here a fact carrying a blocker weakens *nothing* and lets a clean path through unchallenged.

  **Fix shape (design decision needed — hence `stage: design`).** The blocking-findings question and the connectivity question are currently answered by the same edge set, and that conflation is the root cause. Candidates: (a) keep `review_edges` connectivity-only but have `coverage_verdict` sweep **all** review facts whose `head_tree` is reachable on the found path (including self-loops at those nodes) for unresolved blocking findings before returning `covered` — this also closes door 2, since the `base_tree == target_tree` short-circuit would still have to run the sweep at that node; (b) refuse to *record* a degenerate interval at the mint site (`begin_review`) — cheaper, but it discards a legitimately-recorded finding rather than honoring it, and needs a story for what `verify-resolutions` does when a real blocker survives with no tree change; (c) both — (b) as prevention, (a) as the correctness fix for facts already in the store. Note the store is append-only and shared across worktrees, so any already-recorded self-loop facts persist and (a) is what makes them count. Whatever lands needs a regression test that pins the exact live case: record a BLOCKING fact at `T -> T`, compose over a span through `T`, assert the verdict is `blocked` (not `covered`) — an assertion that fails on today's code. Governance-protected (`plugin/lib/coverage_algebra.py`, `plugin/lib/gates.py`, Critic data plane) → full Critic + PR review. (user — observed live)

- **[CRT-9T6M]** `critic-begin` has no in-flight guard — a concurrent dispatch destroys another review's manifest and partials mid-review, silently losing its findings and chunk attribution
  `effort: S · impact: M · area: critic · kind: bug · source: user · added: 2026-07-29 · reviewed: 2026-07-30 · status: open · stage: design · related: CRT-5H2D, CRT-2X7R, CRT-3X9D, CRT-7Q2T, CRT-3F7M, CRT-6Q2N, CRT-4B7X, CRT-8L3Q, BLD-7W2J · refs: plugin/lib/critic_consolidate.py:447-451 (`begin_review` — the unconditional `remove_partials` + manifest overwrite, and the `cleared_leftovers` flag that reports it as routine), :220 (`begin_review`), :1077 (the `--chunk` the ledger argv reads back off the manifest), `partial_path` (the fixed, un-keyed `<role>.json` rendezvous — the second collision path; name-anchored, since this branch has moved the numeric anchors above), plugin/bin/prawduct-hook (`cmd_critic_begin` — its docstring documents the partials reset as intentional; the PDT-WT9K wrong-tree refusal shows where a refusal belongs), CLAUDE.md § The Critic ("while a review is active … the session-mutating `prawduct-hook clear` refuses to run")`

  **Observed live 2026-07-29**, same session as **CRT-5H2D**: a concurrent `critic-begin` rewrote another in-flight dispatch's manifest mid-review, and the resulting ledger event lost its chunk attribution. Two agents driving Critic runs in one session is not guarded anywhere.

  **Mechanism — verified in code.** `begin_review` (`critic_consolidate.py:447-451`) unconditionally calls `remove_partials(prawduct_dir)` and then `atomic_write_text(manifest_path(…), …)`. There is no check for an existing manifest, an in-flight review id, or the critic-active marker. `cmd_critic_begin`'s docstring documents the reset as *deliberate* — "consolidate removes partials only on success, so a waived or stale-failed review leaves them behind, and a leftover partial at the SAME commit as a fresh dispatch would merge as if the new reviewer had written it." That rationale is sound for the **stale** case and wrong for the **concurrent** case, and the code cannot tell them apart. Consequences, in order of severity: partials already written by the first review's reviewers are deleted (findings lost outright); surviving reviewers write partials that consolidate merges against the *second* manifest, so `files_reviewed`/`base_tree`/`head_tree`/`chunk` are attributed to the wrong interval; and the ledger `review.critic` argv reads `--chunk` straight off whatever manifest is on disk (`:1077`), which is how the observed attribution loss happened.

  **Why it is the same gap class as CRT-3X9D and CRT-7Q2T.** The load-bearing invariant is enforced at exactly *one* mutation site — the critic-active marker makes `prawduct-hook clear` refuse, and nothing else. `critic-begin` itself, the single most destructive operation against an in-flight review, sets that marker but never consults it.

  **Fix shape.** Have `begin_review` distinguish stale from live before clearing: if a manifest exists whose dispatch is inside the in-flight grace window (`_INFLIGHT_GRACE_MINUTES` already exists for the consolidate-side liveness verdict — reuse it rather than inventing a second notion of "live"), refuse with a named remedy (`critic-end` to abandon, or wait) instead of silently overwriting; past the window, clear as today. A dispatch-id-keyed guard is likely the shared primitive — **CRT-6Q2N** and **CRT-4B7X** both want one on the consolidate side, so check whether one design covers all three before building (they are flagged as merge candidates at next triage). Needs a test that pins the live case: manifest A on disk inside the grace window → `critic-begin` for B refuses and leaves A's manifest and partials intact. Governance-protected (Critic dispatch path) → full Critic + PR review. (user — observed live)

  *Cross-link refresh 2026-07-29:* both consolidate-side siblings are now **archived shipped** — CRT-4B7X (`kernel-evidence-store ch.03`) and CRT-6Q2N (`record-mechanization`), the latter by `ledger.review_event_exists`. So the shared primitive already exists in one place; read it (`plugin/lib/ledger.py`) rather than designing it fresh, and note that it is an *idempotency probe, not mutual exclusion* — the unlocked-overlap residual it leaves is **CRT-8L3Q**, which is the still-open item this one should be designed alongside.

  **Observed live again 2026-07-30 — a second, distinct collision path the Mechanism section above does not cover.** That analysis is entirely about `begin_review` overwriting an in-flight *manifest*. Today's collision happened one layer down, at the **partials rendezvous**: `partial_path` resolves to `.prawduct/.critic-partials/<role>.json` — keyed by **role only, never by review id** — so two single-pass reviews in one worktree contend for one filename. Fork A dispatched `rev-20260730T173411Z-c8cb7f13` at 17:34:11Z; fork B wrote `reviewer.json` against **A's** manifest and consolidated it at 17:38:48Z; A's own `Write` then failed because the file already existed, and A re-dispatched as `rev-20260730T174026Z-c815625e` (consolidated 17:41:19Z) to record its findings. Chunk 03 therefore carries two review facts with different findings — both verifiable in the store.

  Two consequences a fix has to answer, beyond the manifest one:

  1. **The loser's only signal is a failed `Write`.** Nothing in the protocol says what a pre-existing `reviewer.json` means, so a fork that read that failure as terminal would have discarded its findings silently — and in this instance A's finding was **BLOCKING** (`R-1`: the payload split was inert on the instruction path). The silent-discard path runs straight through the severity that gates. Whatever guard lands, the rendezvous wants a review-id-keyed path — or an explicit refuse-and-name-the-remedy on collision — rather than a bare filename race.
  2. **A superseded fact's findings become unreachable** — filed separately as **CRT-2X7R**, since it is a disposition-surface gap that survives any dispatch-side fix.

  **The concurrency itself was operator error, and a documented liveness check would have prevented this instance.** The waiting fork used `until [ ! -e .critic-active ]`, started *before* the other fork had run `critic-begin` — so it exited instantly and read as "the review died," which is what prompted the re-dispatch. The unlocked window this item names is real and independent of that mistake; but the protocol should also say **how to wait for an in-flight review**, because no guard helps a caller who never observes the marker. (user — observed live)

- **[CRT-2X7R]** A superseded review fact's findings are permanently undispositionable — `verify-resolutions` anchors only to the derived cache's single `fact_id`, which always names the newest fact
  `effort: S · impact: M · area: critic · kind: bug · source: user · added: 2026-07-30 · reviewed: 2026-07-30 · status: open · stage: design · related: CRT-9T6M, CRT-7P5J, CRT-8L3Q, CRT-5H2D · refs: plugin/lib/critic_consolidate.py (`_prior_review_fact` — the cache-`fact_id` anchor, the only route a verify-resolutions dispatch has to a prior fact; `fact_to_cache_record` — where that key is set to the just-consolidated fact's id), plugin/lib/dispositions.py (disposition fact ids are `disp:<review_id>:<fid>:<seq>`, so an unanchorable review has no path to one), plugin/lib/evidence.py (append-only store — the superseded fact persists, it is simply unaddressable), .prawduct/.critic-findings.json (the derived view: latest fact only)`

  **Observed live 2026-07-30**, as consequence 2 of the CRT-9T6M partials collision — but the gap is not specific to that cause; any second review fact on the same chunk produces it (a re-dispatch after `critic-end`, a waived-then-redone review). Chunk 03 of `build-plan-record-mechanization.md` carries `rev-20260730T173411Z-c8cb7f13` (1 warning + 1 note) and, two minutes later, `rev-20260730T174026Z-c815625e`; by end of day the cache had advanced through three further facts for the same chunk. A live-pointer snapshot has no stable reading in a durable record, which is itself the point: read the cache's `fact_id` when you work this item rather than trusting any id written here. c8cb7f13's `R-1` warning and `R-2` note were both **fixed** (in `889f902` and `ad3c893`) and neither can be dispositioned by any verify pass.

  **Mechanism — verified by reading the code and the live store.** `_prior_review_fact` is the only way a `verify-resolutions` dispatch locates its anchor: it reads `.prawduct/.critic-findings.json`, takes the single `fact_id` key, and scans the evidence store for that one fact. `fact_to_cache_record` sets that key to the id of the fact **just consolidated**, so the pointer always names the newest review and every earlier fact for the same chunk goes unaddressable the moment a successor lands. Confirmed in the store by the query, not by a remembered count — group every `disposition` and `resolution` fact by `body.finding.review_id`: **146 reviews are targeted by at least one** (5 by a disposition, 145 by a resolution), and `rev-20260730T173411Z-c8cb7f13` is in neither set. It appears in the store only as a *resolving* review of someone else's findings, never as a target of its own. (A bare count here would drift with every review; the query is what reproduces.)

  **Scope — the norm breaks, not the gates.** Neither orphaned finding is blocking, and coverage composition reads the *facts*, not the cache, so no gate is presently wrong (contrast **CRT-5H2D**, where the gate itself is fail-open). What has no path to completion is the **disposition norm** — "every finding gets an explicit disposition" is unsatisfiable for a superseded fact. The case to design against is the one *not* observed: a **blocking** finding stranded the same way stays unresolved in the store indefinitely, and the composition gates would then correctly refuse it with no mechanism available to clear it.

  **Distinct from CRT-7P5J, which is the same root seen from the other side.** That item is the *read* side — the handoff summarizes the cache instead of composing resolutions over facts. This is the *write* side — the cache is also the only anchor a resolution pass can use, so a fact outside the pointer can never acquire resolutions at all. Fixing either leaves the other standing; both trace to a single-latest-fact view standing in for the store.

  **Fix shape (design decision needed — hence `stage: design`).** Candidates: (a) let `critic-begin --mode verify-resolutions` accept an explicit `--anchor <review-id>` resolved straight from the store, bypassing the cache — smallest, but it only reaches a fact whose id the operator already knows; (b) anchor on **every** review fact in the interval carrying undispositioned findings rather than the single newest — makes the norm mechanically completable and is the shape a disposition record-lint control would want anyway; (c) at minimum have the cache carry the superseded fact ids so the gap is *visible* rather than silent. Needs a test pinning the live case: two review facts for one chunk, findings on the older → a verify pass can dispose the older one's findings. Governance-protected (Critic data plane) → full Critic + PR review. (user — observed live)

- **[BKL-7V2D]** The completeness gate cannot see "imported but never reconciled" — a failed close is a warning string, and `verify-migration` never inspects issue state
  `effort: M · impact: L · area: backlog-service · kind: bug · source: critic · added: 2026-07-29 · reviewed: 2026-07-29 · status: open · stage: design · related: BKL-72AS, BKL-6X5D, BKL-6M4T, BKL-9J3F · refs: plugin/lib/backlog/migrate.py:1075 (`_reconcile_status`), :1089-1090 (the swallow), :1096 (`verify_migration`), :1163-1173 (the alias-coverage-only derivation), :909 (`_import_one_with_retry` — the retry that never sees the failure), plugin/lib/backlog/core.py:414 (`set_status`), :487-488 (`from_transport_error` — returns, does not raise), :915 (`iter_alias_issues` — the yield shape), .prawduct/artifacts/build-plan-v3.2.0-golive.md § Chunk 06 (:59, :87 — the irreversible operator-run migration this gates), .prawduct/artifacts/migration-scrub-decisions.md (decision 5 — `--archive-scope all`, which is what creates the close stretch)`

  **TRIGGER — must be closed BEFORE Chunk 06 of `build-plan-v3.2.0-golive.md` runs** (`:59` — "The real prawduct migration + VRF-006 (C4) — irreversible, operator-run"). This is deferral with a named gate, not open-ended deferral: Chunk 06 is the ~900-write irreversible run, and this item is a defect in the gate that says the run succeeded.

  Two legs, both verified by reading at tree `9e7a9b7`.

  **Leg 1 — a failed reconcile is swallowed.** `plugin/lib/backlog/migrate.py:1089-1090`: `_reconcile_status` calls `core.set_status`, which **returns** an error envelope on `TransportError` (`core.py:487-488` — `except TransportError as exc: return from_transport_error(exc)`) rather than raising. The code appends `warnings.append(f"status reconcile deferred for {canonical}: …")` and returns normally, so the record still counts as `created` and `_import_one_with_retry` (`migrate.py:909`) never sees a failure to retry — its `except TransportError` handler is the only thing that triggers the pause-and-retry, and no exception ever reaches it from this path.

  **Leg 2 — the gate cannot detect the result.** `verify_migration` (`migrate.py:1096`) compares the source set against **alias coverage only** (`:1163-1173`: `aliased.update(pfxs)` from `core.iter_alias_issues`, then `missing = [p for p in source if p not in aliased]`) and never inspects issue state. So under `--archive-scope all`, a rate-limited close stretch leaves items **open** on the target while the gate exits 0 and reports complete coverage. That is the F9 failure mode through a **third** door: the gate is green and the migration is not done. (Doors one and two: F9 itself, and BKL-72AS's unaliasable-id class.)

  **DESIGN DECISION THIS ITEM MUST RESOLVE FIRST** (why it is `stage: design`, not `ready`). `core.iter_alias_issues` (`core.py:915`) yields `(number, pfxs, label_names)` and does **not** yield issue state, so leg 2 needs one of:
  - **(a) widen that yield shape.** It is a shared API, so it needs a caller sweep before touching. That sweep is **done and recorded here so the decision rests on facts, not on estimate**: grep on the *function name* (not the pattern — this session hit that trap twice) returns **3 production call sites** — `migrate.py:1035` (the import's alias index), `migrate.py:1168` (this gate), `core.py:1000` (the alias-label restore) — plus **2 test sites** (`tests/test_backlog_pagination.py:188-206`). Small, but it is a shared contract change, not a local edit.
  - **(b) a second scan inside `verify_migration`,** which doubles the gate's API cost against a repo of ~900 issues.

  Pick deliberately; **do not default into (a) because it looks tidier.** One fact that materially informs the choice and should not be re-derived: `iter_alias_issues` already fetches with `state="all"` (`core.py:923`), so the issue state is **already in the payload it decodes and discards** — the question is API shape and blast radius, not whether more data must be fetched. That makes (b)'s "doubles the cost" framing a consequence of choosing a second scan, not an inherent floor; a third shape (a sibling generator, or an opt-in parameter) may dominate both. Also decide whether leg 1 should stop swallowing at all — if `_reconcile_status` re-raised (or returned a failure the caller respected), the retry path would cover the rate-limit case and leg 2 would be a backstop rather than the only detector.

  **Filed rather than fixed on 2026-07-29, deliberately.** Closing this is a **contract decision on the gate guarding ~900 irreversible writes**, and it deserved fresh context rather than being appended to a session already carrying the scrub work. The three sibling findings from the same Critic review were **fixed** rather than filed — this is the one that was not, and the disposition is the new FILE rule in `skills/critic/review-cycle.md` (deferred work with a named trigger), not a dumping-ground entry.

- **[BLD-ZQ2V]** `verify-chunk-refs` inspects the earliest incomplete chunk, not the one being worked on
  `effort: M · impact: M · area: build-plan · kind: bug · source: critic · added: 2026-07-28 · reviewed: 2026-07-29 · status: open · stage: design · related: BLD-5R7K, BLD-3M7K, BLD-9H2M, BLD-5V8F · refs: .prawduct/artifacts/buildplan-chunk-refs-discovery.md (requirements — problem, three-part fix, ordering constraint, success criteria), plugin/lib/buildplan_refs.py:1000 (`_current_chunk_id_from_status`), :1011 (its `_parse_build_plan_status` call), :257 (`_git_aware_progress` — the union rule, and the docstring that anticipates this exact "already-shipped Chunk 01" failure), :396-422 (`_resolve_chunk_progress_from` — where the two readings compose), .prawduct/artifacts/build-plan-v3.2.0-golive.md § Status (11 items, all `[ ]`)`

  Found during **v3.2.0 Chunk 05c** (Critic note, 2026-07-28).

  `_current_chunk_id_from_status` (`buildplan_refs.py:1000`) derives the "current" chunk from the build plan's Status section via `_parse_build_plan_status`. On `build-plan-v3.2.0-golive.md` — whose Status section deliberately leaves **every** chunk `[ ]` until release ("built 2026-07-28, `[ ]` until release" — the convention across Chunks 02 through 05c) — that answer is Chunk 01, and stays Chunk 01 for the whole release while later chunks are built. **Why** it stays there is the Root cause below; the checkbox convention is contributory, not causal.

  Verified empirically at the current tree:

      current chunk id: 01
      current_chunk: 'Chunk 01: Live verification (VRF-005/007/008) — the relayout merge already landed (PR #137)'

  **Consequence:** `verify-chunk-refs` and every consumer that agrees-by-construction on "which chunk is current" (the stop hook, Critic mode inference) inspect Chunk 01's refs and nothing else, for every chunk built during v3.2.0. The gate reports `ok` while giving zero coverage to the chunk actually being built. This is not hypothetical — it is how a bare `artifacts/migration-scrub-decisions.md` ref (should be `.prawduct/artifacts/...`) reached the Chunk 05c body unflagged; the Critic caught it by reading, not the gate.

  **Root cause — Chunk 01 is not a wrong answer to the question asked; it is the right answer to the wrong question.** Chunk 01 ("Live verification", VRF-005/007/008) is genuinely still incomplete — the plan's own Context line says *"Next is **Chunk 01** … then **Chunk 06**"* (`build-plan-v3.2.0-golive.md:85`). So "first incomplete chunk" correctly returns 01. The defect is that every consumer treats **"current chunk" as a proxy for "the chunk being worked on right now"**, and those two diverge the moment work proceeds **out of order** — which is exactly what happened here: 05, 05b and 05c were built while 01 stayed open. Under out-of-order execution, "earliest incomplete" and "in progress" are simply different questions, and the gate is asking the first while every consumer reads the answer as the second.

  **What that means for the fix.** It is *not* only "make the checkbox/commit union honest under `[ ]`-until-release." Even a perfectly honest first-incomplete reading returns 01 while 05c is being built, and `verify-chunk-refs` still inspects the wrong chunk. **A fix addressing only the union would leave the gate inspecting Chunk 01 throughout out-of-order work.** The gate needs the chunk **whose refs are in the working diff** — or the one named by the commits/edits at hand — not the earliest incomplete one.

  **Contributing cause — why the git-aware path could not rescue it** (corrected at filing time: the git reading is NOT degraded here). The original note attributed this to the checkbox reading being in force. Verified against the tree: `resolve_chunk_progress` returns `git_derived=True` (`ChunkProgress(total=11, complete=2, current_id='01', …)`), so the **git-aware** reading answered and still resolved Chunk 01. The cause is `_git_aware_progress`'s union rule (`:257`): an item is complete iff its box is `[x]` **or** its chunk id appears in a commit subject **since base**. For Chunk 01 both legs fail — its box never flips (the `[ ]`-until-release convention) and its completing work landed in a *prior* merge (PR #137, behind the base), so no commit since base names it. It is therefore permanently "the first incomplete item."

  **This makes the docstring's own guarantee void under this convention.** `_git_aware_progress`'s docstring anticipates precisely this failure — *"a commit-only reading resolves 'current' back to an already-shipped Chunk 01 — strictly worse than the checkbox fallback"* — and says the union prevents it. That protection assumes boxes flip when a chunk completes. Under `[ ]`-until-release they do not, so the checkbox leg contributes nothing and the union collapses to the commit-only reading it was built to avoid.

  **Distinct from BLD-5R7K — do not merge, and the reframing widens the gap rather than narrowing it.** BLD-5R7K is the `git_derived=False` degradation (undocumented commit-convention precondition unmet → falls back to checkboxes); it is a defect in *how well* the first-incomplete question is answered. This item says the **question itself is wrong** for consumers that need "in progress." So BLD-5R7K sits entirely inside the contributing cause here: even if it were fully fixed — precondition documented, degradation announced, git reading always trustworthy — out-of-order work would still route `verify-chunk-refs` to Chunk 01. They share a symptom ("always reports Chunk 01") and a fix surface (`buildplan_refs.py`) and should land coherently; neither fix closes the other.

  The contributing interaction is between two reasonable designs: "current = first incomplete" is right for plans whose checkbox means *built*, and "`[ ]` until release" is right for a plan whose checkbox means *released*. Neither is wrong alone; together they remove the union's ability to compensate. Fix directions, in root-cause order: (1) give the gate a **working-diff-derived** current chunk — the chunk whose refs/files the uncommitted edits and at-hand commits touch — and fall back to first-incomplete only when that is empty; (2) as a cheaper partial, derive git-wise against the *release* base rather than the branch base when the checkbox convention is release-gated; (3) as a detector rather than a fix, warn when the current chunk hasn't moved across N commits. Only (1) addresses out-of-order execution.

  **Fixing this gate unmasks a second, currently-invisible defect — record the coupling so whoever fixes it isn't ambushed.** `_verify_chunk_refs` resolves refs relative to the repo root only (`target = project_dir / ref`); it has **no plugin-root fallback**. Build plans in this repo routinely write plugin-relative shorthand — `lib/gates.py`, `lib/backlog/context.py`, `lib/backlog_probes.py`, `lib/gitstate.py`, `lib/upstream_probes.py`, `skills/backlog/SKILL.md`, `skills/backlog/migration-scrub.md`, `.claude-plugin/plugin.json` — all of which exist under `plugin/` and resolve to nothing from the root. They are invisible today **only** because the gate inspects Chunk 01 and nothing else. The moment current-chunk resolution is fixed, they surface as **blocking** across Chunks 03, 05c, 07 and 08.

  **So the fix is two parts, not one:** correct current-chunk resolution **and** give `_verify_chunk_refs` a plugin-root fallback (or normalize the shorthand). Landing only the first turns a silent gate into a blocking one on refs that were never wrong in spirit.

  *Precision on the evidence:* the **class** is confirmed — each of the eight tokens above was checked at this tree and resolves under `plugin/` but not from the root, and `_verify_chunk_refs` was read and confirmed to have no fallback. The **count** is not independently established: the Critic's "~8 literal tokens" figure came from a replay of `_looks_like_file_path` that was coarser than the real gate (it scanned the whole plan rather than one chunk's extracted refs, and did not strip `file.py:LINE` suffixes). Treat the direction and the coupling as confirmed, the exact count as the Critic's figure.

  A second, independent contributor to the same blind spot on this branch was a stale PATH `prawduct-hook` resolving to the main worktree's 2026-07-21 build — a known hazard already covered by an existing learning, deliberately **not** filed separately. (critic — v3.2.0 Chunk 05c)

  **Update 2026-07-29 — requirements written (`.prawduct/artifacts/buildplan-chunk-refs-discovery.md`), and three things above are now measured rather than estimated.** All measured against the real gate functions (`_extract_chunk_refs` / `_looks_like_file_path`, not a replay) at tree `02faf2f`, residual 0. They correct, in order:

  1. **The fix is THREE parts, not two** — supersedes "So the fix is two parts, not one" above. A third defect class was found: `_looks_like_file_path` classifies **GitHub repo slugs and issue refs** (`owner/repo`, `owner/repo#N`) as file paths — **4 occurrences**. These are *not* reachable by the plugin-root fallback (they are not files anywhere), so part 2 does not cover them; they need their own exclusion.
  2. **The plugin-relative count is 17 across seven chunks (02, 03, 05, 05c, 07, 08, 09)** — not the Critic's estimated ~8 across four. The *Precision on the evidence* paragraph above already flagged that count as unverified and the direction/coupling as confirmed; both hold, and the count is now measured — the unmasking is more than twice as wide as recorded.
  3. **Chunk 01 carries exactly 1 ref.** So for the entire v3.2.0 release the gate has been verifying **one** file reference — quantifying the "zero coverage" consequence above.

- **[BKL-72AS]** Multi-hyphen hand-minted ids are unaliasable — the importer's id-shape regexes stop at the first hyphen
  `effort: S · impact: L · area: backlog-service · kind: bug · source: builder · added: 2026-07-28 · reviewed: 2026-07-28 · status: open · stage: ready · related: BKL-6M4T, BKL-4W7H, BKL-6X5D, BKL-4Z7M · refs: plugin/lib/backlog/legacy.py:46 (`ID_RE`), plugin/lib/backlog/ids.py:178 (`_PFX_RE`, backs `is_pfx`), plugin/lib/backlog/migrate.py:67 (`_ID_MARKER_RE`), plugin/skills/backlog/migration-scrub.md:120 (the step-1b grep; prose at :100-112), plugin/lib/backlog/migrate.py:1163 (`verify_migration`'s `unaliasable` derivation — source-only, never consults the target repo), plugin/lib/backlog/core.py:933 (the `is_pfx` filter over `block.id_aliases()` — real, but it governs alias read-back, not the gate), .prawduct/artifacts/migration-scrub-decisions.md (decisions 4 and 5), .prawduct/artifacts/build-plan-v3.2.0-golive.md (Chunk 05c, ahead of Chunk 06), 7565787 (the completeness gate that made unaliasable an exit-4 conflict)`

  **Defect, not a new requirement.** Parent requirement is **MIG-2** (multi-prefix absorption): the importer is specified to absorb whatever ID-shaped token a source used across its hand-minted prefixes. Ids carrying 2+ hyphens are in that set and are silently not absorbed.

  Four copies of the same id-shape pattern all encode `PREFIX-SUFFIX` with **exactly one** hyphen:
  - `plugin/lib/backlog/legacy.py:46` — `ID_RE` (parses `[PFX]` out of the bullet)
  - `plugin/lib/backlog/ids.py:178` — `_PFX_RE` (backs `is_pfx`, which gates alias minting)
  - `plugin/lib/backlog/migrate.py:67` — `_ID_MARKER_RE` (strips the `[PFX]` marker from the imported title)
  - `plugin/skills/backlog/migration-scrub.md:120` — the step-1b grep, plus its prose at lines 100-112

  An id like `MIG-M4-REMOVE` matches **none** of them: `[A-Za-z][A-Za-z0-9]*` cannot consume the second hyphen, so the item parses as markerless, `is_pfx` returns False, no `id:` alias is minted, and commit `7565787`'s completeness gate reports it **unaliasable (exit 4)**.

  **Impact — not hypothetical, and it now blocks a shipped gate.**
  - A survey of 14 local prawduct repos found **4 such items across 3 distinct products** (~21% of products, one item each): prawduct `[MIG-M4-REMOVE]` (Archive), hallucinote `[AUD-TIMBRE-CALIB]`, scriob + scriob2 `[ENG-9V2K-F]`. A recurring class, not a one-off.
  - `artifacts/migration-scrub-decisions.md` **decision 4** (2026-07-18) records: import `MIG-M4-REMOVE` as-is, content-digest idempotency key, no alias, accepted for shipped history. **Decision 5** sets `--archive-scope all`, so it *does* get imported. Commit `7565787` then made unaliasable an exit-4 conflict. **Chunk 06 cannot satisfy both the recorded decision and the shipped gate.**
  - The documented escape hatch **cannot** work — and not just for this class. `verify_migration` derives `unaliasable` at `plugin/lib/backlog/migrate.py:1163` as `[r.title for r in records if not r.pfx]`: purely from parsing the source markdown, never consulting the target repo. So no label, block edit, or other issue-side action can clear that list for **any id of any shape**. The runbook's hand-label remedy (`migration-scrub.md:~308`) contradicted the code's own remedy string at `migrate.py:1212-1219` (*"give each a real PFX in the source BEFORE importing"*). That runbook defect is fixed as part of the same chunk.

  **Fix:** widen all four copies to `[A-Za-z][A-Za-z0-9]*(-[A-Za-z0-9]+)+`. The item keeps its identity, decision 4 stands as written, the gate passes, and no rename/docstring sweep is needed (`MIG-M4-REMOVE` is referenced in ~12 places including two shipped docstrings at `plugin/lib/core.py:4` and `plugin/lib/__init__.py:13`).

  **Rejected alternatives** (each unwinds something already decided): rename in source contradicts decision 4; `--archive-scope open` contradicts decision 5 and drops 121 archive items; hand-label after import cannot work at all — `verify_migration` derives `unaliasable` from the source parse and never reads the target repo, so no issue-side action clears it.

  **Known cost:** the marker slot gets slightly greedier for bracketed non-ids carrying 2+ hyphens (e.g. `[pass-through-cache]`). Single-hyphen bracketed non-ids are already absorbed today, so this **widens an existing behavior** rather than introducing a new one. Digit-led brackets such as `[2026-07-28]` stay rejected — the leading `[A-Za-z]` is unchanged.

  Scheduled as **Chunk 05c** of `build-plan-v3.2.0-golive.md`, ahead of Chunk 06 (which it unblocks).

  **[BUILT 2026-07-28 — v3.2.0 Chunk 05c, on `feature/v3.2.0-c02-adapter-safety`.]** All four id-shape copies widened; the runbook's contradicted hand-label remedy fixed in the same chunk. Suite **2768 passed / 7 skipped**. Verified live against this repo's own `backlog.md`: **333 items parse, `MIG-M4-REMOVE` absorbed, zero unaliasable remaining** — so Chunk 06's completeness gate (`7565787`) now clears without unwinding decision 4 or decision 5. `status=shipped` flip **deferred to release (Chunk 09)** per this release's `[ ]`-until-release convention, so the item stays `open`.

- **[GOV-2K7R]** The `gh` grant narrowing shipped without a regression pin — restoring `Bash(gh *)` on the `pr` skill keeps the suite green, while the backlog-grant half of the same fix did get a test
  `effort: S · impact: M · area: skills · kind: bug · source: critic · added: 2026-07-28 · reviewed: 2026-07-28 · status: open · stage: ready · related: BKL-4M6T, GOV-5J9Q, BKL-5N9W, CRT-9V4T, GOV-6J3P · refs: plugin/skills/pr/SKILL.md:6 (the narrowed `Bash(gh pr *)` grant — unpinned), tests/test_backlog_skill_metadata.py (the sibling pin, and the test family to extend), tests/test_pr_reviewer.py:620 (the only existing assertion over this same `allowed-tools` line — presence of `ledger-append`, not narrowness of `gh`), 85753fd (the interpreter-fence fix that narrowed it)`

  **Filed from the 2026-07-28 cumulative Critic review (`rev-20260728T215801Z-c74ba442`, warning) — deliberately filed rather than fixed, under the termination rule this same bundle shipped (CRT-8N5V): once zero blocking remain, file the rest.**

  `85753fd` narrowed the `pr` skill's `Bash(gh *)` to `Bash(gh pr *)` as half of the interpreter-fence fix. **Nothing asserts it.** Verified against the tree: `grep -rn "gh pr \*\|Bash(gh" tests/` returns **zero** hits, so restoring the wildcard leaves the suite fully green. The single existing assertion over that exact frontmatter line (`tests/test_pr_reviewer.py:620`) checks that `ledger-append` is **present** — a positive-presence check that a widened `gh` grant passes unchanged.

  **The asymmetry is the argument.** The sibling half of the same fix — the backlog grant — *did* get a metadata test (`tests/test_backlog_skill_metadata.py`, the CRT-9V4T pin). So one half of a security-adjacent narrowing is pinned and the other is not, which is the same shape as **BKL-4M6T** (BKL-2Q7F shipped unpinned while both Chunk 03 siblings got tests). A grant is a security boundary whose only enforcement is the text of one line; an unpinned narrowing is a boundary that any future edit silently reopens.

  Fix-shape: extend the existing skill-metadata test family to assert the `pr` skill's grant is `gh pr`-scoped and that no bare `Bash(gh *)` appears in any skill frontmatter. Cheap, and it closes the class rather than the instance (per CRT-6R3W — pin the *rule*, not the site). Governance-protected (`skills/`) → full Critic + PR review. (critic — `rev-20260728T215801Z-c74ba442`)

- **[BKL-6T3P]** `adapter-mode.md` bounds the model's mutation surface by "the ops in the usage table" — no usage table exists anywhere in the plugin, so the bound has no referent
  `effort: S · impact: M · area: skills · kind: bug · source: critic · added: 2026-07-28 · reviewed: 2026-07-28 · status: open · stage: ready · related: BKL-8V3D, BKL-5N9W, BKL-2Q7F, BKL-4M6T, BKL-9XQ2 · refs: plugin/skills/backlog/adapter-mode.md:96 (the unresolvable bound — "you never invent a mutation path — the adapter exposes exactly the ops in the usage table"), `grep -rn "usage table" plugin/` → 1 hit, the reference itself (verified 2026-07-28), plugin/lib/backlog/cli.py:33 (the real op set, which no skill surface tabulates), tests/test_backlog_instruction_surface.py (the instruction-surface pin family BKL-8V3D landed in)`

  **Filed from the 2026-07-28 cumulative Critic review (`rev-20260728T215801Z-c74ba442`, warning) — deliberately filed rather than fixed (CRT-8N5V termination rule).**

  The sentence is the *only* thing standing between the model and an invented mutation path: *"you never invent a mutation path — the adapter exposes exactly the ops in the usage table."* It bounds by reference to a table that **does not exist**. Verified: a tree-wide `grep -rn "usage table" plugin/` returns exactly one hit — this sentence citing it. **A bound that cannot be resolved is not a bound**, and the failure is silent in the worst direction: a model that cannot find the table does not stop, it falls back to its own notion of the op set.

  **Deliberately NOT folded into BKL-8V3D, though it is the same file and the adjacent clause — read this before merging them.** BKL-8V3D is the `--apply`/dry-run false claim at the same `:96` region, and that claim **appears already corrected in this branch**: the line now reads *"No generic preview-or-apply flag sits over those mutations,"* which is the true statement BKL-8V3D asked for (corroborated by GOV-7W3D, which quotes the corrected wording). BKL-8V3D nonetheless still reads `status: open`. Folding a live defect into an item that is plausibly closable would make BKL-8V3D un-archivable for a reason unrelated to why it was filed. Keep them separate: **BKL-8V3D closes on its own merits; this item carries the residual.** (See the note in this filing's report — whether BKL-8V3D is in fact shipped is an explicit call for the owner, never inferred from the tree.)

  Fix-shape: replace the dangling reference with the bound itself — name the permitted ops inline (or add the table the sentence promises), and pin it in `tests/test_backlog_instruction_surface.py` so the referent cannot silently vanish again. Governance-protected (`skills/`) → full Critic + PR review. (critic — `rev-20260728T215801Z-c74ba442`)

- **[BKL-8H2W]** The durable CLI confirmation gate is named as debt in prose only — its sole pointer is BKL-5N9W, which archives at the Chunk 09 flip, so at release the debt becomes unreferenced
  `effort: M · impact: M · area: backlog-service · kind: debt · source: critic · added: 2026-07-28 · reviewed: 2026-07-28 · status: open · stage: design · related: BKL-5N9W, BKL-9XQ2, BKL-2Q7F, BKL-8V3D, BKL-6T3P, GOV-4B9N · refs: .prawduct/change-log.md (the 85753fd entry naming the debt — "the durable fix is a confirmation gate in the CLI, which binds regardless of caller; it is not built here and is named debt, not a silent omission"), 85753fd (the skill-level closure this backstops), plugin/lib/backlog/cli.py (the durable binding site — no confirmation gate today), .prawduct/artifacts/security-model.md § Direction (the amended operation-level approval norm this gate would implement — see GOV-4B9N), .prawduct/artifacts/build-plan-v3.2.0-golive.md (Chunk 09 — where BKL-5N9W's flip drops the only current pointer)`

  **Filed from the 2026-07-28 cumulative Critic review (`rev-20260728T215801Z-c74ba442`, warning) — deliberately filed rather than fixed (CRT-8N5V termination rule).**

  **The defect is the reference, not the debt.** `85753fd` closed the model-initiated destructive path **at the skill**, and its change-log entry says plainly that the durable fix is a **confirmation gate in the CLI, which binds regardless of caller** — correctly labelling it debt rather than omitting it silently. That is good practice. But **no backlog item holds it.** Its only pointer is **BKL-5N9W**, and BKL-5N9W archives at the **Chunk 09 flip** — so the release that ships the narrowing is the same release that makes the debt unreferenced. Prose in a change log is a record, not a tracker: once its citing item archives, nothing surfaces the gap to `pick`, to triage, or to any future reader.

  **Why the skill-level closure is not sufficient on its own.** A grant narrowing bounds *who may call*; it does not bound *what the call does*. The identical distinction is already recorded under BKL-9XQ2 — *"a tools-allowlist narrowing is not a substitute for the adapter guard — permissions bound who may call, not where the call may send"* — and the same argument applies here: the CLI is reachable by any caller that is not the skill (an operator, a runbook, a future surface), and none of them inherit the skill's fence. This gate is also the concrete mechanism that would satisfy the **amended** operation-level approval norm (GOV-4B9N) at the data plane rather than in prose, which is the framework's standing MG4/G1 split: the model is in the decision, never in the data plane.

  **`stage: design` deliberately.** What the gate confirms (blast radius, irreversibility, op scope), how it behaves non-interactively (CI, unattended), and how it relates to the existing `restructure-preview` are open design questions — this is not pick-and-build.

  **Second "named as debt with nothing holding it" instance surfaced on 2026-07-28** (see the item filed for the same review round). Two in one day is a pattern worth naming: a change-log entry that says *"this is named debt, not a silent omission"* is only true if something durable holds it — otherwise the sentence documents the omission rather than preventing it. Candidate for `learnings.md`, and a data point for **GOV-6D4Q**. (critic — `rev-20260728T215801Z-c74ba442`)

- **[BKL-5Q9C]** F8 — `pick` can return an item closed seconds earlier: the close→list path has no settle-retry, though the create→get path already has one for the same replication window
  `effort: M · impact: M · area: backlog-service · kind: bug · source: critic · added: 2026-07-28 · reviewed: 2026-07-28 · status: open · stage: ready · related: BKL-4H8P, BKL-3N8Q, BKL-6M4T, BKL-9J3F · refs: .prawduct/operator-verification.md VRF-010 (where it was observed live), .prawduct/artifacts/functional-audit-v3.2.0.md F8 (the audit entry), plugin/lib/backlog/core.py:296 (the existing bounded post-create settle retry — the precedent mechanism), plugin/lib/backlog/query.py (the `pick` list path — no settle equivalent), plugin/lib/backlog/transport.py (classification; no retry loop for single ops)`

  **Filed from the 2026-07-28 cumulative Critic review (`rev-20260728T215801Z-c74ba442`, warning). Found live in VRF-010 and recorded in two artifacts with an explicit "file it" — never filed. The review caught the miss, which is itself the finding worth keeping: "file it" written into an artifact is not a filing.**

  **What was observed.** After `status --to shipped`, a subsequent `pick` returned the **just-closed item**. Re-running seconds later was correct.

  **Correctly diagnosed as a replication window, not a filter bug** — this distinction is the whole item, and a fix aimed at the filter would be wrong. At the moment of the stale read, GitHub was **already correct**: the issue carried `state: closed` with `closed_at` set, and a direct `issues?state=open&labels=stage:ready` query **already excluded it**. So the query is right and the data is right; the **list endpoint** served a stale page.

  **The workflow shape is what makes it more than a curiosity.** An agent closes an item and *immediately* picks its next work — that is the single most common transition in the tool's intended loop, and it lands precisely inside the window. The failure is quiet and plausible: the agent is handed back the thing it just finished, with nothing marking the result as stale.

  **The fix likely already exists in the codebase.** `file` carries a **bounded settle-retry** for the documented 404-after-create window (`core.py:296`) — the same class of eventual-consistency gap, already solved once. **close→list has no equivalent.** Fix-shape: apply the same bounded mechanism to the close→list path (re-query when a returned candidate's local state says closed), reusing `core.py:296`'s budget discipline rather than inventing a second retry policy — and note **BKL-4H8P**'s warning that a retry contract without a stated max/deadline is what produced an unbounded loop before, so the budget must be explicit at the contract, not just in the code. (critic — `rev-20260728T215801Z-c74ba442`)

- **[DOC-7J4M]** Eight stale `migration-scrub.md` step references across docs and artifacts — and the durable lesson is that renumbering them would have been wrong too
  `effort: S · impact: S · area: docs · kind: bug · source: critic · added: 2026-07-28 · reviewed: 2026-07-28 · status: open · stage: ready · related: BKL-2Q7F, BKL-4C9P, BKL-4M6T, CRT-6R3W, CRT-4X2N · refs: plugin/skills/backlog/migration-scrub.md (renumbered 0–5 → 0–6 this cycle; has NO lettered sub-steps), plugin/lib/backlog/migrate.py (the one in shipped code — already fixed, and fixed by re-anchoring to a NAME), documentation/backlog-service-prd.md:218 and documentation/backlog-service-requirements.md:405 ("step 2c"), .prawduct/change-log.md:228 / :799 / :874 / :1429, .prawduct/artifacts/migration-scrub-decisions.md:103 ("steps 0–4"), .prawduct/artifacts/build-plan-v3.2.0-golive.md:335, and the step-anchored refs inside BKL-2Q7F and BKL-4C9P themselves, .prawduct/learnings.md (the name-anchoring preference this repo already documents)`

  **Filed from the 2026-07-28 cumulative Critic review (`rev-20260728T215801Z-c74ba442`, warning) — deliberately filed rather than fixed (CRT-8N5V termination rule).**

  The runbook was renumbered **0–5 → 0–6** this cycle. The single reference in **shipped code** (`migrate.py`) is fixed; the rest sit in records and are stale.

  **The durable lesson, which is why this item is worth more than its effort suggests: "renumber them to match" would ALSO have been wrong.** Several stale references point at **`2c`** — and `migration-scrub.md` has **no lettered sub-steps at all**. So the obvious fix (bump the numbers) would have produced a *differently* wrong pointer that looks right, and the class would regenerate at the next renumber. The fix that actually landed in `migrate.py` **re-anchors by name**, which is this repo's documented preference and the reason the class exists: *a step number is a claim about a document the citing work exists to change, so it is stale the moment the work starts.* **BKL-2Q7F** already learned this the hard way and re-anchored its own refs from line numbers to step+op — and note the recursion, that BKL-2Q7F's step-anchored refs are themselves now among the stale sites.

  **Fix-shape: propose name-anchoring all eight, not renumbering them.** And per this branch's headline lesson (CRT-6R3W; the sweep-that-stops-short recurrence): the done-when must state the **query that returns empty**, not a count of sites fixed. A count is unfalsifiable — it is true of any prefix of the real set. Concretely, the completion claim is a command like `git ls-files | xargs grep -n "migration-scrub.*[Ss]tep [0-9]"` returning no numeric-anchored citation, **not** "I fixed eight." The eight enumerated in `refs:` are the sites known at filing, offered as a starting set and explicitly not as the closure condition. (critic — `rev-20260728T215801Z-c74ba442`)

- **[GOV-3D6X]** VRF-010's closure of F1 did not propagate — `project-state.yaml`, the Chunk 09 gate list, and BKL-3N8Q all still read as though the readers are unverified
  `effort: S · impact: S · area: governance · kind: task · source: critic · added: 2026-07-28 · reviewed: 2026-07-28 · status: open · stage: ready · related: BKL-3N8Q, GOV-7W3D, BKL-6M4T, BKL-9J3F · refs: .prawduct/operator-verification.md VRF-010 (the live verification of all three relationship/timeline readers, discharging MIG-3), .prawduct/artifacts/functional-audit-v3.2.0.md F1, .prawduct/project-state.yaml (`design_decisions.infrastructure_dependencies.integration_test_strategy` — records BKL-3N8Q by id; this is the "stable id, do not renumber" citation), .prawduct/artifacts/build-plan-v3.2.0-golive.md (Chunk 09 — the flip list this closure is owed to), BKL-3N8Q (still `status: open`, now fully dischargeable)`

  **Filed from the 2026-07-28 cumulative Critic review (`rev-20260728T215801Z-c74ba442`, warning) — deliberately filed rather than fixed (CRT-8N5V termination rule).**

  **VRF-010 verified all three relationship/timeline readers live and discharged MIG-3.** That closure never reached three surfaces that still encode the pre-verification state: `project-state.yaml`, the **Chunk 09 gate list**, and **BKL-3N8Q**.

  **BKL-3N8Q is now fully dischargeable and still reads `open`.** Both its halves are closed: its `pick`-path half shipped with **Chunk 05b**, and its foreign-API half is exactly what VRF-010 verified. It is the item whose "no open blockers" readiness string was the vacuous verdict — so leaving it open misreports the one defect the verification was run to settle.

  **This item is about the flip LIST being complete, not about flipping now.** The release convention holds: the status flips are **owed at Chunk 09**, and nothing here should be marked `shipped` ahead of that. What is missing is that Chunk 09's list does not currently *contain* these flips, so following the plan literally at release would leave them behind — the same deferral-cascade failure this branch has already hit once (a chunk's membership changing while the sections that compute against it are not opened). Note also that `project-state.yaml` cites BKL-3N8Q **by id** under an explicit "stable id — do not renumber" instruction, so the propagation must update that citation's surrounding claim rather than the id.
...
- **[CRT-8N5V]** The Critic review loop has no structural exit condition — agents in production get trapped for 3–4 rounds reviewing the records of the previous round
  ...
  **Owner escalation, 2026-07-28, verbatim:** *"this 'fixing records about records' has been prawduct's downfall in production. agents get trapped for 3-4 rounds before giving up."* Declared blocking for other work.

  **The mechanism is not that warnings are too strict.** WARNING and NOTE gate nothing — `review-cycle.md` states it directly ("WARNING and NOTE are advisory at the PR gate"), and both the PR gate and the Stop gate require only *coverage* plus *zero unresolved BLOCKING*. The loop ran because `methodology/building.md` said warnings "should be addressed," which reads as must-fix. From there the cycle is self-sustaining: the agent fixes one → the fix is a commit → the commit outruns coverage → a new pass runs → **that pass reviews the records the previous round just wrote**. Prose has no passing condition the way a test does, so every round finds something true about the previous round's narration. (This is the same "re-review treadmill" CRT-4J8W named and fixed at the *gate-chaining* level; what was missing is a termination rule for the **builder**.)

  **Measured live this session:** four review rounds, ~40 minutes wall-clock, on a ~40-line code change (Chunk 05b). Every finding was correct and correctly severity-rated — this is not a false-positive problem. Zero blocking after round 1. `check-cumulative-critic` alone takes 2m17s per invocation.

  **Round 4 was required by nothing, and that is the production trap.** `coverage_algebra.is_judgeable_path` already returns False for `change-log.md`, `learnings.md`, `.prawduct/artifacts/**` and product docs — verified by direct call. The last fix commit touched only the change log, so the gate would have passed. The round ran because the agent inferred "coverage must reach HEAD" from gate output printed *before* those commits, and never re-ran the one command that would have said otherwise. **A stale gate line, carried forward as a belief, is what traps the agent** — an agent-behavior defect, not a gate defect. Any fix that only hardens the gate will miss it.

  **Rounds 2–3 were dragged in by a comment-only `.py` edit** — a docstring in `tests/spikes/s2_migration.py`, judgeable by extension. That is **COV-3M8Q**, already open; it had been filed but its cost had never been measured. Measured here: ~13 minutes.

  **Shipped at `67fe565`** (the instruction half; the owner scoped this as "instructions first, code after"):
  - `methodology/building.md` — once zero blocking remain, **file the rest rather than fixing them**; re-run the gate rather than infer another round from earlier output.
  - `skills/critic/review-protocol.md` — WARNING means true **and** worth the builder's time; name the consequence or rate NOTE. Record-only prose defaults to NOTE unless it ships as a false claim or misleads someone into a wrong action.
  - `skills/critic/review-cycle.md` — the full rule under "The review loop terminates": exit condition, re-run-the-gate instruction, the `is_judgeable_path` trap, both severity bars with examples, diminishing-returns signal.
  - Token budgets raised by **owner ruling** (`review-protocol.md` 3530→3620, `building.md` 4600→4660), a deliberate departure from MET-3Q8V's "stay green without raising budgets" and from the test comments' own trim-or-relocate rule. Rationale recorded at both assertions in `tests/test_v5_methodology.py`.
...
- **[GOV-6D4Q]** A deletion-only simplification pass over the governance surface — the 2026-07-02 review's diagnosis still holds, but its fix program was outrun 3.4x, and one change-log sentence has been standing in for the review ever since
  `effort: L · impact: L · area: governance · kind: improvement · source: user · added: 2026-07-28 · reviewed: 2026-07-28 · status: open · stage: ready · related: CRT-5M9J, ENV-2W7K, LRN-7M4D, MET-8J5R, DOC-3V7T, STN-4W7R, BLD-8N4W, MET-2X6F, MET-9W2P, STH-5D8J, REL-4Q9V, SCN-7K4B, MET-7R4J · refs: .prawduct/artifacts/framework-efficiency-review-2026-07-02.md (the parent requirement doc — § Verdict, § Overbuilt, § Underspecified, § Structural diagnosis, § The fix program waves 1–3, § For the future agent picking this up), .prawduct/change-log.md:14 (the unbacked "a simplification review is open against exactly that" claim this item now backs), .prawduct/artifacts/build-plan-v3.2.0-golive.md § Status (Chunk 07/08 deferrals + Chunk 09 re-scope, both 2026-07-28), plugin/methodology/ (6 .md), plugin/skills/ (20 .md), plugin/docs/ (8 .md), plugin/templates/ (28 .md), CLAUDE.md — the surface under review`

  **Why this exists — work was deferred against a review that did not exist.** `.prawduct/change-log.md:14` defers four fixes on the reasoning that fixing them "*adds* governed surface and a simplification review is open against exactly that." A repo-wide grep for `simplification review` returns **that one line and nothing else** — no backlog item, no artifact, no plan (verified 2026-07-28: `grep -rn "simplification review" . --include="*.md" --include="*.py" --include="*.yaml"` → 1 hit). A change-log sentence has been carrying the weight of a review that was never filed. This item *is* that review, made real.

  **The four deferrals that cited it must be re-checked once this pass runs** — the norm dispositions, the Chunk 05 verify-api step, the runbook's chunk-number coupling, and ONB-3F9P's status pointer are all currently deferred on a premise that was false when written. Whatever this pass decides, those four need an explicit disposition rather than continued inheritance of a bad reason.

  **The measured problem (26 days, 2026-07-02 → 2026-07-28).**
  - **Open backlog items: 50 → 169** — 3.38x. This is the headline number and it is exact.
  - **Enforcement Python: ~15,500 → ~27,400 LOC** — `plugin/lib` 22,730 + `plugin/bin/prawduct-hook` 4,129 + `plugin/hooks` 525. (The owner brief said ~26,900; the small delta is file-set choice, not a different claim.)
  - **Governance prose: ~95,700 words** across `plugin/methodology` (6 files), `plugin/skills` (20), `plugin/docs` (8), `plugin/templates` (28), plus CLAUDE.md.

  **Attribution of the growth (corrected 2026-07-28 — the first draft did not add up).** The open count grew by **119 net** (50 → 169). The 2026-07-02 fix program contributed **17** of those (12 still open + 5 shipped), leaving **~102 net new from elsewhere**, which sampling of the 2026-07-26/27 filings suggests are predominantly `source: critic` findings against governance machinery. Treat the 119 and the 17 as exact and the "~102 predominantly Critic" split as a characterization from sampling, not a full census — if this pass needs the attribution to bear weight, count it properly first. (The original wording claimed 17 *and* ~120 out of 119, which is impossible; it is corrected rather than deleted because the error is instructive — a number that felt right was propagated through a brief and a filing without either party adding it up.)

  That is the loop to break, stated plainly: *governance work generates findings, findings become items, items become governance work.* The review that was supposed to shrink the surface is one of the things that grew it.

  **Fix-shape — the constraint IS the design (owner decision, 2026-07-28).** This pass **may not file new backlog items.** Its only permitted output is a **delete/keep list against existing surface**. A conventional audit is *explicitly rejected*: the 2026-07-02 review was a seven-agent sweep whose diagnosis was correct and owner-accepted, and the surface grew 3.4x anyway. Another sweep is predicted to file another twenty items and make the number worse. **The diagnosis is already in hand; what is missing is a deletion decision.** Treat "we should audit this properly first" as the failure mode this item exists to refuse.

  **Do not re-derive the diagnosis.** Read `.prawduct/artifacts/framework-efficiency-review-2026-07-02.md` **in full** first — it carries evidence these one-liners cannot. Its Wave 2 and Wave 3 items are **12 still-open, already-scoped deletion candidates**, verified open 2026-07-28: CRT-5M9J, ENV-2W7K, LRN-7M4D, MET-8J5R, DOC-3V7T, STN-4W7R, BLD-8N4W, MET-2X6F, MET-9W2P, STH-5D8J, plus REL-4Q9V and SCN-7K4B (which carry the tag without a wave label). Wave 1 shipped 5 (MET-3Q8V prose-diet, VWS-6R4T, GOV-7T2M, CRT-3F6W, STH-8R3Q).

  **Waves 2 and 3 have not been picked once in 26 days.** That non-pickup is not an accident of scheduling — it is evidence, and it belongs in the pass. Twelve scoped, ready items that nobody reaches for are themselves a signal about what the surface is worth.

  **Sequencing (owner decision, same session).** Runs **after** v3.2.0's **Chunk 06** (the irreversible backlog migration) and **before Chunks 07 and 08**, which are now deferred out of the release precisely because they add governed surface. Both deferrals and the Chunk 09 re-scope are recorded in `.prawduct/artifacts/build-plan-v3.2.0-golive.md` § Status. Chunk 06 is the one remaining release chunk that **reduces** load rather than adding to it — it gets 169 items out of a ~5,000-line markdown file that an LLM rewrites on every mutation.

  **This item is itself an instance of what it forbids.** Filing it moved the open count 169 → **170**: a pass whose one rule is "file no new items" arrived as a new item. The body's measurements are left reading 169 because that was true at the moment of the decision — the discrepancy is deliberate, not drift, and a re-measure finding 170 has found this sentence, not an error. The pass should count itself among the deletion candidates: if it ships, it archives.

  **Known tension — record, do not resolve here.** The backlog service this release ships is itself **the largest new governed surface prawduct has added** (four go-live blockers, its own runbook, its own advisory, its own label taxonomy). Migrating **relocates** some tax as much as it reduces it. The owner made the call knowing this. The pass should treat the service as **in-scope for deletion review like anything else** — not exempt because it is new. (user)

- **[CRT-4M2P]** Nothing records WHICH of the three triggers consolidated a review — so a dead consolidation trigger is indistinguishable from a working one, which is why CRT-2J8N hid for 17 days and why VRF-002 cannot return a meaningful verdict
  `effort: S · impact: L · area: critic · kind: bug · source: critic · added: 2026-07-27 · reviewed: 2026-07-27 · status: open · stage: design · related: CRT-2J8N, GOV-2W7Q, CRT-6B9M, CRT-3F7M, TEL-2B6K · refs: plugin/lib/critic_consolidate.py (`consolidate` — the single entry all three triggers share, and the `review.critic` ledger argv built ~:1073-1080 carrying only `--scope`/`--chunk`/`--model`), plugin/bin/prawduct-hook (`cmd_critic_consolidate` — the explicit CLI path; `cmd_subagent_stop` — the hook path; `cmd_stop`'s abandoned-review self-heal — the backstop path), plugin/lib/ledger.py (`ledger_append` — the closed `--event/--scope/--chunk/--model/--findings` arg set a `--trigger` field must extend), plugin/skills/critic/SKILL.md:56-57 (which path each roster uses), .prawduct/operator-verification.md VRF-002`

  **The three call sites, verified at the source — all three reach the same function with the same signature and none of them says who it is.**
  1. **The `SubagentStop` hook** — `cmd_subagent_stop` → `critic_consolidate.consolidate(project_dir)`.
  2. **The explicit CLI call** — `cmd_critic_consolidate` → `critic_consolidate.consolidate(project_dir)`. This is what `/prawduct:critic` runs itself for a single-pass roster (`SKILL.md:56`) and what CLAUDE.md tells the main agent to run before reading `.critic-findings.json`.
  3. **The session-end backstop** — `cmd_stop`'s abandoned-review self-heal → `_critic_consolidate().consolidate(project_dir)`.

  The `review.critic` ledger event is built as `["--event", "review.critic"]` plus optional `--scope`, `--chunk`, `--model`. **There is no trigger field, and `consolidate()` takes no parameter that could carry one.** The three paths are therefore observationally identical: same evidence fact, same `.critic-findings.json`, same cleared marker, same ledger line.

  **Consequence 1 — it is why CRT-2J8N was invisible for seventeen days.** A dead `SubagentStop` hook and a working one produce the *same observable end state*, because the other two paths silently cover for it. The system had no way to report "the trigger you think is running has never run." This is the missing observability that let the first defect hide, which is why this item outranks a normal telemetry improvement.

  **Consequence 2 — it bit again the same day, in the review OF the fix.** The reviewing agent's summary asserted that the `SubagentStop` trigger *"did consolidate here without an explicit call"* while that same review's warning 7 states that nothing records which trigger fired. **Those cannot both be true**, and the first is unfalsifiable as the system stands. Independently verified and it is worse than unfalsifiable — it is almost certainly false: the plugin that actually fires hooks is the installed one (`~/source/prawduct`, `main@f802971`), and `git show f802971:plugin/hooks/hooks.json` still carries the bare `critic-reviewer` matcher. **The fix was not live in the session that claimed to observe it working.** Treat the claim as unsubstantiated. Worth keeping as a specimen: the absence of this field does not merely fail to answer the question, it invites a confident wrong answer, from a reviewer that had *just written down* why it could not know.

  **Consequence 3 — the real cost: VRF-002 is unverifiable as written.** Its verification step is *"confirm `.critic-findings.json` appears … WITHOUT the main loop having run `critic-consolidate` by hand (i.e. the hook did it)."* But "findings appeared and I did not type the command" is equally consistent with the **backstop** having done it, i.e. with the hook still being dead. The entry cannot distinguish its pass condition from its fail condition. It has been annotated with this, but **the annotation is a workaround; recording the trigger is the fix** — and until it lands, that queue entry can never return a meaningful verdict no matter how conscientiously it is run. This is the concrete instance of GOV-2W7Q's "write-only queue": an entry that is not just unrun but *unrunnable*.

  **The fix (the Critic's shape, agreed): a `--trigger` field on the `review.critic` ledger event**, set explicitly by each of the three call sites, so **a single review** makes a recurrence visible instead of requiring a 17-day archaeology. Implementation note so the size is not underestimated: `ledger_append` parses a **closed** arg set, so this touches the parser and the record builder as well as the three call sites and `consolidate()`'s signature — small, but not a one-liner. Do **not** try to infer the trigger inside `consolidate()`; the whole point is that the paths are indistinguishable from the inside.

  **`stage: design` — the shape is agreed, two details are not:** the trigger vocabulary (`subagent-stop` / `explicit` / `session-end-backstop`, and whether `explicit` should further distinguish the skill's own call from a human's), and whether the field is required or optional (required makes an unlabelled write a bug and is the stronger choice, but it means every caller must be updated in the same commit). Distinct from **TEL-2B6K**, which adds new event *kinds* (`gate.blocked`, `probe.fired`) and finding dispositions — this adds a field to an event that already exists, for a specific diagnostic that is blocking a verification entry today. Cross-linked, not merged. Governance-protected (Critic data plane + ledger) → full Critic + PR review. (critic — `final` review of the CRT-2J8N fix, 2026-07-27)

- **[GOV-2W7Q]** `.prawduct/operator-verification.md` is a write-only queue — the gate that drains it is OFF, six entries are pending, and one of them named CRT-2J8N's root cause seventeen days before it was found
  `effort: M · impact: L · area: governance · kind: bug · source: critic · added: 2026-07-27 · reviewed: 2026-07-27 · status: open · stage: requirements · related: CRT-2J8N, CRT-6B9M, CRT-3F7M · refs: .prawduct/project-state.yaml:352-361 (`operator_verification_required: false` and the comment stating WHY), plugin/bin/prawduct-hook:2798-2828 (`cmd_check_operator_verification` — `if not result["required"]: return 0`), plugin/lib/operator_verification.py (`run_check_operator_verification`), plugin/skills/pr/SKILL.md § Step 2b (the blocking caller), .prawduct/operator-verification.md (the queue — VRF-002 :46, VRF-003 :108, VRF-005 :182, VRF-006 :212, VRF-007 :247, VRF-008 :284)`

  **This is the escape analysis for CRT-2J8N, and it outranks the bug it explains.** The `SubagentStop` matcher defect was not missed for lack of thought. `operator-verification.md` **VRF-002** was filed **2026-07-10** to verify exactly it, and its own final verification step reads: *"If findings do NOT appear event-driven… investigate the matcher string (`prawduct:critic-reviewer` vs `critic-reviewer`) against the installed version."* It named the suspect **seventeen days** before the defect was found by an unrelated cumulative review. The system that was supposed to catch this did catch it — on paper — and then never ran.

  **Queue state, counted at the source 2026-07-27.** Eight entries; **six pending**: VRF-002 (added 2026-07-10 — the oldest pending, 17 days), VRF-003 (07-16), VRF-005 (07-17), VRF-006 (07-17, deferred), VRF-007 (07-19), VRF-008 (07-19). Two verified: VRF-001, VRF-004. Work is deferred *into* the queue correctly and conscientiously — the entries are detailed and well-written — and then never comes back out. That is the defining property: **write-only.**

  **Sub-problem 1 — the gate is off, so nothing enforces the queue.** `project-state.yaml:361` sets `operator_verification_required: false`, and `cmd_check_operator_verification` short-circuits on exactly that (`if not result["required"]: return 0`), so `/prawduct:pr create` Step 2b never blocks. The flip is one line, **but do not just flip it**: it would immediately block the next PR on six pending entries, several of which need a live harness or an unrun migration. This needs a **drain plan** (verify / accept-with-rationale / re-scope each of the six) sequenced ahead of the flip, not a flag change. *(Deliberately NOT flipped while filing — owner's call, and flipping today would have blocked the release.)*

  **Worth surfacing, because it reframes sub-problem 1: the stated reason the gate is off no longer describes what the queue holds.** `project-state.yaml:352-359` justifies the default as *"Prawduct has no human-facing UI surface to verify pre-merge; product repos that ship **visual changes** opt in."* But the queue's actual contents are not visual checks at all — they are **live-harness integration checks**: does a hook fire, does a matcher match, does a CLI round-trip against a real GitHub repo, does the briefing advance a coverage layer. Prawduct has no UI, so the stated rationale is true and irrelevant; it has a great deal of harness-integration surface that only a live session can exercise, which is precisely what the queue accumulated. **The queue drifted from the category its gate default was written for, and the default was never revisited.** Whoever takes this should decide the question the flag actually poses today — *does prawduct require live-harness verification before a PR?* — rather than re-answering the visual-changes question the comment asks.

  **Sub-problem 2 — the deferral test was too coarse, and that is the generalizable half.** VRF-002 deferred **all three** of its integration facts to a live check on the grounds that *"matcher-anchoring semantics vary by Claude Code version."* That is true of fact 3 (delivery — does the harness actually emit the event) and of fact 1. It is **false of the static half of fact 2**: whether a matcher *can* match a given `agent_type` is pure static analysis against a documented rule, it was testable on 2026-07-10, and it was broken. Bundling it with the genuinely-live facts moved a testable, false assertion into a queue that never drains.

  **The rule worth extracting and writing down** (this is the durable output, and it should land where deferral decisions are made — the operator-verification template and/or `building.md`): when deferring something to live verification, **split it in two** — *"can this be true in principle"* (static, testable now) and *"does the harness actually do it"* (live). **Only the second half belongs in the queue.** A deferral that carries a statically-decidable claim along with it launders an untested assertion into a backlog nobody reads.

  **Stage is `requirements`, not `ready`** — the drain plan, the flag's real question, and where the split-the-deferral rule lives are three decisions, not an edit. Governance-protected (a release gate) → full Critic + PR review. (critic — escape analysis for CRT-2J8N, 2026-07-27)

- **[CRT-6B9M]** Review heuristic worth having: "a guard behind a gate that never opens is not a guard" — a downstream defense cited as licensing upstream uncertainty
  `effort: S · impact: M · area: critic · kind: improvement · source: critic · added: 2026-07-27 · reviewed: 2026-07-27 · status: open · stage: design · related: CRT-2J8N, GOV-2W7Q, CRT-4M2P · refs: plugin/skills/critic/review-protocol.md (where a goal/heuristic would land), .prawduct/operator-verification.md VRF-002 fact 2 (the instance), plugin/bin/prawduct-hook (`cmd_subagent_stop` — the defensive `agent_type.endswith("critic-reviewer")` gate; **anchored by symbol, not line**: the original `:1193-1229` anchor went stale within one commit of being written, which is this repo's own documented reason for preferring symbols), plugin/hooks/hooks.json (the `SubagentStop` matcher — the gate that never opened), .prawduct/learnings.md`

  **The instance.** `cmd_subagent_stop` early-returns unless the reported `agent_type.endswith("critic-reviewer")` — a real, correct, defensive check. `operator-verification.md` VRF-002 fact 2 cited exactly that defense as the reason matcher uncertainty was tolerable: *"matcher-anchoring semantics vary by Claude Code version — the command defends with an `agent_type` endswith-check and is no-op-safe regardless."* But the check sits **downstream of the matcher**. The matcher never fired, so the guard never ran. It could not possibly have compensated for the thing it was cited as compensating for, and its presence is part of what made the uncertainty feel acceptable for seventeen days (CRT-2J8N).

  **Why this is a heuristic and not a note about one file.** The shape is general and recurs wherever a pipeline stage is defended: *a defense placed downstream of an unverified dispatch/routing/matching step provides zero coverage for that step's failure, yet reads as coverage in exactly the argument that should have questioned it.* The tell is a sentence of the form "X is uncertain, but it's fine because Y handles it" where **Y only executes if X succeeded**. That sentence is self-defeating and is detectable by reading, which is what makes it a Critic-shaped check rather than a test.

  Sibling shape already in the backlog: TST-9M2X's vacuous prose pin (a check that reports coverage it does not provide). Same family — *apparent* coverage that terminates the search — reached from the opposite direction: there the check runs and asserts nothing, here the check asserts correctly and never runs.

  **`stage: design` — the decision is WHERE it lives, and the options are not equivalent.** (a) A clause in the Critic's review protocol under a coherence/soundness goal, so it is applied every review — highest leverage, highest false-positive risk, since plenty of legitimate defense-in-depth is downstream of something and is not claimed to compensate for it. The discriminator must be the *claim*, not the topology. (b) A durable rule in `.prawduct/learnings.md` — cheap, no false positives, but only fires if someone reads it. (c) Both, with (b) first as case-law, consistent with how the ephemeral-reference firewall shipped (`cross-cutting-concerns.md:40` — case-law first, mechanical tripwire deliberately deferred). **(c) is the recommendation**; the repo has an established precedent for exactly this sequencing. (critic — escape analysis for CRT-2J8N, 2026-07-27)
...
- **[BLD-5R7K]** `resolve_chunk_progress`'s git-derived reading has an undocumented precondition — a commit convention written down nowhere — and degrades silently to the reading that always reports Chunk 01
  `effort: M · impact: M · area: build-plan · kind: bug · source: critic · added: 2026-07-27 · reviewed: 2026-07-29 · status: open · stage: design · refs: .prawduct/artifacts/buildplan-chunk-refs-discovery.md (requirements — success criterion 6 covers this item's two-part fix), plugin/lib/buildplan_refs.py:369 (`resolve_chunk_progress`), :185 (`_committed_chunk_ids` — the `Chunk NN` subject regex at :152 plus the scope filter), :343-366 (the `ChunkProgress.git_derived` docstring, which names this gap itself), plugin/templates/build-plan.md, plugin/methodology/building.md, tests/test_handoff_parser_correctness.py:227 (the only reader of `git_derived` anywhere)`
...
- **[GOV-8C3W]** Norm-disposition sweep stopped at the instance: `security-model`'s `--apply`/dry-run norm is undisposed in both session plans, while this bundle ADDED a destructive act
  ...
  **Verified counts.** `security-model.md` § Direction carries **three** norms: untrusted-governance-state-is-data; **destructive/irreversible operations require explicit owner approval at the OPERATION level, with preview-by-default where the command *is* the operation** (amended 2026-07-24 — this line previously quoted the pre-amendment absolute form, "no destructive action without an explicit `--apply` step / state-mutating lifecycle commands default to a dry run," and the sweep this item charters must be written against the *amended* norm, not the withdrawn one); and no cross-owner content egress. Both build plans dispose of exactly **one** — the untrusted-state norm, recorded retroactively during the Chunk 01 cumulative. The other two are undisposed in both. By contrast `architecture` (4 dispositions), `api-contract` (3) and `observability-strategy` (3) are complete in both plans. So the gap is specific to `security-model`, and it is a four-line sweep.
...
- **[DOC-4T6P]** Five documentation-accuracy defects: `cmd_clear`'s docstring keeps a quantifier its own change-log retracts, the change-log names the wrong function, the Enforcement table skips one of three new prose pins, a concerns cell cites a plan that will be deleted, and a test comment asserts a hook exclusion this bundle removed
  ...
  Filed from cumulative Critic review fact `rev-20260727T164842Z-1b70981d` (branch `feature/session-handoff-continuity`), findings **R-8** (item (d)) and three further documentation warnings, plus item (e) from the `final` review of the CRT-2J8N fix. Grouped as one small item because all five are single-line corrections; each is independently verified at the source.
  ...
  **(d) `cmd_clear`'s docstring keeps the quantifier its own change-log entry records as falsified.** Finding **R-8** (warning, goal *Everything Is Coherent*), verbatim summary: *"cmd_clear's docstring keeps the quantifier its own change-log entry records as falsified."* The docstring at `prawduct-hook:785-786` reads *"A continuation neither sweeps nor refuses — it **mutates nothing**, so the guard has nothing to protect."* The change-log entry in this same bundle (`change-log.md:103-104`) retracts exactly that shape: *"'The sweep is the one mutation on the continuation path' was a quantifier over-claim falsified by this bundle's own test asserting `.advisories.json` is written."* That counter-example is pinned by `tests/test_session_boundary_events.py::test_resume_refreshes_advisories` (`:176-182`), and `_untrack_session_files` additionally runs `git rm --cached` on the same path, mutating the index. So the sibling claim was corrected and this one — carrying the identical over-claim — survived the sweep. The **correct qualified form already exists 29 lines below at `:815`** ("mutates no session state"), so the module disagrees with itself about the exact property the CRT-3X9D guard rests on; the fix is to make `:786` say what `:815` says. **This is the coherence face of CRT-7N4B's correctness finding (R-1) — same sentence, same defect. Fix them together**: under CRT-7N4B remedy (b) this correction *is* half the remedy, and under remedy (a) the sentence has to change anyway. Note also that this is a **third** instance of the branch's headline lesson — a claim sweep that fixed the sites a review named and not the class of sentences sharing the reversed wording.

  **(e) A test comment asserts a hook exclusion this bundle removed.** `tests/test_plugin_methodology_digest.py`, in `test_digest_matcher_includes_compact`, opens with: *"Unlike the clear/briefing hook (a state-reset, excluded on compact), the digest is pure guidance."* Verified against `plugin/hooks/hooks.json` as it now stands: the clear hook's two registrations are `startup|clear → clear --session-start` and **`resume|compact|fork` → `clear --session-start --brief-only`**. The clear/briefing hook **does** fire on `compact` — that is precisely the "INCIDENTAL GAP" the session-boundary-events work closed (see SCN-5B8Q). So the contrast the comment draws no longer exists, and its parenthetical is now false. The **assertions are unaffected and still pass correctly**; only the comment is stale. Fourth instance of the same claim-sweep miss.

  **THE INSTANCE IS ONE LINE; THE SEARCH-SPACE GAP IS THE DEFECT.** The sweep covered runtime strings and durable prose, and **test comments were never in the search space**. The cheap widening: the next claim sweep's grep surface must include `tests/`, not just `plugin/` and `.prawduct/`. A test comment is durable prose that a maintainer reads as license — `learnings.md` already records the worst case of exactly this (a test docstring that "hands a maintainer written license to revert the test back to the defect").

  **Deliberately filed rather than fixed in the working tree (sequencing, not deferral).** A `verify-resolutions` pass was running against the tree; `tests/` is judgeable, so a comment edit landing after that fact would put the working tree outside the coverage span and demand another delta pass — the fifth round on a one-line matcher change, on the branch that spent six rounds learning that *each fix round generates the next round's review subject*. A known-false comment filed **with its exact location and correction** is a better state than an unfiled one, and materially better than a fifth round before a release.

  **I ran the widened sweep so this item states a result, not a suspicion.** Over `tests/`, for the reversed propositions of this bundle: **exactly one genuine stale claim — this one.** Three other hits checked and correct, most usefully `tests/test_plugin_runtime.py::test_state_reset_matcher_excludes_every_continuation`, which faces the identical two-entry split and handles it properly (it filters the reset entry by the *absence* of `--brief-only` rather than by the `clear` token, and its comment explains the successor relationship). **That test is the model the digest test's comment should be reconciled against** — the correct treatment already exists one file away.

  **Two refinements, because the obvious fix is the wrong one.**
  - **Do NOT "fix" this by adding `tests/` to the prose pin's `SURFACES` roster.** Verified by running the pin's own regex over both files: widening the roster catches **nothing here — including this defect** — because `_BARE_NEXT_SESSION` encodes one proposition family ("next session start" / "auto-clears next session") and this comment asserts a *different* proposition ("excluded on compact"). File-surface widening and claim-inventory widening are separate moves, and only the second finds this.
  - **The deeper cause is `learnings.md`'s round-five lesson (a) recurring, not a new one.** That entry says: *grep the reversed sentence's own wording, not the wording of the sites you already found* — and its own missed hit was in a test module's docstring, generalized there to **case-insensitivity**. Since `git grep -i` inherently covers `tests/`, tooling was never what excluded them: the **proposition inventory** was incomplete. This bundle reversed **two** propositions — the waiver lifetime *and* the compact exclusion — and only the first was swept. Worth adding to that learning as the third repetition: enumerate the propositions the change reversed, then sweep each one's own formulations; sweeping one proposition thoroughly still misses the other entirely.

  **Related but NOT filed, deliberately** — the same test's `assert trigger in matcher` uses substring reasoning, while the matcher it inspects (`startup|resume|clear|compact|fork`) is a `|`-separated list of exact strings, so membership ≠ substring. Note the attribution: that is true of `SessionStart` under `SessionStart`'s own literal class — **not** a consequence of the `SubagentStop` rule CRT-2J8N established, which does not carry across events. It is **loose, not false**: every trigger it checks is a genuine element of `startup|resume|clear|compact|fork`, so all assertions are true today and the test's intent holds. The weakness is a possible false *pass* against a hypothetical future matcher (`compacting` would satisfy `"compact" in matcher` while never matching). A one-line strengthening is available for free — reuse the `_matcher_matches` helper now in `tests/test_critic_reviewer_agent.py`, which encodes the documented two-path rule — and is worth doing **if someone is already in the file**, but it is not a defect and was not filed as one. (critic — `final` review of the CRT-2J8N fix, 2026-07-27)
...
- **[COV-3M8Q]** Review treadmill on behaviour-neutral `.py` edits (docstring/comment-only) — the content-equivalence route is RULED OUT (2026-07-29); the still-open half is making review cheaper, not skippable
  `effort: M · impact: M · area: governance/gates · kind: question · source: user · added: 2026-07-21 · reviewed: 2026-07-29 · status: open · stage: research · related: COV-2P7F, COV-4H7N, COV-6T3P, PR-5K8D · refs: plugin/lib/coverage_algebra.py:63 (is_judgeable_path — the extension/prefix-only classifier; its docstring now carries this ruling), plugin/lib/coverage.py (cmd_check_pr_doc_only), plugin/lib/waivers.py + plugin/lib/compliance.py:114 (the prawduct:allow source-comment pragma — why AST equality is unsound HERE), tests/preferences/test_session_boundary_prose.py:37-45 (SURFACES includes plugin/lib/critic_marker.py — tests assert over .py prose), .prawduct/artifacts/nonfunctional-requirements.md § Direction (the ratified goal that stays live), .prawduct/artifacts/build-plan-record-mechanization.md Chunk 03 (per-mode reviewer payload — the sound route), review rev-20260730T010655Z-b7af83df (10 blocking findings on the reverted attempt), .prawduct/learnings.md (#258 — separate the rejected direction from the rejected primitive; #91 — language-agnosticism)`

  **RULED 2026-07-29 — content-based AST equivalence is unsound in this repo. Do not rebuild it.** Not reasoned: the mechanism was fully built (normalized-AST keying for Python + lazy retry in both gate paths, proven on real git blobs), reviewed at `rev-20260730T010655Z-b7af83df` — **10 blocking findings, all three coordinator reviewers independently finding the same hole** — and reverted. `plugin/lib/gates.py` is byte-identical to `origin/develop`; `coverage_algebra.py` carries only the ruling docstring, no mechanism.

  **Why no narrowing fixes it.** `plugin/lib/waivers.py` defines the `prawduct:allow` **source-comment** pragma and `plugin/lib/compliance.py:114` acts on it, so adding `# prawduct:allow prawduct/broad-except -- looks fine` to an existing `except Exception:` suppresses a compliance check while leaving the AST **byte-identical** — an AST-keyed free edge would demand *no* review at exactly the point `skills/critic/review-protocol.md` Goal 3 obliges a reviewer to judge that waiver. Compounding channel: text-level pins in `tests/` assert over `.py` prose (`tests/preferences/test_session_boundary_prose.py` pins `plugin/lib/critic_marker.py` among its surfaces), so docstring edits can break tests too. The general lesson: **AST equality proves the *parser* sees the same program, not that the *system* behaves the same** — and here source is read as text. Leg 2 below was exactly right: *"a bug in the equivalence check ships unreviewed code. That is the unsafe direction."*

  **Why this stays open, and where it points now.** The treadmill is real and unfixed, and the goal stays ratified (`nonfunctional-requirements.md` § Direction — review wall-clock is P0, run-count is the lever). The sound route is to make review **cheap** rather than skippable: **Chunk 03 of `build-plan-record-mechanization.md`** (per-mode reviewer payload — a `verify-resolutions` reviewer today loads the full 217-line protocol plus the review-cycle to answer three goals, measured 5–10 min against a 1–2 min target).
  ...
  **Relationship to the existing family** — this is a *different axis*, not a duplicate. COV-2P7F / the `.prawduct/`-umbrella item / PR-5K8D all argue about which *paths* belong on which side of a path-based classifier. This one asks whether a *content-level proof* may ever override a path classification at all. Answering "no, deliberately" is a legitimate and cheap outcome — **and that is the answer that came back (2026-07-29); the rationale is now recorded in `is_judgeable_path`'s docstring as this item asked.**
...
- **[BKL-7Q4M]** Safe upstream filing — a private consuming repo must be able to file a prawduct bug into prawduct's PUBLIC repo without leaking its own content; the missing design is CONTENT MINIMIZATION, not auth
  ...
  **The questions settled during requirements (XP4–XP7) — now the inputs the design answers to:**
  ...
  3. **Owner preview-and-consent** — is preview-and-consent **mandatory per report**? Is the shape a dry-run/`--apply` pair, consistent with the standing norm — which since 2026-07-24 reads *"destructive/irreversible operations need explicit owner approval at the **operation** level, preview-by-default where the command IS the operation"*, **not** the pre-amendment absolute *"no destructive action without an explicit `--apply` step"* this line used to quote? The amendment matters to this design: operation-level approval means one informed consent for a batch, not an `--apply` on every action. (Cross-check BKL-8V3D: `adapter-mode.md` already *claims* an `--apply`/dry-run contract that `lib/backlog/` does not implement — do not design against a contract that does not exist yet.)
...
- **[REL-7D4X]** Release runbook’s change-log sweep selects the wrong set three ways — positional (drops entries that merged below the boundary), scope-narrowed (drops whole scopes, W-1), and under a pruned release it tags withheld work as shipped (REL-8P6M (e), held here with its trigger)
  `effort: M · impact: M · area: docs · kind: fix · source: user · added: 2026-07-21 · reviewed: 2026-07-29 · status: open · stage: design · related: REL-2N8K, REL-5W2J, REL-9F2T, REL-8P6M, REL-6Q4M · revisit: the change-log-ledger decision (adopt or reject `.prawduct/artifacts/change-log-ledger-design.md`) — that design deletes this sweep outright, so the selection rule is not rewritten until the decision is taken; until then every release tags its shipping subset by hand, once, across every scope · refs: .prawduct/runbooks/cut-and-publish-a-plugin-release.md (Phase 1 steps 2–3 — step 2 now carries the positional warning, the scope warning, and the 🚧 hold note; `last_verified: null`), .prawduct/artifacts/release-plan-v3.1.1-hotfix.md, .prawduct/change-log.md, docs/release-process.md, .prawduct/artifacts/change-log-ledger-design.md (the pending decision that is this item’s trigger), .prawduct/artifacts/build-plan-release-readiness.md (Chunk 03 — where (e) was held)`
  ...
  **Carries REL-8P6M (e) and W-1 as of 2026-07-29 — this is now a three-dimensional selection defect, not one.** REL-8P6M shipped its other five parts (a–d, f) and archived; (e) was **deliberately held** and lives here, because all three defects rewrite the same rule in Phase 1 steps 2–3 and must be resolved together:
  1. **Positional** (this item as filed) — the sweep flips everything above the boundary, so a genuinely unreleased entry that merged *below* it is dropped silently.
  2. **Scope-narrowed** (W-1, PR review of `feature/v3.2.0-c02-adapter-safety`) — re-deriving the set with `grep scope=<the-one-you-remember>` returns a subset that looks complete. Measured on `feature/rel-8p6m-releasability-gate` @ `1a353d1`, above the `release=v3.1.2` boundary: **23 release-pending entries across six scopes**, of which `v3.2.0-golive` is 7 — so that grep misses **16 across five other scopes** (`release-readiness` 7, `coverage-perf` 4, `chunk-refs-gate` 2, `critic-disposition` 2, `review-loop-termination` 1). One is the fleet-visible `protected_path_violation` widening, which also shortens the consumer-facing release notes, since step 10 derives them from the same set. *(The figure is a measurement of one tree and moves with every merge — the runbook carries the derivation command; recount rather than cite.)*
  3. **Wrong subset under a pruned release** (REL-8P6M (e)) — the sweep appends `release=vX.Y.Z | status=shipped`; under a pruned promotion, tagging everything above the boundary marks **withheld** work as shipped. prawduct has now pruned twice (v3.1.1, v3.1.2), so this is the pattern, not the exception.

  **Why it is not fixed yet, and what un-holds it.** Owner decision 2026-07-29: `.prawduct/artifacts/change-log-ledger-design.md` proposes replacing this machinery with a ledger, and lists REL-7D4X, REL-8P6M(e) and W-1 under *"closed outright — the mechanism is gone, not fixed."* Rewriting the selection rule before that decision is throwaway work. **The trigger is the ledger decision itself (adopt or reject)** — not a date; see the `revisit:` field. Until then the stopgap is in force and is documented at the point of use: Phase 1 step 2 carries a 🚧 note naming this hold, so a reader who correctly judges the rule wrong does not "fix" it in ignorance of the pending decision.

  **`stage: design`, downgraded from `ready` on 2026-07-29** — the fix is no longer an edit. Which shape survives (patch the sweep vs. delete it with the ledger) *is* the pending decision, so presenting this as buildable would route someone straight into the work the owner declined. Re-raise to `ready` when the ledger decision lands. Effort raised S→M for the same reason: three dimensions, not one. (owner — REL-8P6M part (e) retention, build-plan-release-readiness Chunk 03)
...
- **[CRT-3F7M]** Critic coordinator dispatches reviewer subagents from a skill fork; they die with the fork, leaving a valid manifest and zero partials — the review hangs silently forever
  ...
  **BLOCKER ON THIS REPRO CLEARED — 2026-07-27 (CRT-2J8N, shipped).** Run the repro *now*, and re-read the symptom evidence above before you do. The `SubagentStop` matcher was `critic-reviewer` while the runtime `agent_type` of a plugin subagent is the scoped `prawduct:critic-reviewer`, and **for `SubagentStop`** Claude Code treats an alphanumeric matcher as a literal — one exact string, or a `|`/`,`-separated list of exact strings (https://code.claude.com/docs/en/hooks, verified 2026-07-27; corroborated against the installed Claude Code 2.1.220 matcher implementation). Not a cross-event rule: the `SessionStart` matchers in the same file are exact lists and were always correct. **So the per-reviewer consolidation trigger had never fired on any install since it shipped.** That is very likely the mechanism behind this item's whole symptom class: reviewers complete, nothing consolidates, and from the caller's seat "the Critic ran and nothing came back." Every prior observation here was taken with that confound present, so the "died with the fork" readings are not clean evidence about dispatch lifetime. Fixed to `(^|:)critic-reviewer$` and pinned; the repro can now measure await-in-fork rather than measuring that bug.
...
- **[ONB-3F9P]** `/prawduct:onboard` and `/prawduct:doctor` never provision the backlog surface or the label taxonomy — GV5/GV6 have no implementation
  ...
  **[STATUS POINTER 2026-07-24 — partially built; the body above reads as fully unfixed.]** Leg (1) — the `init-product --backlog-repo` flag recording `backlog_service_repo` — is built on `feature/v3.2.0-c02-adapter-safety`; the `status=shipped` flip is deferred to **Chunk 09** per this release's `[ ]`-until-release convention. **Legs (2) and (3) remain open** (the onboard provisioning step and the doctor `reconcile-labels` repair), so unlike its Chunk 02/03 siblings this item is genuinely *partly* unfixed — do not read the pointer as "done." A 2026-07-24 Critic warning also notes the new onboard section **states none of the cutover's cost**: writing that scalar is the same flip the scrub runbook Step 6 calls "the cutover," which retires six markdown-premise advisory probes and fires `backlog-checks-dormant`. An owner choosing day-one Issues silently accepts that the Critic's backlog reconciliation walk and four hygiene checks, the PR reviewer's two consistency checks, the janitor's Backlog Health block, and three norm-lifecycle probes are dormant from the repo's first session. The scrub path's own standard is "state the cost plainly before the owner chooses" (Step 3c); onboard should meet it. (Prose left intact per BKL-8Q3D.)
...
- **[BKL-2Q7F]** `migration-scrub.md` never selects, creates, or provisions the target repo — `--repo <owner/repo>` is an unbound placeholder in six commands across four steps, and `provision` appears in no skill file
  ...
  **[STATUS POINTER 2026-07-24 — the body above reads as unfixed; it is BUILT.]** The work is built on `feature/v3.2.0-c02-adapter-safety` (Chunk 03); the `status=shipped` flip is deferred to **Chunk 09** per this release's `[ ]`-until-release convention, so the item correctly stays `open`. **Fix-shape divergence, recorded so the delivered design is not mistaken for a partial one:** the shape written above — "recorded to `backlog_service_repo` before any adapter call" — is the shape **Chunk 03 decision #1 deliberately REJECTED.** The delivered shape binds the target as a **scrub decision** (`--repo <target>`, bound once and reused by every subsequent step) **rather than flipping the scalar early**; flipping `backlog_service_repo` before the migration would make the cutover the *first* step instead of the *last*, and the scalar's flip is what retires the markdown-premise probes. Regression-pin gap tracked separately as **BKL-4M6T**. (Prose left intact per BKL-8Q3D — this pointer supersedes it rather than rewriting it.)
...
- **[BKL-8V3D]** `adapter-mode.md:96` promises the model an `--apply`/dry-run safety contract that does not exist — zero occurrences in `lib/backlog/`
  ...
  **[STATUS POINTER 2026-07-24 — the body above reads as unfixed; it is BUILT.]** The work is built on `feature/v3.2.0-c02-adapter-safety` (Chunk 02); the `status=shipped` flip is deferred to **Chunk 09** per this release's `[ ]`-until-release convention, so the item correctly stays `open`. The "worth a guard" test above **shipped** as `tests/test_backlog_instruction_surface.py`. (Prose left intact per BKL-8Q3D.)
...
- **[BKL-5N9W]** `skills/backlog/SKILL.md` pairs `disable-model-invocation: false` with a wildcard `Bash(prawduct-hook backlog *)` grant — narrow it to the ops the skill drives
  ...
  **[STATUS POINTER 2026-07-24 — the body above reads as unfixed; it is BUILT.]** The work is built on `feature/v3.2.0-c02-adapter-safety` (Chunk 03); the `status=shipped` flip is deferred to **Chunk 09** per this release's `[ ]`-until-release convention, so the item correctly stays `open`. The metadata test shipped, in both invocation forms. **Two follow-ons from the 2026-07-24 cumulative Critic — one closed, one open:**
  - **CLOSED (`4e5bb19`)** — the Critic warned the negative assertions were exact-string rather than parsed, so a *broader* wildcard (`Bash(prawduct-hook *)`, or `Bash(prawduct-hook backlog*)` with no space) would re-grant every withheld op while leaving all three tests green. `test_scrub_ops_not_granted` now expands each granted `Bash(...)` pattern glob-style and asserts no pattern **matches** a scrub-op invocation, so the check catches the **widening** rather than one spelling of it. Verified red-first. The point it encodes is worth keeping: widening a grant is the most natural way this rail gets dismantled ("the prompts are annoying"), so a negative safety assertion has to be semantic, not literal.
  - **CLOSED (`85753fd`) — GOV-5J9Q** — the narrowing did not ripple to `janitor`/`pr`/`runbook` SKILL.md, which still carried the wildcard in both invocation forms. All six grants were deleted (none of the three invokes the CLI; all reach the backlog through `/prawduct:backlog`). `janitor` needed more than deletion: its legitimate `Bash(python3 *)` grant subsumes the python3 twin, so the model-initiated path was closed with `disable-model-invocation: true` — a field it was missing entirely, and one that is opt-out.

  (Prose left intact per BKL-8Q3D.)
...
- **[BKL-2K8V]** pick latency ~12.4s at 209-issue scale, ~6x the NFR <2s floor — dominated by the gh-subprocess full-scan; the "flat across candidates ⇒ not N+1" reading is RETRACTED
  `effort: M · impact: M · area: backlog-service · source: user · added: 2026-07-17 · reviewed: 2026-07-28 · status: open · stage: design · related: BKL-6M4T, BKL-3N8Q · refs: .prawduct/operator-verification.md VRF-009 (the identical retraction, against the 2026-07-24 re-run), tests/spikes/s2_migration.py (`check_pick_latency` docstring — records the invalid inference), plugin/lib/backlog/query.py (`pick` — the limit-bounded lazy fan-out), documentation/backlog-service-nfr.md §4 (PROBE-LAT)`

  Settled live by SPIKE-S2 (2026-07-17 dry-run, ~209-issue throwaway repo). pick_latency_ms_by_candidates measured {1: 12528, 3: 12390, 5: 12380} — i.e. ~12.4s and FLAT across 1/3/5 candidates. Two reads: (a) ~~GOOD — the ready-work fan-out is NOT N+1 (no growth with candidate count), consistent with a batched/cheap fan-out~~ **← RETRACTED, see below; this read was false and the probe could not have produced a disconfirmation**; (b) BAD — the absolute floor is ~6x the NFR <2s target, dominated by the FIXED cost of query._all_issues fetching every open issue across paginated `gh` subprocess calls (subprocess spawn + REST round-trips), not the per-candidate blocker fan-out. This SETTLES the NFR §4 PROBE-LAT open question (previously target-grade, S2-to-measure): the <2s floor is NOT met on the `gh`-subprocess path at ~200-issue scale. Resolution: the <2s floor requires the raw-HTTP/GraphQL fast-path (roadmap W1) or a scoped candidate query that avoids the full-issue-scan — not the current `gh` REST-over-subprocess path. Record the measured number into NFR §4 and re-state pick's <2s as W1-gated (or scoped-query-gated), not slice-native. Caveat (honest confidence): measured under light concurrent read load (a 4-min progress poll) and on one machine/one API-latency sample; the ~12s magnitude and flatness are robust, the exact ms is one sample.

  **Read (a) RETRACTED (recorded here 2026-07-28; the same retraction was first written against VRF-009 on 2026-07-24). The probe was structurally incapable of detecting what read (a) claimed.** The candidate count the probe varies **is `limit`**, and `pick` applied `limit` only *after* fanning out over every eligible issue — one `list_blocked_by` REST read per eligible issue, then a slice at the end. So varying 1/3/5 varied **nothing whatsoever** about the number of blocker reads: flatness was **guaranteed by construction**, whatever the underlying call shape. What the probe actually measured was the constant `_all_issues` full-scan. The fan-out **was** N+1 REST the entire time, and there is **no GraphQL anywhere in `plugin/lib/backlog/`** — zero matches — so the "batched/cheap fan-out" it was read as evidence for was never built. Keep this on the record rather than deleting it: the trap is reusable and worth recognizing — a confirmation was read out of a measurement whose design could not have produced a disconfirmation, so the flat numbers felt like evidence while carrying none. Read (b) is untouched by this and remains the item's substance.

  **What changed since — Chunk 05b (2026-07-28, commit `2ee2a17`).** `pick` now ranks first and takes the dependency reads **lazily in rank order, stopping once `limit` is filled** (`plugin/lib/backlog/query.py`, `pick`), for an identical result set at O(limit + blocked-skipped) instead of O(backlog). Two consequences for this item: (1) the per-candidate reads no longer contribute to the tail, so a re-measure would isolate the `_all_issues` full-scan cost this item is really about; (2) the probe's candidate-parameterization is **finally meaningful** — a re-run yields a slope that means something, so read the *slope*, not the magnitude (`check_pick_latency`'s docstring in `tests/spikes/s2_migration.py` now says so at the source).

  **Remaining scope: the `_all_issues` full-scan floor only.** `pick` still fetches every open `stage:ready` issue across paginated `gh` subprocess calls on every call. That fixed cost is what keeps the <2s NFR out of reach, and closing it is **W1-gated** — the raw-HTTP fast-path or a scoped candidate query that avoids the full-issue-scan — not slice-native. Stays `stage: design` because that choice is the open call.
...
- **[BKL-6X5D]** Archive window never quantified (binary `{all,open}` is the only shipped lever); Pacer doesn't meter 900 REST pts/min for the create+close archive stretch — part (b) is a **v3.2.0 release blocker** gating prawduct's own migration (A1 chose `--archive-scope all`)
  `effort: S · impact: M · area: backlog-service · source: user · added: 2026-07-17 · reviewed: 2026-07-24 · status: open · stage: ready · related: BKL-6M4T · refs: backlog-service-prd.md#89, backlog-service-nfr.md, .prawduct/artifacts/migration-scrub-decisions.md (decision 5 — A1 = `all`), .prawduct/artifacts/release-plan-backlog-service-golive.md (item 8 / C7-before-C4)`
  ...
  **Part (b) BUILT 2026-07-24 (v3.2.0 Chunk 04).** The Pacer now meters *total* REST points against the 900/min burst ceiling — 5 pts/write, 1 pt/read, counted across **create AND close** — via the `_PacingTransport` decorator, closing the create-then-close metering gap (previously only `before_create()` was paced, so the archive stretch ran half-metered — the exact >900 pts/min window this note describes). Design was confirmed against **current GitHub REST docs first** (Chunk 04 Done-when #0) before coding. **NFR §3.3/§9 updated** to match the built behavior; §9's S2 obligation is now the *live confirmation* of the fix — deferred to the **Chunk 05 dry-run** (run `--archive-scope all`, so it proves the Pacer holds and not a small input). **Still deferred:** part (i), the window **quantification** (an N-months / throughput formula between the `{all,open}` poles), stays the adopter-scale refinement — unbuilt, unchanged. The `status=shipped` flip is **deferred to release (Chunk 09)** per the `[ ]`-until-release convention, so this item stays `open`; **stage advanced design→ready**.
...
- **[BKL-3H7W]** `_PacingTransport` meters per METHOD CALL, not per HTTP request — a paged list costs 1 point regardless of page count, so the REST-point budget systematically under-counts
  `effort: M · impact: L · area: backlog-service · kind: bug · source: critic · added: 2026-07-24 · reviewed: 2026-07-24 · status: open · stage: ready · related: BKL-6X5D, BKL-6M4T · refs: plugin/lib/backlog/migrate.py:270-273 (_PacingTransport docstring — "meters every REST call routed through it"), plugin/lib/backlog/migrate.py:296-299 (_cost — classifies by method-name prefix), plugin/lib/backlog/transport.py:112 (_PAGED_MAX_PAGES = 100), plugin/lib/backlog/transport.py:497 (list_labels via _api_paged), plugin/lib/backlog/migrate.py:943 (_create_item calls provision.ensure_labels per item), documentation/backlog-service-nfr.md:146 ("charges every REST call"), .prawduct/operator-verification.md:347 (VRF-009 rest_points_charged: 5360 — derived from the under-counting meter)`

  **Filed from the 2026-07-24 cumulative Critic review (rev-20260724T205224Z-4a1c2528, warning).** Follow-on defect in the Pacer that BKL-6X5D part (b) built in Chunk 04 — not a re-filing of that item.

  `_PacingTransport` charges `cost` once per attribute-call (`__getattr__` → `paced()`), but `GhTransport._api_paged` issues up to `_PAGED_MAX_PAGES` (100) HTTP requests **inside one call**. `list_labels`, `list_sub_issues`, and `list_timeline` all route through `_api_paged`, so each is charged **1 point regardless of page count**.

  Not theoretical for Chunk 06: `_create_item` calls `provision.ensure_labels()` → `transport.list_labels()` **per item**, and `_api_paged`'s own docstring names the exact scenario ("once a migrated repo carries hundreds of `id:PFX` aliases"). For a ~300-item `--archive-scope all` run the label listing grows from 1 to ~4 pages, so actual label-listing requests ≈ 700 while 295 points are charged — roughly a **7% under-count**, growing with backlog size.

  **Scoped honestly: this is an under-count, not an unmetered path.** Every write method on `Transport` is single-request, so **no write escapes the meter**; the shortfall is reads only, and VRF-009's ~2x observed margin (peak ~500 pts/min vs 900) absorbs it — which is why the review graded it warning, not blocking.

  **Three durable surfaces now state a completeness the mechanism does not have, and must be corrected with the fix** (this is the reason the item is `impact: L` despite a small runtime effect — the claims outlive the margin that currently covers them):
  1. `plugin/lib/backlog/migrate.py:270-273` — "meters **every** REST call routed **through it**";
  2. `documentation/backlog-service-nfr.md:146` — "charges **every** REST call";
  3. **the VRF-009 record** — `.prawduct/operator-verification.md:347`.

  **VRF-009's settled §9 S2 constant was derived from the under-counting meter.** `rest_points_charged: 5360` ("~296 pts/min average") is therefore a **floor, not an exact figure** — the true total is higher. The qualitative conclusion **survives**: under-counting cannot manufacture a false "no waits" result, so "budgets never bind under serial I/O" still holds. But the number must not be re-used as exact, and the record should say so.

  **Fix-shape (two options, the review's):** meter inside the **request seam** — charge `_REST_READ_POINTS` per `_api` call via a per-request hook on the transport — **or** restate all three claims as "every transport *call*, one point per paged list regardless of page count" and annotate VRF-009. The first makes the claims true; the second makes the claims honest. Pick deliberately. (critic — rev-20260724T205224Z)

- **[BKL-8K2N]** Pacer telemetry is dropped and every throttle sleeps silently — a paced ~900-issue migration is indistinguishable from a wedged one
  `effort: S · impact: L · area: backlog-service · kind: bug · source: critic · added: 2026-07-24 · reviewed: 2026-07-24 · status: open · stage: ready · related: BKL-6X5D, BKL-6M4T, BKL-3H7W · refs: plugin/lib/backlog/cli.py:507 (_run_import — the production entry point, passes no pacer), plugin/lib/backlog/migrate.py:171-184 (Pacer.before_points — sleeps with zero output), plugin/lib/backlog/migrate.py:251 (RateLimitBackoff.pause — sleeps with zero output), plugin/lib/backlog/migrate.py:142-144 (points_charged / point_waits — accumulated then dropped), tests/spikes/s2_migration.py::_record_pacing (the only reader in the tree)`

  **GATES CHUNK 06.** Filed from the 2026-07-24 cumulative Critic review (rev-20260724T205224Z-4a1c2528, warning) — escalated here because Chunk 06 is a single irreversible bulk write of ~900 issues and this is the observability gap that matters most for it.

  The production entry point `cli._run_import` passes **no `pacer`**, so the default `Pacer()` accumulates `points_charged` / `point_waits` / `total_point_waited` / `waits` / `total_waited` (and `RateLimitBackoff.pauses` / `total_paused`) and then **drops them**. `tests/spikes/s2_migration.py::_record_pacing` is the **only reader in the tree** — the telemetry exists solely for the SPIKE-S2 harness.

  Compounding it: `Pacer.before_points` can sleep up to ~60s and `RateLimitBackoff.pause` up to 60s × 5 retries, **both with zero output**. The only `_diag` call in `migrate.py` is the unexpected-boundary handler. Human-mode result is one line: created/skipped/collisions/total.

  **Net effect on the one irreversible operation:** during a ~20–40 minute run the operator **sees nothing**; a throttled or backing-off run is **indistinguishable from a hang**; and afterwards nobody can say where the budget stood — the exact quantity Chunk 04 was built to produce and Chunk 05 spent a live throwaway run measuring. An operator with no signal is an operator who kills a healthy run.

  Fix-shape: thread a `pacer` through `_run_import`, emit progress + pacing state on the human path (and into the `--json` envelope), and give both sleep paths a diagnostic line so a wait announces itself as a wait. (critic — rev-20260724T205224Z)

  **[BUILT 2026-07-24 — `aaf068f` on `feature/v3.2.0-c02-adapter-safety`.]** `status=shipped` flip deferred to **Chunk 09** per this release's `[ ]`-until-release convention, so the item stays `open`.

  **Diagnosis correction — the body above is slightly wrong, and the fix-shape line is wrong in a way worth naming.** `import_items` **already constructed a default `Pacer()`**, so the run was **always paced**; the migration was never running unmetered. The defect was narrower than "telemetry is discarded on the real migration path" implies: the **counters were dropped** and **both sleeps were silent**. Threading a pacer from the CLI was therefore *not* the fix — surfacing the one already there was. (Left in place per BKL-8Q3D; this note supersedes it. The title overstates the same way — read it as "telemetry is discarded," not "pacing is absent.")

  **What shipped:**
  - **All three sleep paths announce before blocking** — `Pacer.before_create`, `Pacer.before_points`, `RateLimitBackoff.pause` — each naming the budget, spend-so-far, and resume time. The rate-limit line also names the server's `Retry-After` when it gave one, so "why this long?" is answered in the line itself.
  - **`migrate.pacing_summary()` rides EVERY import exit path** — success and both resumable cuts — because a run that stopped is exactly when the question gets asked.
  - **It prints in human mode too**, not just `--json`: `migration-scrub.md` invokes import without `--json`, so a JSON-only footer would have missed the actual operator.
  - **Counter names match the SPIKE-S2 recorded-facts vocabulary** (`rest_points_charged`, `rest_point_waits`, `content_creation_waits`, `rate_limit_pauses`, …), so a dry-run and a real run read side by side with no translation step.
  - **7 tests, red-first**, including `test_an_unthrottled_run_stays_quiet` — a guard that pacing output is **exception reporting, not commentary**. A summary that narrates a healthy run trains the operator to ignore it, which would have re-created the defect in a louder form.

  **BKL-3H7W is unaffected and still open** (now cross-linked): this changed what is **reported**, not what is **charged**. The per-request under-count — a paged list costing 1 point regardless of page count — is orthogonal, and the newly-surfaced `rest_points_charged` figure inherits it. Read the reported number as a **floor** until BKL-3H7W lands.

  **The floor is rendered, not just documented (`cli.py:876`).** The human-mode footer prints `pacing: ≥N REST points` — the `≥` is deliberate and load-bearing, not decoration: the meter charges per transport *method* call rather than per HTTP request, so an unqualified number would read as precise to the one person sizing an irreversible run. **The title of this item was also corrected 2026-07-24** for the same class of reason — it previously read "telemetry is discarded on the real migration path — `cli._run_import` passes no `pacer`", which invited the false conclusion that the run was unmetered; a title travels into `list`/`pick` where the body's correction does not. `tests/test_backlog_migrate.py:1278` (`assert "≥" in out`) pins the marker so it cannot quietly degrade to a bare number before the under-count is fixed. **The `≥` comes off when BKL-3H7W lands** — that is the visible signal this item's accuracy is still owed.

- **[GOV-7W3D]** 6 of 10 `## Direction` norms across the plan's `governed_by:` artifacts carry NO disposition — including the `--apply`/dry-run norm that most directly governs Chunk 06 — and `observability-strategy.md` is absent from `governed_by:` entirely
  `effort: M · impact: M · area: governance · kind: debt · source: critic · added: 2026-07-24 · reviewed: 2026-07-24 · status: open · stage: ready · related: BKL-8V3D, BKL-5N9W, ONB-3F9P, MET-6T4K · refs: .prawduct/artifacts/build-plan-v3.2.0-golive.md (governed_by: frontmatter), .prawduct/artifacts/security-model.md (§ Direction — the --apply/dry-run norm), .prawduct/artifacts/api-contract.md (§ Direction — additive-first), .prawduct/artifacts/architecture.md (§ Direction — "writes nothing into a governed repo except its own .prawduct/ state"), .prawduct/artifacts/observability-strategy.md (three ratified Direction norms, absent from governed_by), plugin/skills/backlog/adapter-mode.md ("No generic preview-or-apply flag sits over those mutations")`

  **Filed from the 2026-07-24 cumulative Critic review (rev-20260724T205224Z-4a1c2528, warning; fid R-22).**

  `build-plan-v3.2.0-golive.md`'s `governed_by:` frontmatter records **one** disposition for `security-model` (which carries 3 Direction norms), **two** for `architecture` (4 norms — and one of the two quotes a *sub-clause* of the Local-first norm rather than a norm), and **one** for `api-contract` (3 norms). Applicability for the remaining six was **assumed, not recorded**.

  **Three of the undispositioned norms are directly implicated by this bundle:**
  - **(a)** `security-model`'s *"No destructive action without an explicit `--apply` step; state-mutating lifecycle commands default to a dry run"* — the norm that **most directly governs the Chunk 06 bulk write**, and the one **Chunk 02 in this very bundle established the adapter does NOT satisfy** (`adapter-mode.md` now says plainly "No generic preview-or-apply flag sits over those mutations"). An undispositioned norm that the bundle itself proved non-conforming is the sharpest case: silence reads as `conforms`.
  - **(b)** `api-contract`'s additive-first norm — implicated by BKL-5N9W narrowing an existing `allowed-tools` grant so previously-silent ops now prompt.
  - **(c)** `architecture`'s *"the plugin writes nothing into a governed repo except its own `.prawduct/` state"* — implicated by `init-product --backlog-repo` writing `backlog_service_repo`.

  **Separately:** `.prawduct/artifacts/observability-strategy.md` carries **three ratified Direction norms with plain jurisdiction over this bundle** — advisory emission (the BKL-6J2X hold), hook stdout/stderr (the new `init-product` warnings), and the in-transition emitted-id norm — yet is **absent from `governed_by:` entirely**.

  Fix: record a disposition **per norm** (`conforms` | `ruling needed` | `exception` | `amendment proposed` | `inapplicable because X`) **before Chunk 06 runs**, and add `observability-strategy.md` to `governed_by:`. Per `/prawduct:methodology norms`, an exception must file a tracking item carrying `revisit:`. (critic — rev-20260724T205224Z)

  **[PARTIALLY RESOLVED 2026-07-24 — `aaf068f`. Norm (a) is settled by owner ruling; the rest of this item STANDS. Do not close.]**

  **Owner ruling on norm (a).** The `security-model.md` § Direction norm *"No destructive action without an explicit `--apply` step; state-mutating lifecycle commands default to a dry run"* was judged **too black and white**. Owner: *"we need high level approval for destructive, but not for each granular action."* The norm is **amended, not weakened** — it now requires explicit owner approval at the **operation** level (one informed confirmation naming blast radius and irreversibility, given *before* the first write and never inferred from an adjacent request), with granular steps **inside** an approved operation proceeding without re-confirmation. Chunk 06's disposition is recorded in `build-plan-v3.2.0-golive.md`'s `governed_by:` and **conforms** on the amended text.

  **Why the old form was doubly broken — this is the reusable part, and the reason the ruling is an amendment rather than an exception:**
  1. **Unsatisfiable.** The adapter implements no generic `--apply`/dry-run contract (BKL-8V3D), so **shipped code already departed from the norm with no recorded decision**. A norm that live code silently violates is not governing anything; it is a claim.
  2. **Wrong to satisfy literally.** Gating ~900 migration writes on per-action confirmation **manufactures the rubber-stamping the norm exists to prevent** — confirmation fatigue is a safety *regression*, not a safety measure. A norm whose literal satisfaction defeats its own purpose needs its text fixed, not an exception filed against it.

  The amendment keeps preview-by-default (`--apply`) where **the command is the operation** (migrate/init/scaffold/repo-disable/learnings-audit) and drops it where one approved operation performs thousands of mutations.

  **What remains OPEN on this item — the majority of it:**
  - the **other five undispositioned `## Direction` norms** across the declared artifacts, including (b) `api-contract`'s additive-first norm and (c) `architecture`'s "writes nothing into a governed repo except its own `.prawduct/` state";
  - **`observability-strategy.md` is still absent from `governed_by:` entirely**, despite carrying three ratified Direction norms with plain jurisdiction over this bundle.

  One norm of ten is settled. The structural finding — that applicability was **assumed rather than recorded** — is untouched.

- **[BKL-4M6T]** BKL-2Q7F shipped without a regression pin — nothing prevents migration-scrub's six `--repo <target>` placeholders from drifting back to unbound, while both Chunk 03 siblings got tests
  `effort: S · impact: M · area: backlog · kind: test · source: critic · added: 2026-07-24 · reviewed: 2026-07-24 · status: open · stage: ready · related: BKL-2Q7F, BKL-8V3D, BKL-5N9W · refs: plugin/skills/backlog/migration-scrub.md (the 7 --repo <target> occurrences + Step 6's cutover scalar), tests/test_backlog_instruction_surface.py (the pin BKL-8V3D got), plugin/lib/backlog/cli.py (_repo_defaults — returns (None, None, None) on absent --repo, no git-remote fallback), plugin/lib/backlog/ids.py (parse_repo / _valid_segment — reject path-traversal segments)`

  **Filed from the 2026-07-24 cumulative Critic review (rev-20260724T205224Z-4a1c2528, warning). This is a missing-test item, NOT a defect** — the Critic verified the binding itself is correct and complete.

  **Verified sound:** all 7 `--repo` occurrences in `migration-scrub.md` carry `<target>` (the 6 pre-existing `<owner/repo>` placeholders — export/list/import/merge/status/counts — plus the new `provision` call); Step 6's cutover scalar reuses `<target>`; no step re-templates or re-derives; the runbook explicitly forbids git-remote inference. The CLI **cannot silently substitute a repo**: `_repo_defaults` returns `(None, None, None)` on an absent `--repo` with no git-remote fallback, every `GhTransport` method builds `repos/{owner}/{repo}/...` from explicit arguments, and `ids.parse_repo`/`_valid_segment` reject path-traversal segments.

  **The gap is regression protection.** This same bundle pinned the *weaker* sibling claims with tests — **BKL-8V3D** got `tests/test_backlog_instruction_surface.py` and **BKL-6J2X** got its held-roster pin — so BKL-2Q7F is the only one of the three Chunk 03 safety rails shipping unpinned. A prose-only guarantee in a file the migration rewrites is exactly the shape that silently regresses.

  Fix-shape: assert every `--repo` in `migration-scrub.md` is followed by `<target>` (no bare `<owner/repo>`, no literal repo), and that Step 6's scalar matches. Cheap, and it closes the set. (critic — rev-20260724T205224Z)

- **[CRT-2P8V]** `_GIT_REF_PREFIXES` omits `feat/`, `chore/`, `wip/`, `dependabot/` — consumer repos keep the BLOCKING `missing-ref:` false positive prawduct just fixed for itself
  `effort: S · impact: L · area: critic · kind: bug · source: critic · added: 2026-07-24 · reviewed: 2026-07-24 · status: open · stage: ready · related: BLD-6P8T, BLD-3M7K, BLD-7K3Q, BLD-4K7P · refs: plugin/lib/buildplan_refs.py:237-239 (_GIT_REF_PREFIXES — the 8-entry set), plugin/lib/buildplan_refs.py:270-272 (the git-flow-prefix skip rule), 171e630 (the fix that introduced the set), plugin/skills/critic/review-protocol.md (missing-ref: is BLOCKING)`

  **Filed from the 2026-07-24 cumulative Critic review (rev-20260724T205224Z-4a1c2528, warning).**

  `_GIT_REF_PREFIXES = {feature, fix, hotfix, release, bugfix, support, origin, upstream}` fixes the false positive **for git-flow shops and silently leaves it for everyone else.** Conventional-Commits branch naming (`feat/`, `chore/`, `refactor/`, `perf/`, `ci/`, `wip/`), bot branches (`dependabot/`, `renovate/`), and personal-prefix conventions (`brooks/foo`) all still parse as file paths.

  **The downstream cost is not cosmetic:** `verify-chunk-refs` `missing-ref:` is **BLOCKING** in the Critic protocol, so a consumer product that backticks its own branch name in a plan gets a **spurious blocking finding** — the exact pain 171e630 was written to remove for prawduct itself. The **Check-7 three-products test** makes the gap concrete: a web app on `feat/`, firmware on `wip/`, a CLI on `dependabot/` are all unhelped.

  Fix-shape: extend the set with the conventional-commit and bot prefixes (all safe under the existing extension rule — `feat/foo.py` keeps its extension and stays checked), **or** replace the enumerated set with a shape rule that does not require guessing a repo's branch conventions. BLD-3M7K (`stage: idea`) already framed the general problem and flagged that branch-like tokens are hard to distinguish from real paths; this item is the concrete, shippable follow-on to the fix that actually landed. Governance-protected (gate logic) → full Critic + PR review.
...
- **[LRN-4K8T]** learnings.md has regrown to 116KB — third compaction pass, and the size nudge has been firing every session without changing behavior
  `effort: M · impact: M · area: memory/learnings · kind: chore · source: user · added: 2026-07-29 · reviewed: 2026-07-30 · status: shipped · closed-by: learnings-compaction · stage: ready · related: MET-6W3J, LRN-9K2P, LRN-7M4D, GOV-6D4Q · refs: .prawduct/learnings.md (121KB → 34KB; 156 rules, all retained), .prawduct/learnings-detail.md (the destination; 116 → 161 entries), plugin/lib/record_lint.py (`learnings-entry-shape` — the per-entry control, `_LEARNINGS_RULE_MAX` = 400, added-lines-only), tests/test_record_lint.py (the 6 tests, incl. `test_does_not_apply_to_the_detail_file` and `test_quiet_on_the_files_own_preamble`), plugin/lib/briefing.py (the `> 40000` size nudge that had been firing unheeded), plugin/methodology/reflection.md (the When-X-do-Y-because-Z entry format), MET-6W3J (the 2026-06-10 pass: 79.5KB → 32.3KB, 48 of 58 entries)`
  ...
  **SHIPPED 2026-07-30 (`learnings-compaction`). The pass ran, and — answering this item's own flag — it did not just repeat the sweep.** `learnings.md` went **121KB → 34KB** (121,559 → 34,194 bytes; it had grown a further 5KB between filing and the pass). All **156** rules retained, **nothing deleted**: heading count is 156 before and after, and `learnings-detail.md` went 116 → 161 entries with **zero** removed. Every narrative body moved to detail under the same heading — **41 new detail entries, 11 merged into existing ones, 1 already present verbatim**. A conservation check refused to write unless every moved byte was found in the destination; it passed with **0 missing**.

  **Root cause of the regrowth — which this item suspected but did not name.** The standing rule is "keep the rule here, move the narrative to detail." Both prior sweeps enforced that on **bodies only**, so narrative migrated into the `##` **heading** — which is where the rule lives, and which no sweep ever measured. On the eve of this pass the longest "rule" was **1,921 characters**: a paragraph wearing a heading. Moving every body alone would have landed at ~37KB against the 40KB advisory — **3KB of headroom, roughly ten new entries** before firing again, which is exactly the observed history. Bounding *both* channels landed at 34KB with 6KB of headroom. That is the answer to this item's "why is a third pass owed at all."

  **The structural half — why this should be the last hand-run pass, and what makes it different from MET-6W3J and LRN-9K2P.** Both prior passes were *sweeps*: a one-time subtraction against a continuous addition. This one adds a **per-entry control**. `record_lint` gained a `learnings-entry-shape` check — an added `learnings.md` heading over **400** characters is carrying its evidence; an added narrative line belongs in detail. **Added-lines-only**, so existing entries are grandfathered and the check costs nothing until someone writes a new learning, which is the moment the guidance is actionable. Measured yield on the pre-compaction file: **20 over-long rules, 285 narrative lines** — contrast the `dangling-ref` check, built and *removed* at 0 true positives, per the norm that a control catching nothing is deleted. The threshold was sized against the corpus, not invented: median heading **181** chars, p90 **433**. Six tests, including one pinning that `learnings-detail.md` is exempt and one that the file's own preamble is not a finding.

  **Honest residue, recorded rather than smoothed.** Only **4** of the 20 over-long headings were split by hand — the 713–1,921 character ones. **The other 16 stand, grandfathered** (verified against the check's own measure: 20 over-400 before, 16 after). That is deliberate proportionality, not an omission: the check bounds *growth*, and an entry gets fixed when someone next touches it. If a future pass finds the file creeping again, those 16 are the first place to look.

  **Cross-link check, done rather than assumed — none of the four links is stale, and one is now in tension.** `MET-6W3J` is `status: shipped` (the 2026-06-10 pass) and stays as history. **`LRN-9K2P` is still `status: open`** and is *not* itself the 2026-07-17 compaction — it asks to modernize ~28 terse headings into self-contained When-X-do-Y-because-Z form, which pushes headings **longer**, directly against the new 400-char check. Whoever picks it up must reconcile the two: the rule form is still right, but "self-contained" now has a budget. `LRN-7M4D` (memory convergence, `stage: design`) and `GOV-6D4Q` (deletion-only governance simplification, `stage: ready`) are both **still open and unresolved by this work** — neither is stale, and this pass addresses neither.

- **[CRT-2W8J]** Coordinator roster threshold counts non-judgeable files — a metadata-heavy 5-file diff dispatches a full coordinator *(resolved 2026-07-30 by rejecting this item's prescription — see closing note)*
  `effort: S · impact: M · area: critic · source: critic · added: 2026-07-13 · reviewed: 2026-07-30 · status: shipped · closed-by: record-mechanization · stage: ready · related: COV-6T3P · refs: plugin/lib/critic_consolidate.py (`_derive_roster` — now risk-first; the roster config comment block above it carries the 82-review replay table; `COORDINATOR_JUDGEABLE_THRESHOLD` = 12, the volume escalator; `COORDINATOR_FILE_THRESHOLD` = 5, retained as the no-risk-signal fallback and no longer the primary rule), plugin/lib/risk.py (`paths_touch_risk_surface` — the predicate that replaced the count), plugin/lib/coverage_algebra.py (`judgeable_files` — the count this item proposed, and the conflation to avoid), plugin/lib/critic_mode.py (`_rule_final_fires` — this item's SECONDARY ask, still unaddressed; see the residue note)`
  ...
  **CLOSED 2026-07-30 (`record-mechanization` chunk 04). The diagnosis was right; the prescription was wrong — and it was wrong in a way that would have caused harm, so this is archived rather than left `ready`.** Counting non-judgeable files *is* the wrong question for roster selection. But so is counting judgeable ones. Replaying **all 82 `final`/`cumulative` review facts** in the evidence store (35 blocking findings) scored every candidate rule by the blockers it would have sent to a single reviewer: keying the roster to judgeable count demotes **19 of 35 blocking findings — 54% — at *any* threshold** (`>= 5` and `>= 12` both score 19). The reason is that this item's target slice is not what it looks like: diffs with ≥5 files and <5 judgeable are **20 reviews carrying 17 blocking findings, 13 of them pointing at CODE**. Record-heavy diffs are *governance* diffs, and governance diffs are where the blockers are — reviews touching the gate kernel yield 0.96 blocking findings each against 0.22 for everything else. The sharpest counter-example is the AST free-edge relaxation: **3 judgeable files, 5 total, and the coordinator returned 10 blocking findings**. Under this item's fix that review would have been single-pass.

  **The conflation it rested on, recorded because it will recur.** `coverage_algebra.is_judgeable_path` answers *"does this change need review **coverage**?"* — a gate question, and THE predicate there. It does not answer *"how much review **depth** does this deserve?"* Depth is a **risk** question. The item's appeal to "the same single predicate the gates already compose over" is exactly the seductive step: predicate reuse looked like consistency, but the two questions are different, and reusing one predicate for both silently imports the gate's answer into a decision it was never built to make.

  **What shipped instead.** `_derive_roster` keys on **risk-surface intersection first** (`risk.paths_touch_risk_surface`), with **`judgeable >= 12` as a secondary volume escalator** — the one size threshold the data supports (5–11 judgeable files with no risk surface touched is 13 coordinator reviews with **zero** blocking findings). Because that replay came from **this repo only** — where derived risk surfaces match 77% of reviews — a repo that declares no `risk_surfaces:` **keeps the previous 5-file rule unchanged**, since the risk predicate would never fire there and the rule would collapse to precisely the 54%-demoting row. So `COORDINATOR_FILE_THRESHOLD` still exists: this item's `refs:` were **accurate about the symbol and wrong about its role** — it is now the conservative fallback, not the primary rule.

  **Residue — one ask this closure does NOT cover.** The item's second sentence asked whether `critic_mode._rule_final_fires`' no-plan `>= 5` heuristic wants the same treatment. It does not, but **not because the replay answered it**: that replay scored *roster* selection (coordinator vs single-pass), whereas `_rule_final_fires` decides *mode* inference (whether `final` runs at all) — a different question on a different axis, and the evidence above does not transfer to it. Verified 2026-07-30: `critic_mode.py` is untouched by this work and still reads `len(diff_files) >= 5` over `_get_uncommitted_code_files`, so the item's own "verify, don't assume" caveat about that file source remains **unverified**. Flagged rather than folded in — if it is worth answering it needs its own item, and it must not inherit this one's rejected prescription. (closed by the Chunk 04 Critic review `rev-20260730T185224Z-1674abec`, finding R-3)

- **[GOV-4B9N]** `project-preferences.md` still states the destructive-action norm in its pre-amendment absolute form — two governing artifacts disagree about the norm governing the irreversible migration
  `effort: S · impact: M · area: governance · kind: bug · source: critic · added: 2026-07-28 · reviewed: 2026-07-29 · status: shipped · closed-by: critic-disposition, fix/security-model-self-contradiction · stage: ready · related: GOV-7W3D, GOV-8C3W, BKL-8V3D, BKL-6T3P, MET-6T4K · refs: .prawduct/artifacts/project-preferences.md:108 (the stale absolute row — "No destructive action without explicit `--apply`"), .prawduct/artifacts/security-model.md § Direction (the amended text, owner-ruled 2026-07-24), GOV-7W3D (where the owner ruling is recorded verbatim — "we need high level approval for destructive, but not for each granular action"), .prawduct/artifacts/build-plan-v3.2.0-golive.md `governed_by:` (Chunk 06's disposition, which conforms on the AMENDED text)`

  **Filed from the 2026-07-28 cumulative Critic review (`rev-20260728T215801Z-c74ba442`, warning) — deliberately filed rather than fixed (CRT-8N5V termination rule).**

  **This is doc-sync of an already-ratified amendment, not a re-decision.** The owner amended the norm on 2026-07-24 (ruling recorded in **GOV-7W3D**): from the absolute *"No destructive action without an explicit `--apply` step; state-mutating lifecycle commands default to a dry run"* to **operation-level approval** — one informed confirmation naming blast radius and irreversibility, given before the first write and never inferred from an adjacent request, with granular steps inside an approved operation proceeding without re-confirmation. `security-model.md` § Direction carries the amended text. **`project-preferences.md:108` still carries the old absolute form**, verified against the tree 2026-07-28.

  **Why it matters more than a stale sentence usually does.** This is the norm that most directly governs **Chunk 06, the irreversible bulk migration**. Chunk 06's recorded disposition conforms on the *amended* text; a reader who reaches `project-preferences.md` first will find a norm the adapter demonstrably does not satisfy and conclude Chunk 06 is non-conforming — or, worse, that the amendment never happened. Two governing artifacts stating different versions of one norm is exactly the condition the norm lifecycle exists to prevent: an amendment that is ratified in one home and silently absent from another is indistinguishable from erosion.

  Fix-shape: update the `project-preferences.md:108` row to the amended operation-level text and point its rationale home at `security-model.md` § Direction, so the two artifacts have one source of truth rather than two copies. While the fix is in hand, run the falsifying query rather than fixing the site — `grep -rn "without.*--apply\|default to a dry run"` over `.prawduct/` and `plugin/` — and assert it returns nothing stating the absolute form (per CRT-6R3W: sweep by the rule that generates the class). (critic — `rev-20260728T215801Z-c74ba442`)

  **[SHIPPED 2026-07-29 — both clauses verified done. `closed-by: critic-disposition, fix/security-model-self-contradiction`.]**

  **Clause 1 (the filed defect) — done, and not by this branch.** `project-preferences.md:108` now reads *"Destructive/irreversible operations need explicit owner approval at the OPERATION level (not per action); preview-by-default where the command is the operation"* with its rationale home pointed at `security-model.md`. Authoring commit traced to **`70ca632`** (*"fix(critic): the disposition change's own record was wrong in the file adopters read"*, change-log `scope=critic-disposition`); the row's rewrite is visible in that commit's diff, so the primary `closed-by` handle is that scope rather than a plausible guess (verified 2026-07-29).

  **Clause 2 (the falsifying query) — done on `fix/security-model-self-contradiction`, and it found a live site the first archive attempt missed.** The query's whole value showed up here: `security-model.md` was **contradicting itself**. Line 37 records the amendment away from the absolute form, while **lines 117–119, eighty lines below in the same file**, still stated that absolute form (*"No destructive action without an explicit apply step … default to a dry run and require an explicit `--apply`"*) as a **live control** in the § Abuse Prevention list — unchanged since `d9cd518` and present at v3.2.0. Worse, the clause-1 row points its rationale home *at that file*, so a reader following the pointer landed in the contradiction. The § Abuse Prevention control is rewritten to track the amended § Direction norm (operation-level approval; one informed confirmation naming blast radius and irreversibility; preview-by-default only where the command *is* the operation), with an explicit note that the pre-amendment absolute form is recorded at line 37 as history and must not be restated there as live.

  **State of the falsifying query, stated precisely.** `grep -rn "without.*--apply\|default to a dry run"` over `.prawduct/` and `plugin/` returns **~22 hits across 7 files, not 5** — and every one is a *record*, not a live assertion: `security-model.md:37` (the amendment record itself), `change-log.md:1079` (names it "the pre-amendment form") and `:3657` (historical norm-name list), `build-plan-v3.2.0-golive.md:13` (disposes against the AMENDED text), `build-plan-backlog-skill-repoint.md:23` and `build-plan-skills-cutover-awareness.md:30` (historical dispositions citing the then-live norm), `backlog.md:307/311/315` (this item's own body), `:555`/`:705`/`:3486`/`:3496` (other items quoting the then-live norm, including the PARTIALLY-RESOLVED item that carries the owner ruling), `.governance-ledger.jsonl:162/218/219` (append-only Critic review facts), plus two pure regex artifacts — `cross-cutting-concerns.md:31` and `change-log.md:7081`, where the matched `--apply` belongs to `coverage-scaffold` / `learnings-audit` and not to this norm. A broader class sweep (`dry.?run by default|requires? an explicit .--apply|per-action (gate|confirm)|explicit --apply step` over `.prawduct/artifacts/` and `plugin/`) adds only command-level previews the amended norm **explicitly endorses** (`coverage-scaffold`, `init_product.py:28`). **Zero live assertions of the absolute per-action form remain.**

  **Process lesson — this item survived one archive attempt, and the second attempt repeated the defect in miniature.** A peer correctly **refused** to archive it an hour earlier: the disposition it was handed asserted the falsifying query "returns only historical sites" and enumerated `security-model.md:37`, an enumeration that was **wrong** because it missed the live `:117-119` in the same file. That was caught only by *executing* the query. The disposition supplied for *this* attempt then made a weaker version of the same error — it claimed the re-run "returns only [5] sites," when the query in fact returns ~22 across 7 files. The substance was right and the fix was real, but the enumeration was again not the query's output. **The rule this item pays for: a supplied falsifying query is not evidence until the archiving agent runs it — a disposition asserting a state never checked against the artifact is the same defect class whether it is off by one live site or off by seventeen records.** Cf. CRT-6R3W (sweep by the rule that generates the class).

  **Note on the handle.** `closed-by:` carries **two** refs because the item closed in two places: `critic-disposition` (clause 1, `70ca632`) and `fix/security-model-self-contradiction` (clause 2's doc-sync). Recording only the requested `critic-disposition` would have left clause 2's landing site untraceable — a reader following that scope finds the preferences row but never the § Abuse Prevention repair.
...
- **[CRT-6Q2N]** critic-consolidate's ledger anchor lacks an idempotency guard — retry in the ledger-append→remove_partials crash window lands a duplicate `review.critic` line
  `effort: S · impact: S · area: governance/critic · source: critic · added: 2026-07-13 · reviewed: 2026-07-29 · status: shipped · closed-by: record-mechanization · stage: ready · related: CRT-4B7X, CRT-8L3Q, CRT-9T6M · refs: .prawduct/artifacts/build-plan-kernel-evidence-store.md, .prawduct/.governance-ledger.jsonl, plugin/lib/ledger.py (`review_event_exists` — the dispatch-id-keyed guard this item asked for), tests/test_governance_ledger.py (`TestReviewAnchorIdempotency`), tests/test_critic_consolidate.py (`test_retry_after_success_appends_no_second_fact`)`
  ...
  **Status 2026-07-29 — the guard shipped; this item's scope is now stale, and archiving it is a
  triage call.** `ledger.review_event_exists` is the dispatch-id-keyed guard this item asked for.
  It closes the crash-window/re-materialized-manifest replay path named above *completely* — i.e.
  the whole of what this item describes. Deliberately left `open` rather than archived because
  D4 makes "shipped" an explicit call, not one inferred by a filing agent. The narrower residual
  that survives the guard — the **unlocked** overlap window, a different fix (mutual exclusion,
  not idempotency) — is carried by **CRT-8L3Q**. Next triage should either archive this as
  `shipped closed-by: <the v3.2.1 scope>` or explicitly narrow its title to whatever it still
  claims; it should not stay open as-titled, since the guard now exists.

  **Archived shipped 2026-07-29 — `closed-by: record-mechanization`.** The triage call the status
  note above asked for, made by reading the item rather than inferring it from the diff. The scope
  match is exact: the defect this item describes is the *replay* path (a retry after the crash
  window between ledger-append and `remove_partials`, or a re-materialized same-id manifest), and
  `ledger.review_event_exists` is the dispatch-id-keyed guard it asked for — the probe reads the
  ledger for an existing event at the review id before appending, so a retry or a re-materialized
  same-id manifest finds the prior anchor and skips. Pinned by
  `tests/test_governance_ledger.py::TestReviewAnchorIdempotency` plus the integration assertion
  `tests/test_critic_consolidate.py::test_retry_after_success_appends_no_second_fact`
  (mutation-proved: stubbing the probe fails that test and only that test).

  **What this archive does NOT claim — the sibling prediction is corrected, not inherited.** The
  original body predicted that "a dispatch-id-keyed guard likely fixes both" this item and
  CRT-4B7X. That is **half right, and the other half is still open.** The *concurrent/overlap*
  half is **not** closed: the probe is read-then-write with no lock, so two consolidations that
  get past the manifest-existence check can both probe before either appends. What shipped
  **narrowed** that window — from the whole consolidate body down to the probe→append window —
  it did not close it, and the same release made overlap *more* reachable by mandating concurrent
  coordinator dispatch. That residual is carried by **CRT-8L3Q** (already cross-linked, and its
  `revisit:` trigger is a *second* duplicated `review.fact_id` appearing). So: this item ships on
  its own scope; the duplicate-ledger-line symptom via concurrent firings does not ship with it.

- **[REL-8P6M]** The release runbook can publish work that is not fit to ship — Phase 1 has no releasability gate (it asks only whether anything is unreleased), and Phase 2 has no pruned-release path for the promotion shape prawduct has now used twice
  `effort: L · impact: L · area: release-tooling · kind: improvement · source: user · added: 2026-07-27 · reviewed: 2026-07-29 · status: shipped · closed-by: release-readiness · stage: ready · related: REL-3K9P, REL-7D4X, REL-5W2J, BKL-6J2X, BKL-5N9W, BKL-2Q7F, BKL-8V3D · refs: .prawduct/runbooks/cut-and-publish-a-plugin-release.md ("When to use this" — the `git diff --stat origin/main origin/develop` non-empty test, and "When NOT to use this"; Phase 2 — step 14 `git checkout main`, step 15 `git read-tree --reset -u origin/develop`, step 17 the content-identical check, step 19 publish, step 20 tag; Phase 1 step 3 the change-log tag sweep; "Done when" bullet asserting `git diff --stat origin/main origin/develop` prints nothing; `last_verified: null`), .prawduct/artifacts/release-plan-v3.1.2-pruned.md (where the mechanics currently live), .prawduct/artifacts/release-plan-v3.1.1-hotfix.md, .prawduct/artifacts/release-plan-backlog-service-golive.md (the four blockers' ship list), .prawduct/change-log.md (the incidental catch — the ten backlog-service entries), .prawduct/artifacts/build-plan-release-readiness.md (the plan that closed a–d and f), .prawduct/runbooks/promote-a-pruned-release.md (parts a–d, shipped), plugin/lib/release_readiness.py + tests/test_release_readiness.py (part f, shipped), .prawduct/artifacts/operational-spec.md (§ Direction — the promotion norm, amended 2026-07-29), documentation/release-process.md (the amendment’s second procedure), .prawduct/artifacts/change-log-ledger-design.md (why (e) is held)`

  **The gap.** `cut-and-publish-a-plugin-release.md` Phase 2 promotes by making `main` a copy of `develop`: step 15 is `git read-tree --reset -u origin/develop`, and the phase's "Done when" asserts `git diff --stat origin/main origin/develop` prints nothing. Both statements are only true for a **whole-develop** promotion. A **pruned** promotion — where only a subset of `develop`'s work ships — leaves `main` deliberately *not* content-identical to `develop`, so the prescribed step produces the wrong tree and the completion check can never pass. prawduct has now done a pruned promotion **twice** (v3.1.1 and v3.1.2), which makes it the pattern, not the exception; the runbook still documents only the case that hasn't been exercised.

  **Why the release plan isn't the fix.** v3.1.2's mechanics were worked out ad hoc and written into `.prawduct/artifacts/release-plan-v3.1.2-pruned.md`. A release plan is an **expiring artifact** — it describes one release and is dead the day it ships. The runbook is the durable surface, so the knowledge has to move there or it is re-derived from scratch next time (the same failure mode REL-5W2J records for `docs/release-process.md`).

  **Fix-shape — a Phase 2-alternate ("pruned promotion"), plus a Phase 1 releasability gate — covering:**
  (a) **Build the candidate** as `<prev-tag>` plus `git diff <cut-point>..develop` applied with `--3way`, rather than resetting the tree to `develop`.
  (b) **Publish by ref** — `git push origin <sha>:refs/heads/main` instead of `git checkout main` (steps 14/19). Safer in general, and *necessary* when `main` is checked out in a sibling worktree, where `git checkout main` cannot run at all.
  (c) **A partition check that replaces step 17.** The content-identical assertion is meaningless here; the correct check is that the shipping and withheld sets partition the diff — every path in `origin/main..origin/develop` is accounted for as either shipped or deliberately withheld, with nothing unclassified.
  (d) **A mandatory suite-on-the-candidate-tree plus a per-file import diff.** On v3.1.2 a *clean* `git apply` produced a **broken tree** — so "the patch applied without conflict" is not evidence of a correct candidate. The runbook must require running the suite against the candidate tree and diffing each imported file before publishing.
  (e) **Phase 1 step 3 must tag only the shipping subset.** The change-log sweep appends `release=vX.Y.Z | status=shipped`; under a pruned release, tagging everything above the boundary marks withheld work as shipped. (Adjacent to REL-7D4X, which fixes the *positional* nature of that same sweep — resolve them together, since both rewrite step 3's selection rule.)

  (f) **Phase 1 needs a releasability gate — and this is the highest-value half of the item, not an addendum to (a)–(e).** The runbook's "When to use this" check is `git diff --stat origin/main origin/develop`: non-empty means unreleased content exists, so proceed. That question is *"is there anything to ship?"* — never *"is everything on `develop` **fit** to ship?"*. On v3.1.2 those two answers diverged: the check passed, and following Phase 2 literally would have published the **backlog-service subsystem with all four of its go-live blockers still open** — BKL-6J2X (a `warn` advisory that fires in every un-migrated repo and routes the whole fleet to `/prawduct:backlog scrub`), BKL-5N9W (a wildcard `Bash(prawduct-hook backlog *)` grant on a model-invocable skill), BKL-2Q7F (a scrub runbook that never binds `--repo`), BKL-8V3D (an `--apply`/dry-run safety contract that does not exist anywhere in `plugin/lib/backlog/`). That is an **unrecallable publish**: composed, those four let a scrub write 100–250 real issues into a real repo while the agent running it believes a dry-run guarded it, and GitHub has no ordinary issue-delete.

  **Root cause.** This repo merges release-gated work to `develop` and gates it by *withholding at release time* rather than by branch — so the releasability question lives **nowhere**: not in the runbook, not in the branch topology, only inside a release plan for a different release. What actually caught it on v3.1.2 was **incidental** (enumerating change-log entries happened to surface ten from an unexpected subsystem), not any step of the procedure. That is why this outranks (a)–(e): (a)–(e) make a pruned promotion *mechanically correct*; (f) is what decides whether the promotion should happen at all, and its absence is the more dangerous half.

  **Shape.** The gate must require naming, **per unreleased scope**, either the release it ships in **or** the open blocker withholding it — nothing unclassified. A `release-gated:` marker on the backlog item would make that mechanical rather than a judgment call (note the shape rhyme with (c)'s partition check: same "every element accounted for as shipped or deliberately withheld" discipline, applied to *scopes at Phase 1* rather than *paths at Phase 2*). Also add to **"When NOT to use this"**: do not run this runbook while any release-gated work sits unmerged-but-on-`develop` without a recorded withholding decision.

  Note the runbook is still `last_verified: null`, so this is a good occasion to verify it end-to-end against whichever promotion shape the next release uses. (user)

  **Disposition 2026-07-29 — (a)(b)(c)(d)(f) landed; (e) explicitly retained, NOT shipped.** Built under `.prawduct/artifacts/build-plan-release-readiness.md` (scope `release-readiness`), whose title and Out-of-scope section both name the item as *parts a–d and f*.

  - **(f), the highest-value half — landed.** `prawduct-hook check-releasability [--release vX.Y.Z]` (`plugin/lib/release_readiness.py`, `tests/test_release_readiness.py`) plus a new **Phase 0 — Releasability** ahead of Phase 1 in `cut-and-publish-a-plugin-release.md`. Every release-pending scope (a tagged change-log entry carrying no `release=`) must appear exactly once, classified `ships` or `withheld: <ITEM-ID>` naming an **open** backlog item; exit 1 names every unclassified scope, every withholding whose blocker has since closed (a stale decision, not a valid one), and every table entry with nothing behind it. The `release-gated:` **backlog marker this item proposed was not taken** — the classification table lives in `.prawduct/artifacts/release-plan-<version>.md`, concentrating the decision in one reviewable table rather than spreading it across N items (build plan, open assumption 1; the owner can still correct this). **The proposed "When NOT to use this" prohibition was NOT added — Phase 0's gate supersedes it, and that is a descope, not an omission.** The item asked for prose saying *do not run this runbook while any release-gated work sits unmerged-but-on-`develop` without a recorded withholding decision.* A fail-closed gate at step 0 enforces exactly that and cannot be skimmed past, so the bullet would be a weaker duplicate. Recorded here rather than claimed done: the first version of this disposition asserted the bullet was in the runbook, and it never was — caught by the Critic (`R-1`, blocking) against the diff. Same defect class as commit `fc9b61f`, and the reason this bundle's own new rule is *verify a disposition against the diff before recording it.*
  - **(a)(b)(c)(d) — landed as `.prawduct/runbooks/promote-a-pruned-release.md`**, a second runbook rather than the Phase 2-alternate *section* this item imagined: `plugin/docs/runbook-authoring.md` budgets ≤20 steps and the release runbook already stood at 21, so an inline alternate would have shipped a 30-step document a third of whose steps never apply to the reader executing it. Whole-develop Phase 2 is untouched and still correct; Phase 0's output routes between the two shapes, and each gets its own honest `Done when`. Blocked mid-chunk on a **ratified norm**: `operational-spec.md` § Direction bound promotion to a content-identical tree-set, which (a)/(b)/(c) replace. Raised as a blocking precondition rather than edited quietly, and **amended by owner ruling 2026-07-29** — the binding property is now the **partition** (every path shipped or withheld, nothing unaccounted), with content-identity as its special case when nothing is withheld. The amendment cascaded to `documentation/release-process.md`, the second active procedure.
  - **(e) — HELD, and carried by REL-7D4X.** Owner decision 2026-07-29: `.prawduct/artifacts/change-log-ledger-design.md` proposes deleting the tag-sweep machinery outright, so rewriting its selection rule now is throwaway work (the design doc lists REL-7D4X, REL-8P6M(e) and W-1 as "closed outright — the mechanism is gone, not fixed"). **Trigger: the change-log-ledger decision — adopt or reject.** Until it is taken, the sweep tags its shipping subset **by hand, once per release, across every scope**, and that stopgap is written into Phase 1 step 2 as a 🚧 note naming this item, the hold, and the design doc — so the next reader does not "fix" the rule in ignorance of the pending decision. **REL-7D4X stays open carrying (e) with its `revisit:` trigger; archiving this item does not drop it.**
  - **Folded in en route: W-1** (PR review, `feature/v3.2.0-c02-adapter-safety`) — the sweep is *scope-narrowed* as well as positional. Measured on `feature/rel-8p6m-releasability-gate` @ `1a353d1`, above the `release=v3.1.2` boundary: 23 release-pending entries across six scopes, of which `v3.2.0-golive` is 7, so `grep scope=v3.2.0-golive` misses **16 across five other scopes**; one is the fleet-visible `protected_path_violation` widening that step 10's consumer-facing headline is derived from. Documented in the same step-2 warning, which carries the derivation command rather than a fixed number; the underlying rule stays with (e). *(Figure corrected twice — see REL-7D4X.)*
  - **Not verified end-to-end.** `last_verified:` stays `null` on both runbooks — this work wrote the procedures, it did not exercise them. ~~The v3.2.0 promotion is what verifies them, and this item's own closing note (verify against whichever shape the next release uses) still stands for that run.~~

    **↳ Corrected 2026-07-29 (delta review of `c68443d`) — struck, not rewritten: the false claim is the record.** Naming **v3.2.0** as the verifier is the wrong *shape* of trigger, and that is precisely the defect **REL-3K9P** was filed to fix. Whether `promote-a-pruned-release.md` opens at all is decided by the next promotion's **shape**, not its version number: Phase 0 routes a whole-develop promotion down Phase 2 (which never opens the pruned runbook) and a pruned one into it. A version therefore cannot verify this runbook — it can only *turn out to have* the right shape. The sound trigger is **the next pruned promotion, whenever one occurs**, which cannot be scheduled and so needs an *open* tracker. Note the second half of the same defect: this item is archived `status: shipped`, and an archived item tracks nothing — so the correction could not live here even if the trigger had been right. **Tracking now lives in REL-3K9P (open).** Nothing above this line is retracted: both runbooks did ship `last_verified: null`, correctly, because this work wrote the procedures rather than exercising them. (owner — build-plan-release-readiness Chunk 03)
...
- **[GOV-5J9Q]** BKL-5N9W's wildcard-grant narrowing is incomplete — `janitor`, `pr`, and `runbook` SKILL.md still carry `Bash(prawduct-hook backlog *)` while delegating to `/prawduct:backlog` anyway
  ...
  **Filed from the 2026-07-24 cumulative Critic review (rev-20260724T205224Z-4a1c2528, warning) — a concept-ripple miss on BKL-5N9W.**

  `plugin/skills/janitor/SKILL.md:5`, `plugin/skills/pr/SKILL.md:6`, and `plugin/skills/runbook/SKILL.md:6` still carry `Bash(prawduct-hook backlog *)` **and** the `python3 plugin/bin/...` form — i.e. **no-prompt grants over exactly the `import`/`merge`/`provision`/`reconcile-labels` set this bundle removed from the backlog skill**, in a model-invocable maintenance skill among others.

  **All three drive the backlog by delegating to `/prawduct:backlog`** (janitor:176/289, pr:68/79 all say "through the skill, not the file"), so **on the evidence they need no direct adapter grant at all** — this is a removal, not a re-narrowing, unless a caller is found.

  **Framework-wide coherence cost:** one skill's new prose says these ops must prompt while three grant them silently, which makes BKL-5N9W's security narrative — and Framework Check 8's no-contradiction-across-skills rule — inconsistent. Apply the **dual-form rule** (JNT-4R2M): narrowing only the bare form leaves the self-hosted path granted. Note CRT-9V4T's caveat that a skill `allowed-tools` list may be a no-prompt allow-list rather than a hard cap, so this is defense-in-depth, not the primary guard. Governance-protected (`skills/`) → full Critic + PR review. (critic — rev-20260724T205224Z)

  **[STATUS POINTER 2026-07-28 — SHIPPED in `85753fd` (`fix(skills): an allow-list cannot fence an op reachable through an interpreter`), scope `v3.2.0-golive`.]** All six dead grants were **deleted, not re-narrowed** — the item's own reading ("this is a removal, not a re-narrowing, unless a caller is found") held: no caller was found, and the 13-op list was not copied into three more files. `pr` and `runbook` are now clean by deletion alone.

  **Fix-shape divergence, recorded so the delivered design is not mistaken for a partial one.** For `janitor`, deletion alone was **not sufficient** and the item did not anticipate why: janitor legitimately needs a broad interpreter grant (`Bash(python3 *)`, `Bash(npm *)`) to drive whatever toolchain the product uses, and that grant *semantically subsumes* `python3 plugin/bin/prawduct-hook backlog …` — so **an allow-list cannot fence an op reachable through an interpreter**. The real exposure was also broader than the wildcard: janitor was the only skill never declaring `disable-model-invocation`, and the field is **opt-out**, so it was model-invocable *by omission*. The delivered fix closes the model-initiated path (`disable-model-invocation: true`) rather than trusting the allow-list. This generalizes the item's own CRT-9V4T caveat — that an `allowed-tools` list is a no-prompt allow-list rather than a hard cap — from a footnote into the governing constraint.

  (Prose left intact per BKL-8Q3D — this pointer supersedes it rather than rewriting it.)
...
- **[CRT-2J8N]** The `SubagentStop` matcher is `critic-reviewer` but the dispatched agent type is the plugin-scoped `prawduct:critic-reviewer` — the Critic consolidation trigger never fires, on every product running v3.1.1
  `effort: S · impact: L · area: critic · kind: bug · source: critic · added: 2026-07-27 · reviewed: 2026-07-27 · status: shipped · stage: ready · closed-by: subagent-stop-matcher · related: CRT-3F7M, CRT-8Q6R, ENV-7C4K, GOV-2W7Q, CRT-6B9M · refs: plugin/hooks/hooks.json (the SubagentStop block — sole matcher `"critic-reviewer"`), plugin/bin/prawduct-hook (`cmd_subagent_stop` — the `.endswith("critic-reviewer")` defense-in-depth check that never got reached; **anchored by symbol, not line**: the original `:1193-1229` range went stale within one commit of being written, which is the staleness CRT-6B9M's refs cite as this repo's reason for preferring symbols), plugin/agents/critic-reviewer.md, plugin/skills/critic/SKILL.md:57 (dispatch-and-STOP), CLAUDE.md § The Critic ("run `critic-consolidate` before reading `.critic-findings.json` — an idempotent no-op when the trigger already landed it")`
  ...
  **=== RESOLVED 2026-07-27 (static half — see the closure scope below) — root cause established from the Claude Code hooks documentation, fixed, and pinned. ===**

  **The two documented rules that settle it — both asserted for `SubagentStop` only.** (1) **For `SubagentStop`**, a matcher containing only letters, digits, `_`, `-`, spaces, `,` and `|` is evaluated as a **literal** — a single exact string, or a `|`/`,`-separated **list** of exact strings. Only a matcher containing some *other* character takes the JavaScript-regex path, which is tested unanchored. (2) For a subagent shipped by a plugin, the runtime `agent_type` is the **plugin-scoped identifier** (`prawduct:critic-reviewer`), explicitly *not* the bare frontmatter name. So `"critic-reviewer"` was exact-compared against `"prawduct:critic-reviewer"` and could never match. **The hook never fired, on any install, since it shipped.**

  **Which literal class an event uses is NOT uniform across hook events, so rule (1) is not a global one** — the `startup|resume|clear|compact|fork` `SessionStart` matchers in the same `hooks.json` are lists of exact strings and were always correct. Source for both rules: https://code.claude.com/docs/en/hooks (verified 2026-07-27), corroborated during review against the installed Claude Code 2.1.220 matcher implementation.

  **This SUPERSEDES the `stage: research` paragraph above.** That paragraph kept two hypotheses live and named the substring reading as the one that would force re-derivation. The documentation rules it out: a bare alphanumeric matcher never reaches the regex path at all, so no substring match was ever possible. The `SessionStart` alternation (`startup|resume|clear|compact|fork`) is not a counter-example — it is a `|`-separated list of *exact* strings under rule (1), which is exactly why it works.

  **The fix:** `(^|:)critic-reviewer$`. The parentheses and `^`/`$` push it onto the regex path, and the alternation matches both the bare and any plugin-scoped form while refusing near-misses — it does not match `foo-critic-reviewer` or `prawduct:pr-reviewer`. **Pinned** by `TestSubagentStopMatcherMatchesRuntimeAgentType` in `tests/test_critic_reviewer_agent.py`, whose `_matcher_matches` helper encodes the documented two-path rule; `test_bare_name_matcher_cannot_match_a_scoped_value` asserts the mechanism directly, so a future "simplification" back to the bare name breaks loudly rather than silently.

  **The sharpening about the `SessionStart` registrations (in CRT-7N4B) is unaffected** — that concerned which argv the hook issues, not which matcher fires.

  **CRT-3F7M's sequencing note is now DISCHARGED.** That item's repro was blocked behind "fix or characterise this first, otherwise the repro measures this bug." The confound is removed: **CRT-3F7M's `run_in_background: false` repro can now be run against a matcher that actually fires**, and this is very likely the mechanism behind its symptom class — reviewers complete, nothing consolidates, the caller sees "the Critic ran and nothing came back." It still does not settle the dispatch-lifetime question (a genuinely dead reviewer writes no partial either way), but that question can now be asked cleanly.

  **CLOSURE SCOPE — `status: shipped` covers the STATIC half only.** The filed case set the acceptance condition "**confirm the semantics against the harness** before writing the fix." What was confirmed is *matchability*: that a bare alphanumeric matcher cannot match a scoped `agent_type`, established from the documentation and from the installed Claude Code 2.1.220 matcher implementation, and pinned by a test. What is **not** confirmed is *delivery* — that the harness actually emits `SubagentStop` for a dispatched plugin subagent at all, which is VRF-002's remaining half and which **CRT-4M2P establishes cannot return a meaningful verdict** until a trigger field lands on the `review.critic` event. The item is closed rather than held open because the fix is strictly correct under either delivery answer, the pin prevents regression, and the defect it removes is unambiguous; the residual risk is that the trigger stays dead for a *second, independent* reason.

  **The one observation that would falsify this closure:** a `final`/`cumulative` coordinator review where all roster reviewers report and, with the skill's own explicit `critic-consolidate` call suppressed and `CLAUDE_CONFIG_DIR` pointed at an empty dir, `.critic-findings.json` still does not update. That is exactly VRF-002's remaining step. If it happens, reopen here — do not file a new item.

  **The filed case's third instruction is DISCHARGED (2026-07-27):** "re-check the same wiring for any other matcher naming a plugin-scoped agent." Enumerated every matcher in `plugin/hooks/hooks.json` — the `SubagentStop` block is the **only** one that names an agent; the five `SessionStart` matchers are source lists (`startup|resume|clear|compact|fork`, `startup|clear`, `resume|compact|fork`) and `UserPromptSubmit`/`Stop` are empty. So the defect had exactly one instance and there is no second site to fix.

  **How it escaped is worth more than the fix, and is filed separately** — `.prawduct/operator-verification.md` **VRF-002** named this exact suspect on 2026-07-10, seventeen days before it was found, and was never run. See **GOV-2W7Q** (the write-only verification queue) and **CRT-6B9M** (the "a guard behind a gate that never opens is not a guard" review heuristic). Change-log entry: *"The Critic's SubagentStop trigger had never fired (CRT-2J8N)"*, `type=fix`. (critic — cumulative `rev-20260727T164842Z-1b70981d`; resolved 2026-07-27)
