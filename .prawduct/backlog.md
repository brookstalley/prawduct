...
- **[COV-3R9K]** test-evidence record forces a redundant full-suite re-run after a no-op commit — F4a `changes_referenced` changed-file scope shifts at commit time
  `effort: M · impact: M · area: coverage · source: user · added: 2026-06-26 · status: open · stage: requirements · related: TST-4K2P, TST-7M3K, COV-8R2K · refs: bin/prawduct-hook (cmd_test_evidence), lib/coverage.py (changed-files derivation), bin/test-reference-verify, .prawduct/.test-evidence.json, methodology/building.md (Verify), incoming-bugs/test-evidence-redundant-rerun-after-noop-commit.md`

  **TRIAGE / DEDUP (2026-06-26) — reconcile against the SHIPPED v2.1.8 test-evidence bundle before any build; this is a residual / incomplete-fix follow-up, NOT a fresh defect.** The report (scriob, prawduct 2.2.1, i.e. POST-v2.1.8) proposes four fixes, but two premises are already resolved in shipped code and one suggestion was *deliberately rejected*:
  - Suggested fix #1 (validate by source/tree-content hash) — **rejected by design in TST-4K2P (shipped v2.1.8, #104, commit 1d8a7d1).** That bundle RETIRED `git_sha` entirely: freshness identity is now the `timestamp` alone (compared to `.session-start`), and `cmd_test_evidence` explicitly states "Content-hash freshness is deliberately NOT reintroduced … removed pre-v1.4 for chronic false positives." Freshness alone therefore no longer forces a post-commit re-record; do not reintroduce a content hash without confronting that history.
  - Suggested fix #3 (`--from-junit`) — **already shipped (TST-7M3K, v2.1.8).** `test-evidence record --from-junit <report>` ingests a JUnit XML the builder already produced instead of re-running the suite. If the operator's pain is purely the duplicate TEST run, the immediate answer is methodology/discoverability — note that the cited learning "evidence SHA must equal HEAD" is itself STALE post-TST-4K2P (no git_sha is stamped anymore), and point the "record AFTER the final commit" guidance at `--from-junit` — not new code.
  - The GENUINE uncovered residual: the F4a `changes_referenced` half — the changed-file SET derived in `lib/coverage.py` from `git diff <base>...HEAD` (+ untracked) and consumed by `bin/test-reference-verify` — shifts membership the moment you commit (a modified file moves from "working-tree vs HEAD" into "in HEAD vs base"), so the post-commit gate recomputes a differently-scoped set than the pre-commit recording. THAT is what still forces a re-record of the COVERAGE half, distinct from the git_sha / double-execution legs the bundle fixed. Shares the lib/coverage.py changed-files surface with the open COV-8R2K. Suggested fixes #2 (`--restamp`/`--no-rerun`) and #4 (short-circuit on tree-hash equality) are new, unshipped mechanisms — scope the buildable delta here.

  **Stage override (caller said ready → set requirements):** NOT buildable as-stated — its two obvious fixes either already ship (#3) or collide with a deliberate prior decision (#1). A requirements/design pass is needed to scope the real residual (changed-file-set position-dependence in the F4a path) against shipped work and pick a mechanism that doesn't reintroduce TST-4K2P's false positives. Route to /prawduct:discovery to advance the stage. Governance-protected (bin/, lib/) → full Critic + PR review.
