...
- **[BKL-5N9W]** `skills/backlog/SKILL.md` pairs `disable-model-invocation: false` with a wildcard `Bash(prawduct-hook backlog *)` grant — narrow it to the ops the skill drives
  ...
  **[STATUS POINTER 2026-07-24 — the body above reads as unfixed; it is BUILT.]** The work is built on `feature/v3.2.0-c02-adapter-safety` (Chunk 03); the `status=shipped` flip is deferred to **Chunk 09** per this release's `[ ]`-until-release convention, so the item correctly stays `open`. The metadata test shipped, in both invocation forms. **Two follow-ons from the 2026-07-24 cumulative Critic — one closed, one open:**
  ...
  - **OPEN — GOV-5J9Q** — the narrowing did not ripple to `janitor`/`pr`/`runbook` SKILL.md, which still carry the wildcard in both invocation forms.
