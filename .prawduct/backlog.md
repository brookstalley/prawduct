...
- **[CRT-2X7R]** A superseded review fact's findings are permanently undispositionable — `verify-resolutions` anchors only to the derived cache's single `fact_id`, which always names the newest fact
  ...
  **Observed live 2026-07-30**, as consequence 2 of the CRT-9T6M partials collision — but the gap is not specific to that cause; any second review fact on the same chunk produces it (a re-dispatch after `critic-end`, a waived-then-redone review). Chunk 03 of `build-plan-record-mechanization.md` carries `rev-20260730T173411Z-c8cb7f13` (1 warning + 1 note) and, two minutes later, `rev-20260730T174026Z-c815625e`; the cache now points at a *third*, `rev-20260730T174806Z-2d21b473`. c8cb7f13's `R-1` warning and `R-2` note were both **fixed** (in `889f902` and `ad3c893`) and neither can be dispositioned by any verify pass.

  **Mechanism — verified by reading the code and the live store.** `_prior_review_fact` is the only way a `verify-resolutions` dispatch locates its anchor: it reads `.prawduct/.critic-findings.json`, takes the single `fact_id` key, and scans the evidence store for that one fact. `fact_to_cache_record` sets that key to the id of the fact **just consolidated**, so the pointer always names the newest review and every earlier fact for the same chunk goes unaddressable the moment a successor lands. Confirmed in the store: no `disposition` fact carries review id `rev-20260730T173411Z-c8cb7f13`, while ten other reviews have theirs.
