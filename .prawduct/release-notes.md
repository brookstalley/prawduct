...
>
> It was a derived view, regenerated from `.prawduct/change-log.md`'s `release=`
> tags. Derived views are retired: the change log is the only record of what
> shipped in which release, and it always was — this was a digest of it.
>
> Everything below is preserved as it stood at v3.2.7. It is **not** updated for
> later releases, so a release missing here has not necessarily gone unreleased —
> look it up in `change-log.md`. Hand-editing is pointless rather than dangerous
> now; nothing will overwrite it, and nothing will read it either.
