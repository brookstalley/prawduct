...
## RULING (harness-only-removal-is-not-a-major), 2026-08-11 — owner decision, taken in session on a direct question, not inferred from an earlier one. `api-contract.md` § Direction defers subcommand removal to a major, "never silent"; v3.3.2 removed `build-index` and `user-prompt-submit` in a patch. The clause's why protects CALLERS across versions, and these two had exactly one caller — `hooks.json`, shipped in the same plugin at the same version and updated in the same commit — so no caller could observe a gap; the same artifact already classes them harness-invoked and already records that no supported external consumer of the subcommand surface exists. **Scope: harness-only subcommands.** Deprecate-then-remove still governs everything a human or a skill can call, and `stamp-merged`/`regen-views` stay inert-and-deferred as the contrast that keeps this narrow. Recorded as a `Rulings:` line on the entry rather than as an edit to its Statement or Why — editing a norm to permit your own change is the amend tell — [learnings-detail.md]

## Sharing a MATCHER between two modules shares syntax, not the DEFINITION — the reader that feeds it is the other half, so ask what each module passes IN before trusting "one home". `record_lint` imports `norm_probes._FIELD_MARKER_RE` precisely so one definition of a norm entry cannot drift; adding blockquote tolerance to `_direction_lines` alone left the regex byte-identical and made the two answer differently on identical input — a quoted registry became N entries to the probes and 0 to the `governed_by` lint, which then skipped exactly the products the fix targeted. The failure is worse than two copies, because the shared symbol *reads* as proof they agree and the docstring said they "cannot drift apart on an edit". I checked the sibling deliberately, saw the import, and drew the wrong conclusion — so the tell is not "did I check consumers" but "does the shared thing include the INPUT". Corollary: the fix must move the strip into the shared handle (`_norm_field_re` now returns marker AND prefix), never into both call sites — [learnings-detail.md]
