...
## A cross-reference claiming two surfaces AGREE is a claim about the OTHER one — open it and read its actual condition before writing the sentence, because you generate the claim from the surface you are editing, where it reads as true. Pairing a doctor check to its advisory, "both resolve on the same recorded fact" was written into the check while editing the check: it is satisfied by EITHER the decision block or the flat answer-store fact, while the probe can only see the flat fact — so a product that fills the block and skips the one-line mirror gets a healthy check plus a nudge that never stops, exactly the state the new sentence told its owner was impossible. The precedent it was modelled on says only "both resolve on a recorded decision", and that looseness was load-bearing; tightening it is what manufactured the falsehood. Fix shape: name the asymmetry as a reported state rather than softening the claim back. Sibling of [[When you TRANSCRIBE a rule between two records]] — that one degrades identifiers, this one invents agreement

## A policy spanning many ecosystems must key its DETECTION off the decision being recorded, never off enumerating the ecosystems — any file-list deciding "does this rule apply here" IS the allowlist such a policy exists to reject, and under-enumerates by construction. Ask only "has an answer been recorded", which is universal and needs no scan — [learnings-detail.md]
