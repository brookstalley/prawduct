...
## Sharing a MATCHER between two modules shares syntax, not the DEFINITION — the reader that feeds it is the other half, so before trusting "one home", ask what each module passes IN. A shared symbol *reads* as proof the two agree, which makes this worse than two copies. Fix by moving the missing half into the shared handle, never into both call sites — [learnings-detail.md]
