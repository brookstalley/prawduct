...
## A rule that LOWERS a severity outranks every rule that raises one unless you say it does not — state the floor with the ceiling, in the same sentence, or the suppression quietly becomes the file's highest authority. A prose ceiling ("wording is NOTE unless load-bearing") was written with only an upward exit; two bullets above, an actively misleading README instruction is BLOCKING, and a reviewer who correctly named that wrong action would have landed on WARNING — which gates nothing, so the wrong command ships. The exits that LIFT a ceiling are not the same as the severities it must never touch. Tell: your new rule caps a severity and the only thing you wrote next was how to escape the cap — [learnings-detail.md]

## Scrub the WHOLE diff before dispatching a review, tests and their comments included — a rule you just wrote is a rule you are still violating elsewhere in the same commit, and the reviewer will find it in the place you were not looking. A provenance ban on review history in comments shipped with a violation inside the test that PINS that ban; the scrub had covered the four protocol surfaces and skipped the author's own new test. The reflex it caught is the general one: under review pressure the instinct is to record *why the reviewer was right*, in the artifact, where it reads as documentation and decays into archaeology. Tell: you scrubbed the files the chunk is "about" and not the ones it added — [learnings-detail.md]
