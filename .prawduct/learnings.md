...
## Session-end signals must come AFTER handoff, not before

**Pattern**: The model says "Ready for next session" or "Session is complete" before actually writing WIP, reflection, or backlog — then when the user asks "should I /clear?", the model backtracks with "wait, let me do stuff first." The user reasonably interprets "ready to end" as "handoff is done." This is a variant of "I'd recommend `/clear`, but first..." that the original guidance didn't cover because it uses softer language.

**Resolution**: Broadened building.md and product-claude.md guidance to cover all completion signals, not just explicit `/clear` recommendations. Any signal that work is done (including "ready for next session", "session is complete") implies handoff steps 1-6 are already finished.

**Principle**: Relates to Governance Is Structural (#21) — behavioral guidance alone doesn't interrupt momentum. Relates to Complete Delivery (#2) — the handoff IS part of the delivery.
