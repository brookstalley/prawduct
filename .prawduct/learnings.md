...
## Naming a chunk/scope/tag freely writes into a MACHINE-READ field and can silently switch off the gate that reads it — check the parser's accepted form before inventing an id, because a value it cannot parse yields `null`, and null is NO ANSWER, not a pass. `CH-01` instead of `Chunk 01` left `chunk_graded: null` and `counts["chunk-ref-missing"]: null`, so the deliverable check never ran and nothing said so; `release=unreleased` in a change-log tag was caught only because three tests happened to police that vocabulary. Tell: you chose an identifier or tag value for readability, in a field some gate keys on. Ask what reads it and what it does with an unrecognized value — and prefer the gate that REFUSES an unparseable id over one that returns null
...
## A NEGATIVE assertion forbids everything its wording matches, not the one thing you meant — match the exact string that carries the behaviour you are excluding, because a loose phrase quietly outlaws any OTHER output containing it, and the test then pins the deletion as if it were the requirement. Making agent worktrees durable, I asserted `"ephemeral agent worktree" not in stderr` intending "the refusal did not fire" — but that phrase also occurs in the HEAD-snapshot NOTICE, which answers a different question ("how old is what I am reading", not "may I write here") and still applies. The test locked in the notice's disappearance for exactly the population the change created, and read as rigor while doing it. Tell: a `not in` / `assertRaises`-absent assertion over a substring rather than a whole message, on a subject that emits more than one kind of message. Pair every such assertion with a POSITIVE one for the behaviour that must survive
