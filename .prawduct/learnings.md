...
## A fallback must be checked against the SIZE of the interval it replaces, not just its name — "run a full review" is not a coverage claim, and a demotion can hand back an interval strictly narrower than the one just refused for being too wide. Compare spans before trusting a fallback, and have the refusal name the mode rather than leaving the reader to pick. Tell: a refusal tells the caller to re-dispatch and does not say as what — [learnings-detail.md]
