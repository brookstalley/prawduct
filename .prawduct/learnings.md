...
## When a predicate's job is to classify REAL artifacts, at least one test must read the real artifact — a fixture you wrote encodes your belief about the input, so it can only ever confirm that belief, and a suite made of them is green precisely where the belief is wrong. Widening a norm-detection guard, I wrote TWO over-fire fixtures aimed at exactly the failure that shipped, and never opened `templates/project-preferences.md`: it ships illustrative rows whose cells are non-empty placeholders, which `init_product` copies verbatim, so the guard fired on every freshly-onboarded repo while both fixtures passed. Pin the artifact and the predicate against EACH OTHER (read through `core.TEMPLATES_DIR`), or they drift apart the moment either moves. Mutation testing does not cover this — it probes the line you wrote, and says nothing about inputs you never supplied. Tell: you grepped the real corpus in order to WRITE your fixtures — you have already proved the corpus is walkable, so walk it in the test and assert the property that holds for all of it, rather than hand-copying the members you happened to notice

## A comment that NAMES a new failure mode owes a test in the same commit — if you can write the sentence, you can write the assertion, and a named-but-unasserted mode is worse than an unnoticed one because the comment reads as though it were handled. Widening a build-plan field reader from `.match` to `.search`, I wrote "the cost is that a line merely *discussing* the field parses as declaring it", judged it absorbed by fail-open, and shipped a fallback three lines below where it was NOT absorbed: first-sight return let prose bury the real declaration and report the loss as a typo. Related: widening what an input may BE changes every branch that consumes it, so re-read the whole function, not the line you edited. Tell: your diff adds a comment describing a case, and your diff adds no test for that case
