...
## "Has this repo already decided it?" is the cheapest check that could change a design — run it against the governing artifacts' `## Direction` sections BEFORE deriving, not after. A derived answer arrives without its constraints attached: the socket architecture was ratified in `architecture.md` a week earlier and the version I reasoned out was missing the no-third-party-dependencies implication until I read the norm. Tell: you are pleased with an architecture you have not yet traced to a parent
