...
## A precondition recorded only in PROSE is never re-evaluated when it is discharged — wire it as the machine-readable dependency the tool already honours, because nothing revisits an item whose blocker quietly closed. Triaging 50 design-stage items surfaced three describing already-shipped work: #237 was parked on two guardrails that closed 2026-08-01 and 2026-09-02 (the second superseding the first, and now grading exactly the hazard that parked it); `janitor/SKILL.md` skips check 5 citing a blocker closed a month earlier, whose successor then shipped; #677 described a gate relaxation that landed BEFORE the item was filed. `blocked_by` exists and `pick` honours it (`query.py:287`) — it was wired on 2 of 131 open items. Tell: you are writing "GATED by X" or "land Y first" into a body, and nothing but a human reader will ever check X or Y again — [learnings-detail.md]
