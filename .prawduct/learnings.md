...
## Committing a chunk BEFORE invoking the Critic makes a `final`-mode review grade the WORKING TREE, so any unrelated dirty file silently becomes the changeset and the chunk goes unreviewed behind a report that reads clean. Clean the baseline before the first commit; when work is already committed use `cumulative`, whose scope is the commits rather than the tree. Tell: the review's own signals line describes a changeset you do not recognise
