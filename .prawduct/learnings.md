...
## Record test-evidence AFTER committing — `test-evidence record` stamps `git_sha`=HEAD but tests the working tree, so a pre-commit record reads as stale

Run `prawduct-hook test-evidence record` as the LAST step of a work cycle, after the commit, on a clean tree — because it stamps `git_sha` = HEAD at record time while actually testing the WORKING TREE (HEAD + uncommitted edits). Record before committing and the stamp captures the *prior* commit; once you commit, the stamp lags HEAD and a consumer (the PR reviewer's merge-hygiene check) flags the evidence "stale / ran against a tree without the fix" — a false positive, since the content tested was correct. This is a stopgap until the freshness check becomes content-based ([[backlog]] **TST-4K2P** — stamp a hash of test-relevant files, not the commit SHA). Relates to Honest Confidence (#5 — don't let a false-stale stamp read as a real gap), Validate Before Propagating (#15), and [[when verifying a framework-repo change by running the hook use the repo-local bin/prawduct-hook]].
