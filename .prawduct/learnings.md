...
## When a question has stayed open across MULTIPLE rounds of observation, stop observing and read the mechanism — repeated inconclusive watching is itself the signal that the observable cannot separate your hypotheses, because the rival causes emit identical symptoms. Weight a working field workaround as a measurement, not an anecdote: if people are routinely getting the result the sanctioned path denies, the answer is already in hand
