...
## Inside a fix, the defect is systematically the JUSTIFICATION — an added "because" clause is a new claim carrying the same verification duty as the instruction it supports, and it escapes the check because it reads as commentary. Six rounds on one doc chunk: every defect sat in a justification or a procedure, never in a finding — the remedy I clarified (couldn't converge), the command I named (a passing test proved it can't perform the edit), the bypass I authorized (destroyed the fields whose protection I'd just cited as the reason for bypassing), the reason I supplied (self-contradictory in one sentence). The instruction was right every time. Explanation is generative — it reaches for a mechanism that makes the instruction feel inevitable — so when a fix keeps drawing findings, carry FEWER claims rather than better ones; verifying harder cannot help with a claim that never entered the check set. Sharpens [[The fix for a review finding needs the same adversarial pass as the original work]] by naming where in the fix to look
