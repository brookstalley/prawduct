...
## A measurement with no POSITIVE CONTROL cannot support a claim — before believing "X costs nothing", confirm the instrument moves when it should, because a dead instrument reads zero for the treatment AND the control, and zero is exactly the answer you were hoping for. Polling `rate_limit` reported `used: 0` for three conditional requests; the same poll reported 0 for five unconditional ones and for a 452-item rebuild, which is impossible and was the tell. Reading each response's own `X-RateLimit-Used` header gave 134→135→136 for 200s and a flat 136 across 304s — same conclusion, now actually evidenced. Tell: the confirming result arrived on the first try and the null case was never run

## Verifying a foreign API answers the question you ASKED, so ask what the design depends on rather than what the design says — a step scoped to the plan's own mechanism will confirm that mechanism and miss the one the plan got wrong. Chunk 02's verify-api was scoped to "`since` semantics and etag behaviour" and returned four clean answers; the finding that mattered — a LIST etag does not validate a per-item endpoint, so the column the plan said sync would write could never have worked — came from asking which endpoint each stored value would be replayed against. Rule: for every value you plan to PERSIST from a provider, verify the exact request that will later REPLAY it, not just the request that produced it
