...
## When a field's ABSENCE carries the meaning, a value NAMING the absence is its opposite — and it reads as deliberate, so review cannot see it. `release=unreleased` hid a finished branch from its own release. Ask the CONSUMER, not the reader: run the probe that acts on the field. Guard by BLAST RADIUS — `status=` was guarded, `release=` (drops a whole scope) was not. **And never write "verified" against a reader-check**: the word promotes a guess to a fact that later review reads as settled, so it must name the consumer it ran — `write_marker` overwrites, but `begin_review` refuses *before* reaching it — [learnings-detail.md]
