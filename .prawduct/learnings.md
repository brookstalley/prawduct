...
## RULING (harness-only-removal-is-not-a-major), 2026-08-11 — owner decision: `api-contract.md`'s major-deferral for subcommand removal governs the **externally-callable** surface only; harness-only subcommands may be removed at any tier. **Warrant falsified same day (v3.3.3)** — the tier permission stands, the atomic-update premise does not; read the next rule first — [learnings-detail.md]
