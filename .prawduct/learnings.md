...
## A test asserts what would BREAK, not what you just built — the vacuous-pass shapes are an exit code on a command with its own failure modes, an equality that also holds under the regression, and a fixture constant that erases the variable under test. Red-verify mechanically: break the subject, watch that specific test go red, restore. Three passed-with-the-feature-removed tests shipped on one branch, each written by someone who knew this rule, so the remedy is the mechanical pass and never the intention to be careful

## An apparent duplication across governing docs may be the RECEIPT for a budget already paid — before cutting it to fund a token addition, check whether a test pins it, because a pin's docstring is often the only record that an earlier trim was funded by relocating exactly that rationale. And never fund a budget by moving prose to another file: total context footprint is the only number that matters, so relocation satisfies the assertion while achieving nothing. Order is simplify-real-duplication, then RAISE the ceiling and record what bought the increase at the assertion, where the next person to hit it will read
