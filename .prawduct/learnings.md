...
## An UNEXPECTED PASS is a signal, not a result — when a change you believed would break tests doesn't, find out which branch they are on before banking it. Twice in one session: a restamp guard left 13 tests green because they seed via an on-ramp that records no `evidence_tree`, so they exercise the *uncheckable* branch (investigated — principled, not luck); a mode-inference guard's own new test went green because its fixture never commits the plan file, so the tree is dirty and the redirect it was written to prove is skipped (banked — the fix was inert and shipped to a review). The reflex is to read green as confirmation because that is what green usually means. Tell: you predicted red, got green, and explained it to yourself in one sentence without opening anything
