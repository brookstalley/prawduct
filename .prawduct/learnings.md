...
## A general policy sentence in a document is NOT evidence that a specific procedure inside it inherits the thing you are adding — the evidence is structural, so read the second procedure end to end and ask what it re-states. `adapter-mode.md` says "any confirm-before-write step is yours to run before the call", which is true and does not answer whether its own `### add` inherits `SKILL.md`'s steps; two paragraphs below, that `add` re-states the DEDUP step for itself, and a procedure that re-states a step it would inherit is replacing, not supplementing. The prompt shipped silent on the only backend this repo runs. Tell: you answered "does B inherit from A?" from a sentence about the class rather than from B's own body

## When one rule is carried by two surfaces on purpose, pin it in the module that reads BOTH — a bar reworded in one carrier is two bars for one decision, and no single-file guard can see that. Corollary that cost a round here: bound the assertion to the smallest region that must carry the phrase, because the same phrase appearing elsewhere in the section keeps the mutant alive (`ready to build` sat twice in the region read; stripping it from the offer left the test green on a neighbouring sentence — the fix was in the prose too, since the paragraph stated the bar twice)
