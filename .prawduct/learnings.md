...
## Committing a chunk BEFORE invoking the Critic makes a `final`-mode review grade the WORKING TREE — so any unrelated dirty file silently becomes the changeset and the chunk goes unreviewed behind a report that reads perfectly clean. Three stray files left over from a prior session were graded as "doc-only, 3 files, 0 judgeable" while a three-module Python chunk sat committed and unexamined; the review even produced real, well-argued findings, which is what made it convincing. Clean the baseline building.md asks for BEFORE the first commit, and when work is already committed use `cumulative` (`merge-base...HEAD`), whose scope is the commits rather than the tree. Tell: the review's own signals line describes a changeset you do not recognise
