...
## A change-log entry's `scope=` tag must be copied from the PLAN you are working in, never from the entry above it — under `views_enabled`, `collect_shipped_chunks` matches `scope=` by exact string, so a plausible neighbouring scope silently flips the wrong plan's checkboxes while yours regenerate to `[ ]`, and `diagnose_scope_plan_coverage` stays quiet because the borrowed scope really does have chunks by those ids. Tagged `scope=backlog-cache` on a `backlog-cache-write-path` plan — the same failure class as the `release=` placeholder three weeks earlier, recurring inside the entry that documented the fix for it. Tell: you are writing a change-log entry for a branch whose scope is a NARROWING of an existing one
