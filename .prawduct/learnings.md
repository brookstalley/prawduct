...
## Justify at the ALTITUDE OF THE DECISION, not the altitude of the mechanism — a mechanism claim is a new claim carrying the same verification duty as the instruction it supports, and it escapes the check because it reads as commentary. The test: **will the reader have to reason PAST this instruction** — handle a case it doesn't enumerate, or depart from it? No → mechanism is pure liability. Yes (a norm, a design decision, anything someone will later argue an exception to) → mechanism is load-bearing, so verify it like code. Same species as over-precise counts (*"22 call sites, not 21"* → *"more than 10"*): a claim finer than any decision requires can be wrong and buys nothing when right — cf. D14 "never persist derived counts" and the no-suite-total preference, which institutionalize the numbers half. Six rounds on one doc chunk: every defect sat in a justification, never in a finding — the remedy I clarified (couldn't converge), the command I named (a passing test proved it can't perform the edit), the bypass I authorized (destroyed the very fields whose protection I'd just cited as the reason for bypassing), the reason I supplied (self-contradictory in one sentence). **The instruction was right every time.** Explanation is generative — it reaches for a mechanism that makes the instruction feel inevitable — so when a fix keeps drawing findings, carry FEWER claims rather than better ones; verifying harder cannot help with a claim that never entered the check set. **This is about altitude, never omission**: Principle 4 and the norms lifecycle both require the why, and a rule read as "justify less" is gameable into "drop traceability." Sharpens [[The fix for a review finding needs the same adversarial pass as the original work]] by naming where in the fix to look
