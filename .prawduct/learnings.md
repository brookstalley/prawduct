...
## A refusal predicate is not a severity predicate — when a gate answers "why this cannot be trusted" by folding several conditions into one string, do NOT reuse it to decide how hard to fail, because the mildest condition it refuses is usually ordinary and escalating on it punishes the common case. Split the reasons at the call site, and pin the discriminating one with its own test. Tell: you reached for the function whose NAME matched your sentence, and its docstring already says its outcomes "must not share a sentence" — [learnings-detail.md]
