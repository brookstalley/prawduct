...
## RULING (harness-only-removal-is-not-a-major), 2026-08-11 — owner decision, taken in session on a direct question about THIS clause. `api-contract.md`'s major-deferral for subcommand removal governs the **externally-callable** surface only; harness-only subcommands may be removed at any tier. Deprecate-then-remove still governs anything a human or skill can call — [learnings-detail.md]

## RULING (one-home-is-the-predicate-not-the-token), 2026-08-11 — amends `architecture.md` § Direction *every fact has one home*. Sharing a MATCHER shares syntax, not the DEFINITION: the reader that feeds it is the other half. A shared symbol READS as proof the two agree, so this is worse than two frank copies — nobody re-checks a fact with one home. Test: does the shared thing include the INPUT — [learnings-detail.md]
