...
## RULING (harness-only-removal-is-not-a-major), 2026-08-11 — owner decision: `api-contract.md`'s major-deferral for subcommand removal governs the **externally-callable** surface only; harness-only subcommands may be removed at any tier. **Warrant falsified same day (v3.3.3)** — the tier permission stands, the atomic-update premise does not; read the next rule first. **Scope settled 2026-08-11 (v3.3.4) by `[[deprecation-requires-an-inert-retention-window]]`** at the end of this file — [learnings-detail.md]
...
## RULING (deprecation-requires-an-inert-retention-window), 2026-08-11 (v3.3.4) — owner decision settling the question `[[harness-only-removal-is-not-a-major]]` left open: the harness-only removal exception **requires an inert-retention window**. Unregistering a hook is free and immediate; deleting its subcommand waits until no supported install still registers it. The tier permission is unchanged; the window is what REPLACES the falsified atomic-update warrant, because it delivers mechanically the safety that warrant was assuming. Homed on `api-contract.md` § Direction's Rulings line; unblocks #644 — [learnings-detail.md]
