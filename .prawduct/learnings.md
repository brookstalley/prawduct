...
## Before recommending that something be built, check whether it was already built and REMOVED — a removal comment is a decision with measurement behind it, and re-proposing it without new evidence spends that measurement twice. An audit's headline fix was "lint pinned figures and citations"; `record_lint.py` already had the count half, and the citation half (`dangling-ref`) had been built, measured at 3 findings / 0 true positives, deleted under "a control that fires and catches nothing is removed by default", and annotated with the exact bar for re-adding — which the audit did not clear. The evidence was in hand before the recommendation: a reviewed finding already cited that file by line. Tell: you are proposing a control and have not opened the module that would host it — [learnings-detail.md]
