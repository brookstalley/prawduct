...
## A parser shared between a READER and a WRITER inverts its safety on malformed input — the reader's tolerant default (an unterminated comment/block runs to end-of-document) is the writer's destructive one, because "assume it extends to EOF" becomes "delete to EOF". A writer must claim nothing it cannot delimit, and a well-formed fixture corpus can never show you this. Tell: you reused a scanner written for reporting inside something that edits files

## When you catch yourself defending a design with "we shouldn't lose the record", check whether the real reason is REVERSIBILITY — the two take different fixes and the preservation framing produces the worse one. History earns its keep only by helping going forward; a filed-and-shipped note or a record inside an artifact about to be archived has none. Reversibility is answered by a preview that shows every change plus version control, not by refusing to act. Tell: your design grew a second "report but don't touch" bucket to protect prose

## A list an operation-level approval is given for must IDENTIFY its items — `Path.name` is ambiguous the moment a repo nests plans as `plans/<id>/build-plan.md`, and consent to four indistinguishable lines is not informed consent. Reach for the repo's existing display helper before formatting a path yourself; the defect is usually already solved one module over. Tell: your preview prints bare filenames and every fixture you wrote is flat
