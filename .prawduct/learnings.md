...
## When designing any flow step that records status or bookkeeping, make it ride IN the PR that does the work — a step that can only run post-merge on the integration branch is structurally broken for protected-branch consumers — with one exception the rule must now name: bookkeeping that is NOT a commit. An API-backed status change (a GitHub issue close) has no branch to ride, so "in the PR" buys nothing and costs correctness — run on an unmerged branch it survives an abandoned PR, leaving an item wrongly closed. Ride-in-the-PR is a property of BEING a commit; for a remote side effect the equivalent is running it at the merge, in the same breath, before the local artifacts recording the debt are deleted (`/prawduct:pr` Merge Flow "Close the backlog items this PR resolves"; `documentation/backlog-service-requirements.md` GV3 replaces ship-atomicity with traceability plus a reconciliation sweep). Ask which one you have before applying the rule
