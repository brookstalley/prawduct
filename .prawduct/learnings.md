...
## Surveying a shared thing takes TWO searches: grepping the thing finds its DUPLICATE COPIES, never what DEPENDS on it. Widening a predicate — grep the *shape* for copies, then the *predicate's name* for the callers that branch on it (any shared predicate: a validator, a feature flag, a type guard). Relocating a fact — grep the fact, then grep prose naming its OLD HOME, because "the rule lives in X" goes false the moment you empty X. A survey that ran only the first is incomplete however clean it looked — [learnings-detail.md]
