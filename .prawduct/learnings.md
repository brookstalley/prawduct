...
## Prose describing what a new guard BUYS must state the guard's predicate, not its purpose — and the falsifier is the guard's own deliberate-exclusion tests, the ones written to pin what it does NOT cover. Fixing the gate's blindness to a release-pending entry with no `scope=`, I wrote a runbook paragraph concluding "nothing can hide from a `scope=`-keyed grep"; `test_an_untagged_entry_is_still_invisible`, in the file I had just edited, pins that an entry with no tag line is deliberately outside the set — so the paragraph told the operator to skip the untagged walk, shipping a new instance of the bug it was fixing in the document that teaches it. Motivation ("scopeless entries can no longer hide") and extension ("nothing can hide") are one word apart in English and a whole set apart in code. Sharpest tell: the claim uses a term defined BOTH in code and in operator prose with different extensions — your sentence names the term the DOCUMENT defined, its warrant is a FUNCTION — [learnings-detail.md]

## Naming a prior fix as "the same family" or "the same shape" IS the class finding, not a citation for an instance-level one — a recurrence is evidence the first fix was scoped too narrowly, so the remedy is the construction that would have prevented BOTH members and the search bound is every site the shared predicate could reach. Scrubbing my own chunk I found a new prawduct-only path printed at product repos, wrote down "same family as the no-version hint fixed in R-13/R-30", and then proposed rewording the message; the Critic took the identical fact and required one predicate asked once, suppressing the arm rather than rephrasing it. Root cause: a self-scrub asks "what did I get wrong?", and "has this been fixed here before?" is a different query that nothing in that pass runs. Tell: your own note cites a prior finding id as precedent and your proposed fix touches strictly fewer sites than that precedent did — [learnings-detail.md]
