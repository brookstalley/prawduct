...
## Enumerate the sites answering a question by GREP, never by memory — a fix that threads a resolved value through the two call sites you remembered leaves the third reading the old source, and the comment you write above it ("both fields") is accurate for exactly one commit. Same shape as the bug being fixed, one field over. **And the grep itself is a site**: a find-and-replace over quoted literals is a PREFIX of the real set wherever the code also BUILDS those strings (`f"t{i}"`), and a sweep scoped to `print(...)` arguments cannot see a string returned, appended to a printed list, or assembled into a document. Third and fourth instances, both on the branch that wrote this rule — widen the query until it is falsifiable, then have someone else run it
...
## A filed item's stated MECHANISM is a hypothesis, not a finding — reproduce it against live data before designing the fix, because the reporter saw a correlation and wrote it up as a cause, and the item's own evidence often carries the disproof. "Stage-less items vanish from counts" was really "items with no prawduct provenance vanish"; the tell was in the report — a total that moved, which a stage-bucket bug cannot do — and the live gap was 1 item where the item predicted 9. Cheapest check: run the command and reconcile the number against the source of truth, THEN read the item again
