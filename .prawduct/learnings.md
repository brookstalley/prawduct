...
## When you swap a mechanism's input for a COPY of a file, ask what the original's METADATA was load-bearing for — a byte-identical copy is not an identical input, and the loss is silent. Seeding the Critic's tree capture from `.git/index` instead of `read-tree HEAD` was a pure cost fix on paper; `shutil.copyfile` does not carry mtime, which is the ONLY thing keeping git's racily-clean rule alive, so `add -A` skipped re-hashing a same-tick same-size edit and the captured tree carried the file's PREVIOUS content — a review vouching for a tree that never existed, a fail-open strictly worse than the timeout being traded away. It surfaced as a 1-in-25 suite flake whose failing assertion named two STABLE hashes; that stability is what says "deterministic defect, probabilistic trigger" rather than noise. `copy2` over `copyfile` is the fix; the general form is that filesystem metadata (mtime, mode, xattrs, inode) is often a protocol, not decoration. Tell: you replaced a path with a temp copy and reasoned only about the bytes — [learnings-detail.md]
