...
## A "never raises" contract is a claim about EVERY input — it fails on the one nobody constructed. State a producer's bad-path behaviour only after READING it; treat a fix whose reason names an exception CLASS as class-wide: grep the class across its siblings in one pass. Tell: your reason states "X is a subclass of Y" while your diff touches only what was in front of you — [learnings-detail.md]
