...
- **Presence of `verifier`** is the discriminator: legacy fingerprint-only evidence keeps validating unchanged; new-schema evidence (any record carrying `verifier`) must also carry `tests_executed`, `changes_referenced`, and `coverage_level`, and the schema validator will reject the record if any are missing or wrong-typed.
