...
- An item with `stage: idea|research|requirements|design` → pick surfaces the stage and **routes the next action to the matching guide** (`idea`/`research`/`requirements` → `/prawduct:methodology discovery`; `design` → `/prawduct:methodology planning`), framed as *"this item needs <stage> work next, not implementation."* Advancing the stage is itself the work; the agent updates `stage:` via `/backlog update` as the item matures (and adds a `refs:` link to the requirements/arch doc once written).
