...
- **[P0]** **Retire the `incoming-bugs/` drop-box in lockstep with a minimal same-repo replacement — never before it (MG5).** Migrating prawduct's backlog makes prawduct's own **public** GitHub repo the live upstream target, so the drop-box's file-drop channel is retired — but *only together with* its 1:1 replacement: `/prawduct:report-bug` files a GitHub **issue** into prawduct's own repo via the adapter's create path in place of writing a file, and the receiving-side `untriaged-upstream-reports` advisory counts that intake set instead of `incoming-bugs/*.md`. *(Two clauses here are **superseded 2026-09-07** by the owner-approved `documentation/backlog-service-upstream-filing.md`, which is the design this bullet asked for and therefore outranks it: the issue lands **label-less** — a non-collaborator cannot set labels, so the intake signal is the `[prawduct]` title prefix and the absence of a triage label, per §2/§6 — and the design's §5 removes the local capture named at the end of this bullet. Corrected here rather than in place because a P0 requirement's history is the record of what was asked for.)* This is deliberately a **fixed-target, public-repo issue-create** — a subset of XP1 that needs no new auth (any authenticated user may open an issue on a public repo). The **full** XP1/XP2 surface — arbitrary cross-owner targets, private repos, foreign-identity auth-by-target-owner — **stays W3**; do not pull it forward. The no-channel fallback is the canonical-tracker pointer alone — **superseded 2026-09-07**, same design: §5 is submit-or-nothing, and an upstream bug captured in a product's own backlog reaches nobody who could fix it.
