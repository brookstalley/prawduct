...
Two §11 spikes gate the core — **S1** (now narrowed by O5: confirm ETag + the M6 no-delete/number-non-reuse facts + the cloud-proxy *optimization* test — the core transport, required-`gh`, is decided) and **S2** (the migration dry-run — the slice's first increment, run **prawduct-first** and after the scrub). **O5** (P0-slice auth/transport) is **resolved** (`gh` required, inherit session auth, App optional). **S3** carries one load-bearing constant (the ~500/hr write cap that paces migration); **S4/S5** settle with their layer/feature. Until this level is agreed, no build plan and no field-level schema.
