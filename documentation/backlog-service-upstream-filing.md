...
**[DECISION: submit-or-nothing re-affirmed on evidence, not just first principles (owner, 2026-08-03) | the local-capture leg is not a safety net, it is a landfill]** The "helps no one and clutters the product" clause above was written as an assertion. It now has an observation behind it — owner, 2026-08-03: *no consumer has ever taken a locally-captured upstream bug and manually submitted it.* Not "rarely"; not once. A fallback nobody has ever traversed is not a fallback, and keeping it would mean every declined report leaves a permanent artifact in the product that no one will ever act on. Recorded here because "capture it locally, submit it later" is the obvious-looking kindness a future reviewer will re-propose, and the reason to decline it is **empirical, not aesthetic** — which is a much harder argument to reconstruct from the design alone. Confidence: this is an owner observation over the drop-box's lifetime, not an instrumented count; it is strong enough to settle the design and is not claimed as measured data. The *pointer* fallback (§7 step 1) is untouched and stays — it costs nothing and leaves no residue.
...
The intake/triage half is MG5's receiving end + Security §5, tracked separately on **#234** (drop-box retirement in lockstep + the advisory repoint) and **#544** (the intake/quarantine query — one query under two names); this doc only guarantees the outbound payload carries the signal it needs:

**[Addressee corrected 2026-08-03 — Critic finding.]** This line read *"report-bug receiving side / BKL-6M4T"*. `BKL-6M4T` resolves to **#233 — "run the live prawduct backlog migration"**, which is **closed** and is about the corpus migration, not the receiving side. Every "the receiving-side item" pointer in this doc meant #234/#544. **#234 also carries superseded acceptance criteria** — its Proposed change still specifies that `report-bug` files an **`untriaged-upstream`-labeled** issue and that the advisory *counts labeled issues*. This design supersedes both: §5 arrivals land **label-less** carrying only the trimmed block, and the intake query below is title-convention-plus-no-triage-label. Reconcile #234 before building from it.
...
**[DECISION: adoption is in MG5's scope, not deferred behind it (owner, 2026-08-03) | counting without a route out re-opens on the write side the hole #532 just closed on the read side]** Repointing the advisory makes arrivals **loud**; it does not make them **actionable**. A report filed through §5 lands as a label-less issue carrying only the trimmed block — inert to `pick` — which is the same terminal state a human-filed issue with no block reaches by a different road (#533 *was* the live instance: hand-closed 2026-08-02, still label-less and block-less, i.e. dispositioned entirely outside the tooling — which is the failure mode this op exists to end, not an exception to it). The two populations are *discovered* differently (this section's `[prawduct]`-title intake query vs. #532's untriaged count, which #533 matches and an MG5 arrival does not) but they need the **same remedy**, and it is one op: `backlog adopt <id>`, settled on **#542**. Shipping the count alone would have MG5 delivering issues into the tracker whose only supported exit is hand-editing labels in the GitHub UI — precisely how nine of them silently accumulated before #532. Adoption is **attended-only** (no scheduled sweep), which keeps it inside the §4.3 attended-only posture instead of minting a second unattended write path into someone else's report. Build note for **#234**: `adopt` must **merge into** an existing trimmed block rather than stamp a fresh one, since §5 arrivals already have one. (Precision on the inertness above: an MG5 arrival *does* decode as a prawduct issue — `encode.is_prawduct_issue` scopes on namespaced-label **or** block — so it is outside #532's untriaged set; what it lacks is a triage stage and an `id:` alias, so `pick` skips it and bare-id resolution fails while `#number` works. A #533-style filing lacks both label and block and so falls inside that set. Different defects, one remedy.)
...
- **[§6 — receiving side]** exact intake query + advisory repoint — pinned on **#234** (retirement lockstep + advisory repoint) and **#544** (the query), not here. *Corrected 2026-08-03: this said "the receiving-side item", which §6 mis-resolved to the closed #233.*
