...
| `verify-migration` | **yes** (read-only) | the completeness gate between an import and a cutover: re-parses the source exactly as `import` does (`collect_records` → `apply_archive_scope`, so source and create sets cannot drift) and compares it against the target's `id:` aliases. **Exit 4** on any of `missing` (parsed but never created), `unaliasable` (no usable PFX in the source — derived from the source parse alone, so no issue-side action can clear it) or `collisions`. Human mode **names** the stranded items rather than counting them; the remedy for `unaliasable` is unactionable without them. Fails closed on transport error, alias-scan truncation and scope mismatch | MG1 |
