...
|---|---|---|---|
| `list` | ✓ | **structured** field/label filters + sort + paginate — runs **online off the REST list endpoint**, **strongly consistent in practice** (a just-written item appears immediately; a rare brief replication window exists — the documented 404-retry-after-create case — so it is not an absolute platform guarantee), no cache needed. `--untriaged` **inverts** the scope filter, returning only what `list` normally drops (the `counts` `untriaged` set); it **scans every page and ignores `--per-page`/`--page`**, because untriaged issues are typically the newest and a first ascending page would answer "nothing to triage" while items waited on page 2 | Q1-structured |
