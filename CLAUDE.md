...
The Critic skill runs with `context: fork` (separate context) and restricted `allowed-tools` — it reviews through code analysis only, **never running test suites, builds, or executables**. In the coordinator pattern (medium/large `final`, `cumulative`) the review is split across `critic-reviewer` subagents whose own restricted `tools` bind them (so no-execution is structural for them too — CRT-3X9D), and **persistence is decoupled from the review**: the reviewers write partials and a deterministic `prawduct-hook critic-consolidate` merges them into the findings + ledger anchor (no model in the write path — the fix for the v2.1.198 background-by-default breakage). The load-bearing invariant — *an independent reviewer must never mutate the session it is reviewing* — is enforced at the mutation site: while a review is active (`prawduct-hook critic-begin` until `critic-end` or `critic-consolidate` clears the marker), the session-mutating `prawduct-hook clear` refuses to run.

After review, the Critic records findings to `.prawduct/.critic-findings.json`. After a **coordinator** review (medium/large `final`, `cumulative`), run `prawduct-hook critic-consolidate` before reading the findings — an idempotent no-op when the `SubagentStop` trigger already landed them, and it closes the window where an unfired trigger would leave you reading the *previous* review's file. Fix any blocking findings before proceeding to the next chunk. After resolving findings, reflect: what did the Critic surface that you missed? Capture learnings immediately — Critic reviews are the richest source of methodology insights.
