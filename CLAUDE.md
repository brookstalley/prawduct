...
The Critic skill runs with `context: fork` (separate context) and restricted `allowed-tools` — it reviews through code analysis only, **never running test suites, builds, or executables**. Its data plane is deterministic (kernel v3): `prawduct-hook critic-begin --mode <m>` derives the review interval and roster and writes the dispatch manifest (code, never a model); the reviewer(s) — the fork itself single-pass, or `critic-reviewer` subagents whose own restricted `tools` bind them (no-execution structural — CRT-3X9D) whenever `critic-begin` derives a three-reviewer roster — write partials; and `prawduct-hook critic-consolidate` merges them against the manifest, appends the review fact to the shared evidence store (`<git-common-dir>/prawduct/evidence.jsonl`, tree-keyed, shared by all worktrees), and regenerates `.critic-findings.json` as a derived view (no model in the write path; no gate reads the view — gates compose over facts). The load-bearing invariant — *an independent reviewer must never mutate the session it is reviewing* — is enforced at the mutation site: while a review is active (`prawduct-hook critic-begin` until `critic-consolidate` persists it, or `critic-end` abandons it), the session-mutating `prawduct-hook clear` refuses to run.

Every reviewing fork consolidates and reports its counts — on the coordinator path it awaits its three reviewers first, so that report *is* the findings rather than a dispatch receipt. Before reading `.critic-findings.json` directly, run `prawduct-hook critic-consolidate`: an idempotent no-op once the review has landed, and it closes the window where you would otherwise read the *previous* review's file. Fix any blocking findings before proceeding to the next chunk — the follow-up `/prawduct:critic verify-resolutions` records the resolution facts that unblock them. After resolving findings, reflect: what did the Critic surface that you missed? Capture learnings immediately — Critic reviews are the richest source of methodology insights.
