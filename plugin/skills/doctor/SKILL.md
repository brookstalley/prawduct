...
1. **Install reference** — `.claude/settings.json` matches the whole install contract. **Read the contract from `lib/migrate_plugin.py` `INSTALL_REFERENCE` and compare every leaf in it; do not check from a list here.** (At time of writing that is `enabledPlugins["prawduct@prawduct"]`, and under `extraKnownMarketplaces.prawduct`: `source.source`, `source.repo`, `source.ref`, `autoUpdate` — illustrative, not the set, and the constant wins where they differ.) Missing or drifted → contributors won't get governance on clone. Check every leaf, not just the familiar two: the ambient `install-reference` / `contract-drift` advisory routes here as its recommended action and fires on all of them, so any field it can report and this check cannot repair is a nudge that reports healthy when you follow it.
