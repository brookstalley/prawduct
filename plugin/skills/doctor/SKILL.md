...
- **degraded**: governance works but something is off (missing anchor, a missing non-critical file, discovery uncaptured despite product work, an exposed API with no recorded versioning decision, norm-registry integrity findings, an active strategy-class coverage layer, or a learnings corpus whose descent obligation is missing or below the first rule) — list each with its implication and the fix.
