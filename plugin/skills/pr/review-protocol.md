...
- **Backlog reconciliation** — the PR boundary is the natural "a branch of finished work is merging; were the items it closed updated?" checkpoint (you already read the full diff + backlog). Two checks, both respecting D4 (flag, never infer/auto-update — the explicit `/prawduct:backlog update` is the builder's call).
  - **Check the backend first.** Read the top-level `backlog_service_repo` scalar from `.prawduct/project-state.yaml` (you read that file at step 1). If it is **set**, this project's live backlog is GitHub Issues and `.prawduct/backlog.md` is frozen history — **skip both R-1 and R-2**, and emit one NOTE: "Backlog reconciliation unavailable — this project is on the GitHub Issues backend and these checks have no Issues-mode path yet; they return when the backlog read-through cache lands." Do **not** open `.prawduct/backlog.md` for live state in that mode: every item archived at cutover still parses as open, so R-1 would flag items closed months ago and R-2 would resolve every `closes:` against frozen history — passing or dangling with equal confidence. **Say it even though R-1 is the Critic's walk:** the Critic is dark for the same reason, and R-2 has no other owner anywhere in the pipeline, so silence here reads as "reconciled" when nothing reconciled it. A stated gap is recoverable; a confident wrong answer is not. If it is **unset** (the markdown backend), run both as written:
  - **R-1 (NOTE) — resolved items.** *The cumulative Critic owns this walk (its Backlog Reconciliation cross-check), so don't repeat it.* Flag only what you notice incidentally while reading the diff for your own goals: an open or promoted item the changes resolve → "branch work appears to resolve `[PFX-XXXX]` — verify and `update status=shipped`, or say why it stays open."
  - **R-2 (WARNING):** **always run on this backend — the Critic does not do this check, and no other layer does either.** A change-log entry or commit on the branch references `closes: PFX-XXXX` / `closed-by:` but that item is still `status: open` — a *data inconsistency* (change-log and backlog disagree), not an inferred status, so flag it.
