...
- **Backlog reconciliation** — the PR boundary is the natural "a branch of finished work is merging; were the items it closed updated?" checkpoint. The payload's `backlog` section has already resolved every id the commits and the change-log entry cite, so both checks below read it rather than querying. Both respect D4 (flag, never infer/auto-update — the explicit `/prawduct:backlog update` is the builder's call).
  - **A degraded `backlog` section means the section could not answer, not that nothing matched** — and its reason says which of two things failed. **The store could not be read:** emit one NOTE — "Backlog reconciliation unavailable — [the section's own reason]; run `prawduct-hook backlog sync --repo <scope>`" — and **skip both checks**. **An input could not be scanned** (the reason names the commit messages or the change-log entry): a sync cannot help, so run R-2 **by hand** over that input instead of skipping it. An ok section can carry the same gap as a `NOT SCANNED:` line, and it binds the same way. Silence here reads as "reconciled" when nothing reconciled it, and R-2 has no other owner anywhere in the pipeline.
