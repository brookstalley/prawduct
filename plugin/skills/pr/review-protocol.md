...
- **Test evidence is fresh for the shippable changeset.** Don't run the suite yourself. Read `.prawduct/.test-evidence.json` and validate freshness via `prawduct-hook test-status` (exit 0 = current, 1 = stale) — that exit code is the *only* freshness signal this gate needs; its printed label says whether the 0 is `tree-valid` (the run met this tree) or `session-fresh` (a run from earlier this session, which never met it), and either satisfies this check. Never infer "stale" from a commit/SHA field in the evidence: the record carries none (TST-4K2P retired `git_sha` precisely because a record-before-commit run made it lag HEAD and read as a false stale). If `test-status` reports `stale` or evidence is missing → **WARNING** ("test evidence is not fresh for the changeset I'm reviewing"). This is a release gate (does the evidence match what's shipping), not a judgment on test quality — that's the Critic's.
