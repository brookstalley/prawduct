...
- **Change-log status:** run `prawduct-hook resolve-base`. When the base is an integration branch (`develop`), the Step 1c entry stays **statusless** — a statusless tagged entry IS the release-pending state; the `develop→main` release flips it to `status=shipped`. When the base is the **release surface** (trunk — `resolve-base` prints the `main` family), this merge ships the work: tag the entry `status=shipped` — plus `release=vX.Y.Z` when the product tracks versions (`release-notes.md` groups by `release=`; a shipped entry without it flips checkboxes but never appears in release notes) — and run `prawduct-hook regen-views` so the derived views ride in the PR too. **Read its exit code.** `regen-views` writes views one at a time and reports per-view: **0** = every view written; **3** = *partial* — some views were skipped and named on stderr, the rest were written; **2** = nothing written (an entry it could not interpret at all). On this path the merge IS the release, so a **3 is a release blocker even though views were written** — the withheld view is precisely the one whose plan the release needed to regenerate. Fix what stderr names and re-run until it exits 0; never carry a 3 into the PR on the strength of "it wrote something".
