...
7. **Confirm the bookkeeping merged WITH the PR — there is nothing to commit here.** Create-flow Step 1d put the backlog archives, the change-log `release=` tag, the Status ticks, and (trunk) the plan retirement in the branch, so the merge just landed them atomically. If something was missed, fold it into the next PR that touches the repo — **never** push a bookkeeping commit to the integration branch and **never** open a housekeeping-only PR. On gitflow, release tags additionally self-converge at the release (it tags every unreleased entry regardless) and the backlog skill's reconcile step catches unarchived items; on trunk there is no later release step, so the fold-into-the-next-PR rule IS the convergence — treat a missed `release=` tag as named debt for the next PR, not something that self-heals. Run `prawduct-hook resolve-base` to know which case applies:
   - **Base is `develop`** (gitflow, ahead of a batched `develop→main` release): the work is release-pending — its change-log entry carries **no `release=`** (older logs may also carry a legacy `status=` stamp; it is inert). **RETAIN** the plan live: archiving is what it gets at the release, not at this merge. What happens to the *pointer* depends on how the plan is resolved, and the two cases differ:
     - **The plan declares `branch:`** — **clear `active_build_plan`.** Its branch is merged and deleted, so the declaration resolves for nobody and the plan reads live-but-inactive, which is exactly what RETAIN wants. Leaving the pointer aimed at it puts it back in force on every branch that has no plan of its own, and the briefing then nags to archive a plan you were told to retain — an advisory with no correct action, which is the thing this design removes rather than something to ignore. Clearing costs nothing: the plan stays live and findable, and the release archives it by scope.
     - **The plan is pointer-resolved only** — keep the pointer, because clearing it is what makes governance blind to the plan. A non-blocking "archive the plan" advisory may surface during this window; ignore it until the release ships.

     (Archiving early is not the deletion it used to be — `check-releasability` searches archived plans too — but it moves a plan that is still the live description of unshipped work.)
