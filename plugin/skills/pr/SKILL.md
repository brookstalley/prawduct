...
**Release-promotion guard — check this before routing, and ask the repo rather than a list.** Two commands name the two branches that are not feature branches here:

- the **integration base** — `prawduct-hook resolve-base`, the same resolver Step 1 and the review gates key off. It prints one ref, possibly remote-qualified (`origin/develop`); strip a leading `origin/` before comparing. Exit 1 means the base is unresolvable — say so and stop, rather than falling back to a guess.
- the **release surface** — the repo's default branch: `gh repo view --json defaultBranchRef -q .defaultBranchRef.name`, or `git symbolic-ref --short refs/remotes/origin/HEAD` where `gh` is unavailable.

Compare `git branch --show-current` against those two. If it matches either, rather than being a feature branch, this is a **release/integration context, not a feature PR.**

**Why derived and not a list.** `develop`/`main`/`master` was the whole test, in a skill whose every other step already calls `resolve-base` precisely so no one hardcodes a branch name. A repo that integrates on `trunk`, `integration` or `next` fell straight through the guard and ran the feature-PR gates on its own release — the failure this guard exists to prevent, in exactly the repos that had configured `base_branch:` correctly. **Note what the guard inherits:** `resolve-base` reads `base_branch:` from `project-state.yaml`; when that is unset it consults `origin/HEAD`, prefers it when it names a non-main-family branch, and otherwise falls back to a candidate list headed by `main` (`#254`). Onboarding records the knob when the remote earns it. The residual gap is the repo whose `origin/HEAD` is `main` but which integrates on `develop` — nothing can infer that — so if you are on a branch that is plainly an integration base and the guard did not fire, record `base_branch:` and say so. The `develop`→`main` release is a separate **manual** process owned by your project, and the feature-PR gates below (the cumulative-Critic gate in Step 2, the PR reviewer in Step 3) do **not** apply to it: each feature's cumulative review and release-readiness review already happened on its feature→`develop` PR, and the release is published by that project's own release runbook plus a version bump — not by `/prawduct:pr`. **STOP and hand back to the user's own release process; do not run the Create/Update gates.** (A `check-cumulative-critic` exit-1 in this context is expected and benign — release-prep necessarily touches non-`.md` version files that the gate's docs-only allowance doesn't cover — so it is **not** a gate to satisfy and **not** a waiver case.)
...
- **Backlog — and this one splits by backend, so run `/prawduct:backlog` rather than assuming:** the call is the same either way, `/prawduct:backlog update <id> status=shipped closed-by=<scope>`, but *when* it runs is not.
  ...
  - **Issues backend** (`backlog_service_repo` is set): closing an issue is an **API call with no branch to ride**, so it is the single item deliberately deferred to the Merge Flow's **"Close the backlog items this PR resolves"** step, which fires seconds after the merge succeeds. This is not the "post-merge commit" the rule above forbids: no commit is involved, and the integration branch is never touched. Record which items are owed a close at merge, and carry that into the PR description when you write it at Step 5 — an operator who inherits the PR has no other way to know it is outstanding. (Nothing to write yet: the description does not exist until Step 5.) Do **not** rely on `Closes #N` in the PR body to do it for you: GitHub fires closing keywords only for PRs merged into the repository's **default** branch, so on a gitflow base the keyword is inert. Route `closed-by=<scope>` through `update --closed-by`, which writes a queryable block field (#550/#564); the close op itself still takes no such argument, so it is a second call, not a flag on the first.

  **What the two bullets above give you is the action; the rule they follow is owned by `/prawduct:backlog`'s "When to mark shipped".** That is where the *reason* lives — the atomicity is a property of being a commit, so it holds for a file edit and not for an API call — and where a change to the timing has to be made. Restating the action here is the compromise: a reader mid-PR should not have to leave the step, but nothing here decides the rule, so the two cannot drift the way they already did once.
...
- **Build plan (trunk only):** if this PR completes the active plan, retire it on the branch — **archive it, never delete it**: `prawduct-hook archive-plan <path> --state completed --release vX.Y.Z` (resolve `<path>` the way governance does, never a hardcoded path and never your own pick: **several live plans may declare one branch**, so choosing by eye here can archive a sibling plan that is still live work. `prawduct-hook verify-records` names the plan verbatim in its `ok:` line **when it grades a chunk**; on a plan whose boxes are all ticked — which is what this step's precondition selects for — it grades none and prints `deliverables of no chunk in scope`, naming nothing, and `--json` is no escape because `plan_graded` is null on that path. **When it names nothing, resolve by scope rather than by eye:** the live plan under `.prawduct/artifacts/` whose frontmatter `branch:` names this branch and whose `scope:` matches this PR's change-log entry, and `/prawduct:methodology planning`'s precedence when several qualify), then clear the pointer if the pointer is what named it. A plan that declares `branch:` and is the only claimant has no pointer to clear: archiving alone ends its claim. The command stamps the plan with its terminal state and moves it into `archive/`, where it stays findable by name and stops reading as live work. On gitflow, **RETAIN** the plan live until the release — the work is not shipped yet, and the release checklist's *Archive the plans this release shipped* step is where that retention ends (`documentation/release-process.md`; it runs `prawduct-hook plan-backfill`, which archives every plan whose scope the release just tagged). The Merge Flow’s *"Confirm the bookkeeping merged WITH the PR"* step says what to do with the pointer during that window, and it depends on how the plan resolves.
...
**Sequencing (run the full review ONCE):** land every **judgeable** fix — code, evidence, pointers, configs, and governance-protected prose (`skills/`, `methodology/`, `templates/`, root `CLAUDE.md`, which are judgeable despite being `.md`) — BEFORE the one cumulative run, then commit verbatim (the reviewed tree becomes the commit's tree, so the fact vouches for HEAD). For any fix AFTER the cumulative (its own findings included): **fix, then `/prawduct:critic verify-resolutions`** — its fact extends coverage over the delta at delta-review cost (1-2 min). **Commit the verified state verbatim and HEAD stays covered — but only if you ran the pass over the fix rather than after committing it.** The pass anchors committed HEAD whenever you have committed *content the prior review never saw* — committing the reviewed tree verbatim changes no content and does NOT move it, so the test is what landed, not whether you committed — and once it has anchored there it never saw your uncommitted files; committing them afterwards opens a new delta needing its own pass. (`review-cycle.md` § Verify-resolutions anchoring and demotion carries the derivation.) Run the pass on the dirty tree, then commit it whole, and there is one round instead of two — **when the pass actually grades your working tree.** It may instead anchor on a prior review and grade committed HEAD, in which case it refuses (exit 3) and names your uncommitted judgeable files as NOT REVIEWED; **there the order inverts — commit them first, then run the pass over the delta that appears, which is what the refusal tells you to do.** Do not predict which case you are in: dispatch is seconds and its own answer is authoritative, so run it and read the block. The cost of guessing wrong is one round-trip — the round this sentence exists to save. **You do not have to work out whether that pass is needed — asking is free.** When the delta holds no judgeable file and no finding the pass could resolve, dispatch refuses it in under a second (exit 3, "no review needed") instead of spending a reviewer, so a fix that landed only in `.prawduct/` state or non-governance prose costs nothing — and where the anchor left judgeable uncommitted work outside that delta, the refusal names those files, so read the block rather than the exit code alone. Run it and read the exit code; do not skip it on your own judgement, and do not re-run it in another mode after a refusal. Never re-run a full cumulative for in-scope fixes; re-run it only when the verify pass itself refuses (widened delta, lost anchor). A rebase or amend rewrites the tree — coverage gaps there always need a fresh review.
...
### Step 5: Create PR
Push branch with `-u`. Draft title and description from work context + review findings summary. **The description is where the two backlog obligations land**, and this is the only step that writes it: a `Closes #N` line for each item the PR resolves (it links the PR to the issue, and on a trunk repo whose base IS the default branch it also closes it), and — on an Issues backend, where it does not close anything — an explicit line saying the close is owed at merge. Create via `gh pr create`. Update `pr_number` in the evidence file.

## Update Flow

1. Push new commits to remote
2. Re-run the reviewer **only on a substantive delta**. Substantive = the delta since the reviewed commit contains at least one **judgeable path authored on this branch**. **The reviewed commit is the evidence file's `commit_reviewed` field — read it, never infer it.** **Cross-check it against the ledger before trusting it:** Create Step 4 appended a `review.pr` event whose `review` payload is the evidence record verbatim, so the newest such event in `.prawduct/.governance-ledger.jsonl` **whose `.review.branch` equals the current branch** carries an independent copy at `.review.commit_reviewed`. Match on the branch, not on recency alone: the ledger is per-worktree and holds every PR that worktree ever reviewed, so "the newest `review.pr` event" belongs to whichever branch reviewed last — a second PR from the same worktree would compare against a stranger and report a mismatch with certainty. (The envelope has no `branch` field; the embedded record does.) If the two disagree, the file was edited after the review — treat the delta as **substantive** and re-run, because the only edit that produces a mismatch is the one the protocol forbids. If the ledger copy is *absent* (an event appended before the field existed), you have no second witness: that is the absent-field case below, so re-run rather than trust the file alone. If the field is absent (evidence written by an older reviewer), the delta is **substantive by default**: `timestamp` plus `commits_reviewed` cannot distinguish "nothing landed since" from "a commit landed mid-review", and that is precisely the case the field exists to catch, so re-run rather than reconstruct. Two deltas are not substantive, and **neither is judged by eye** — a re-review of records and a base sync is minutes of opus for nothing to assess:
...
5. **Close the backlog items this PR resolves** — on an Issues backend this is the deferred call from Step 1d, and it is numbered here, ahead of the deletions below, because those destroy the local artifacts that record the debt. `/prawduct:backlog update <id> status=shipped closed-by=<scope>` for each item (the scope is written by `update --closed-by` as a queryable block field, in the same breath as the status change — two calls, not one). **A `Closes #N` / `Fixes #N` / `Resolves #N` keyword in the PR body does not do it for you** — GitHub fires closing keywords only for PRs merged into the repository's **default** branch, so on gitflow (feature→`develop`, default `main`) the keyword is inert and the item silently stays open; check with `gh repo view --json defaultBranchRef -q .defaultBranchRef.name` rather than assuming. The keyword itself belongs in the body written at Create Step 5, not here; if it is missing, that is a note for next time, not a reason to edit the description of a merged PR. Verify each close landed (`gh issue view <n> --json state`). On a markdown backend the archive already merged with the PR and there is nothing to do here.

   **Honest limit — this step is its own only detector.** If you merge through the GitHub UI, or the session ends at the merge, nothing downstream notices the close never fired: the branch and evidence file are gone and no local artifact records that it was owed. The reconciliation sweep that would catch it is prescribed but unbuilt, and tracked as **#712** — `documentation/backlog-service-requirements.md` **GV3** ("merged work whose item is still open… this is the price of leaving git; pay it explicitly"). Until it exists, this step running is the whole guarantee.
...
8. **Confirm the bookkeeping merged WITH the PR — there is nothing to commit here.** Create-flow Step 1d put the change-log `release=` tag, the Status ticks, (trunk) the plan retirement, and — on a markdown backend — the backlog archives in the branch, so the merge just landed them atomically. (The Issues-backend close is not bookkeeping that *rides* anywhere; it fired at step 5.) If something else was missed, fold it into the next PR that touches the repo — **never** push a bookkeeping commit to the integration branch and **never** open a housekeeping-only PR. On gitflow, release tags additionally self-converge at the release (it tags every unreleased entry regardless) and the backlog skill's reconcile step is a catch-net for unarchived items — a sweep somebody has to run, not convergence, which is why the close is a named step above rather than something left to it; on trunk there is no later release step, so the fold-into-the-next-PR rule IS the convergence — treat a missed `release=` tag as named debt for the next PR, not something that self-heals. Run `prawduct-hook resolve-base` to know which case applies:
   - **Base is `develop`** (gitflow, ahead of a batched `develop→main` release): the work is release-pending — its change-log entry carries **no `release=`** (older logs may also carry a legacy `status=` stamp; it is inert). **RETAIN** the plan live: archiving is what the plan gets at the release, not at this merge. What happens to the `active_build_plan` **pointer** depends on how the plan resolves, and the two cases differ:
     - **The plan declares `branch:`** — **clear the pointer.** Its branch is merged and deleted, so the declaration resolves for nobody and the plan reads live-but-inactive, which is exactly what RETAIN wants. Leaving the pointer aimed at it is not inert: on a branch no plan claims, `core.resolve_branch_claim` returns nothing and `core.resolve_build_plan_path` falls back to the scalar, so the finished plan goes back into force on every branch that has no plan of its own, and the briefing then nags to archive a plan you were told to retain — an advisory with no correct action. Clearing costs nothing: the plan stays live and findable, and the release archives it by scope.
     - **The plan is pointer-resolved only** — **keep the pointer**, because clearing that one is what makes governance blind to the plan. A non-blocking "archive the plan" advisory may surface in the briefing during this window; ignore it until the release ships.

     (Archiving early is not the deletion it used to be — `check-releasability` searches archived plans too — but it moves a plan that is still the live description of unshipped work, and the pointer would then name a moved file.)
