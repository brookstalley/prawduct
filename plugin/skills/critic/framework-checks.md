...
- **Concrete shape of "strengthen the generation system":** a product ships a webhook whose retries are unbounded, and the review proposes adding *"check webhook retry limits"* to a goal's bullet list. **Avoid** — that fires for one integration shape and is silent on cron loops, queue consumers, and polling clients, which fail identically. **Prefer** a rule stated at the level of the *property*: "a call that can be retried states its bound, and a call that cannot states why" — which the LLM applies to all four without being told about any of them. The tell is the diff's **shape**, not its length: a new item in an enumeration serves the product that prompted it, while a sharpened rule serves products the framework has never seen. When only the enumerated form is workable, say why the general form fails here — that is a finding about the *rule*, not about the change.
