...
3. **Inference** — the four rules (`verify-resolutions > cumulative > final > chunk`), with the short-plan deferral between the second and third: an eligible plan with code in flight answers `deferred` (rationale `short-plan deferral: …`), which dispatches nothing; a fix-in-progress or a committed bundle still gets the review rules 1–2 name. **Mid-plan** (2+ chunks of the branch's own plan unticked) a clean tree answers `chunk` over the unreviewed interval, not `cumulative` — the stage is keyed on cycle position, not commit order — or `deferred` when nothing is unreviewed or a short plan defers.
...
| **Review interval** (derived by `critic-begin`, recorded in the manifest) | Last blocker-free reviewed tree (else the merge-base on a clean tree, else HEAD's) → captured working tree | Same as `chunk` | Merge-base tree → HEAD's tree (base branch from `prawduct-hook resolve-base`) — the committed PR bundle | Prior review fact's tree → captured working tree (see "Verify-resolutions anchoring and demotion") |
