...
For each resolved item, emit a **NOTE**: "Backlog item appears resolved: [item text]. Verify it, then call `/prawduct:backlog update <id> status=shipped closed-by=<scope>` **when** the backlog skill's "When to mark shipped" rule says (it splits by backend: an Issues close waits for the merge)." Do not change status yourself — the framework never infers status; the builder makes the explicit call.
