...
**Prep work around a cumulative.** A cumulative review takes ~4-10 minutes in a fork that waits for its own reviewers, consolidates, and reports back — your turn ends long before it lands. So the only question is which findings-independent prep overlaps it, and the answer splits on one line: **does the prep move the reviewed tree?** Writes that don't — `/prawduct:learnings` for next topics, drafting the PR description, auditing the backlog, capturing deferred reflections into `.prawduct/` records — run *while* the review does, and invoking first is what buys that overlap. Anything that would move the tree runs **before** you invoke, or it invalidates the coverage you are waiting on.
