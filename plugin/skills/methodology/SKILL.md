...
- `delegation` → `${CLAUDE_SKILL_DIR}/../../methodology/delegation.md` — before splitting work across subagents. When to delegate and when to stay serial; what a delegate verifies (what proves its own change, and nothing beyond it) while the coordinator owns integration and all governance; the anti-patterns each with its tell, and what a brief must say.
