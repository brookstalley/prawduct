...
**Complete required governance at chunk boundaries, then affirmatively signal when `/clear` is safe — never *ask* whether to prepare a handoff; prepare it and say so.** When you've completed 2-3 chunks, or the user switches tasks, complete in order:

1. **Commit** (tests passing). 2. **Critic** (if medium+ and not run yet) — resolve blocking findings. 3. **Persist** pending decisions/plans to artifact files. 4. **Backlog** — file/close affected items via `/prawduct:backlog`. 5. **Update build plan Status** (mark chunks, update Context). 6. **Reflection** — confirm `.prawduct/.session-reflected` has an entry for this chunk; add a synthesis only if a cross-cutting pattern emerged. 7. **Handoff notes** — **reconcile** `.prawduct/.handoff-notes.md` to what the *next* session needs: where you stopped, what you'd do next, what would bite them. Never blind-append: drop what this batch discharged, correct what moved, keep only what still bites. A second batch rewrites the first's notes rather than stacking — a stratified file makes the reader guess which layer is live.
...
**The cheap-check gate** (Principle 24 — Retrieval Over Generation). Before any expensive or hard-to-reverse step — experiment spend, a deploy, a tuning campaign — one line: *what is the cheapest verification that could change this decision, and did I do it?* A free read or short search comes first. Tells you're generating, not retrieving: a confidence word with no citation; tuning a mechanism you haven't read; contradicting an artifact in hand; revising the same decision with no new fact (full detectors: `docs/principles.md` #24).
