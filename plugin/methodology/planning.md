...
**Several plans may declare one branch, and that is ordinary** — a `release/2-0` carrying a telemetry plan and a documentation plan, or a fix branch that grew three. Governance resolves one of them and says which, in the session briefing: the sole claimant if there is one, else the claimant with chunks left, else the one `active_build_plan` names (its remaining job — breaking a tie *within* a branch), else path order. Nothing is silent, because governing by the wrong plan looks exactly like governing correctly unless the surface names its choice. When several plans on a branch are all live work, point the scalar at whichever one you are building now.
