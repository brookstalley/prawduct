...
- **Inner-loop verification**: (unset — what to run while iterating on a change and at each chunk's Verify, before the declared suite runs once, at the boundary review — or say here that this project wants the declared suite at every chunk. In this project's own words: prawduct does not know this project's test regime and will not invent a vocabulary for it. `/prawduct:doctor` will propose a starting point from the same markers, scripts and targets it finds for `Delegate verification`.)
