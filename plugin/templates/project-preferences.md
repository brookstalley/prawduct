...
- **Inner-loop verification**: (unset — what to run while iterating on a change, before the declared suite runs at Verify and at the boundary review. In this project's own words: prawduct does not know this project's test regime and will not invent a vocabulary for it. `/prawduct:doctor` will propose a starting point from the same markers, scripts and targets it finds for `Delegate verification`.)
