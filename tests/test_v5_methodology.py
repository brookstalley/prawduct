...
LAST_MEASURED_TOKENS = {
    ...
    # NET ZERO, reading unchanged: the rule-over-instance POINTER in the Learnings
    # Cross-Check lands here without moving this number. It is the only route to
    # that rule for a SINGLE-PASS final/cumulative fork, which reads this file and
    # not `agents/critic-reviewer.md` (SKILL.md routes it to four protocol files
    # and that is not one of them). Funded in-section, not from the ceiling: the
    # ordering and two-outputs paragraphs were compressed, and the "`chunk` mode
    # skips" clause went — the Per-Mode Behavior table's Goals-skipped row already
    # owns which passes `chunk` does not run.
