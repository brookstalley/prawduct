"""Tests for prawduct-setup.py — init functionality.
...
class TestUpdateGitignore:
    ...
    def test_removes_incorrectly_ignored_managed_files(self, tmp_path: Path):
        gitignore = tmp_path / ".gitignore"
        ...
        assert result["modified"] is True
        assert ".prawduct/critic-review.md" in result["unignored"]
        content = gitignore.read_text()
        # Managed file removed from gitignore
        assert ".prawduct/critic-review.md" not in content.splitlines()
        # Session file preserved
        assert ".prawduct/.critic-findings.json" in content
        # User content preserved
        assert "node_modules/" in content

    def test_removes_multiple_incorrectly_ignored(self, tmp_path: Path):
        gitignore = tmp_path / ".gitignore"
        gitignore.write_text(
            ".prawduct/critic-review.md\n"
        ...
        assert ".prawduct/critic-review.md" not in lines
        assert ".prawduct/build-governance.md" not in lines
        assert len(result["unignored"]) == 2

    def test_remove_and_add_in_same_pass(self, tmp_path: Path):
        """Removes incorrect entries and adds missing ones in one call."""
        gitignore = tmp_path / ".gitignore"
        gitignore.write_text(".prawduct/critic-review.md\n")
...
class TestTemplatePropagation:
    ...
    def test_project_state_has_expected_structure(self):
        ...
        # v6: work_in_progress and build_plan removed — build plan Status is source of truth
...
class TestV1Detection:
    def test_setup_handles_v1_repo(self, tmp_path: Path, monkeypatch):
        """Setup auto-detects v1 repos and runs migration instead of refusing."""
        (tmp_path / ".prawduct").mkdir()
        (tmp_path / ".prawduct" / "framework-path").write_text("/some/path")

        monkeypatch.setattr(
            "sys.argv",
            ["prawduct-setup.py", "setup", str(tmp_path), "--name", "Test", "--json"],
        )
        result = main()
        assert result == 0
