...
**What is no longer here.** The change-log parser moved to `lib/change_log.py`
and plan resolution to `lib/plan_index.py`; their tests moved with them, to
`test_change_log.py` and `test_plan_index.py`. They were not weakened or
dropped — they are the contract for code that outlives this module, so keeping
a second copy pointed at the re-exports would have meant two homes for one
assertion. What remains here is the derived machinery itself: `## Status`
regeneration, the scope-rollup and release-notes views, and the `regen-views`
