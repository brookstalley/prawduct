...
- **Why it fires:** release-prep necessarily touches non-`.md` files outside `.prawduct/` (the
  `version` strings in `.claude-plugin/plugin.json` + `VERSION`). The CRT-7M2D coverage gate only
  excuses docs (`.md`) and `.prawduct/` metadata deltas since the recorded review (CRT-5D8Q — so the
  regenerated `scope_rollups` in `project-state.yaml` no longer contribute), and the version-file
  edits still read as "code changed since review" → exit 1.
