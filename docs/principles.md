...
**Durable artifacts are self-contained; ephemeral scaffolding stays out.** Build plans, chunk labels ("chunk 03"), and work-cycle names are build-time scaffolding — construction equipment (Principle 10) that is *deleted* when the work ships. A durable artifact (code, a comment, a long-lived spec) must never depend on one for its meaning: "per chunk 03" is a dangling pointer the moment the plan is retired, and every project has many "chunk 03"s. Carry the *why* inline. The exception is build-cycle bookkeeping whose job is to record the work — a change-log `chunks=` tag, a backlog `closed-by:`, an operator-verification entry — where the identifier is the audit trail, not load-bearing prose. The test: *will this reference still resolve, and mean the right thing, after the build plan is deleted?*
