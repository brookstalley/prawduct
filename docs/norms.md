...
**Structural-coverage staging — one nudge at a time.** The structural-presence hooks form a
...
(`lib/coverage_probes.structural_characteristics_recorded`) — plus layer 2's artifact-existence
gate, so exactly one speaks:
...
product that has told the framework it is one. Layer 2 keeps its artifact-existence gate (an
all-`(not relevant)` stub set still creates the files, so a product whose norms live only in
preferences + learnings is reached transitively: layer 1 nudges the stubs into existence, then
layer 2 nudges ratification). The chain advances as each layer's condition clears.
...
- **Day one, automatically:** reviewers apply the normative/descriptive test to unmarked prose
  ...
  strategy-class artifacts exist with no Direction sections (or the Enforcement table lacks
