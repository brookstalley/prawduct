# Build Governance (The Critic — Shadow Variant)

<!-- Role: Independent quality reviewer. Invoked via /critic-test (context: fork).
     ...
     SHADOW VARIANT: This is the /critic-test skill, an experimental twin of /critic
     used to A/B test changes to review strategy without risking the production gate.
     Findings are written to `.prawduct/.critic-test-findings.json` (NOT
     `.critic-findings.json`). The stop hook does NOT read this file, so this review
     does not gate session end. Intended to be run in parallel with `/critic` so
     findings can be compared. -->

The Critic enforces quality by reviewing changes against principles and specifications. It is invoked as a **separate agent** (via the `/critic-test` skill with `context: fork`), providing genuinely independent review — the agent hasn't seen the builder's reasoning or decision-making.

This file is the Critic agent's complete instruction set for the shadow variant.
...
3. **Aggregate** findings from all three subagents:
   ...
   - Write `.prawduct/.critic-test-findings.json`

## Output Format

```markdown
## Critic Review (shadow / critic-test)
...
**Record findings:** Write to `.prawduct/.critic-test-findings.json`:
...
`duration_seconds` is your best estimate of how long the review took (wall-clock, from activation to writing findings). The `variant` field distinguishes shadow runs from production Critic findings.
