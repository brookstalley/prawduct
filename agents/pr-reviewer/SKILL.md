...
- **Derived views** (when `views_enabled: true` in `project-state.yaml`): build-plan `## Status` checkboxes are a derived view of change-log.md `status=shipped` tagged entries. The canonical source is the change-log tag line; the Status block is regenerated by `product-hook regen-views`. If the PR's Status diff has no matching change-log tag-line edit, the regen wasn't run or the tag is missing → **WARNING**. Don't review the Status changes as if they were hand-curated content.
