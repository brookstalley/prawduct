...
def generate_alerts(gov: dict | None, critic: dict | None) -> list[tuple[str, str]]:
    ...
    if unreviewed:
        alerts.append((f"CRITIC({len(unreviewed)}f)", BOLD_RED))
    ...
    if critic:
        ...
        if age > 7200 or total_checks < 4:
            alerts.append(("CRITIC!", BOLD_RED))

    # Chunks without review
    chunks_no_review = gs.get("chunks_completed_without_review", 0)
    if chunks_no_review > 0:
        alerts.append((f"CHUNK({chunks_no_review})", BOLD_RED))

    # Overdue governance checkpoints
    cp_due = gs.get("governance_checkpoints_due", [])
    if cp_due:
        alerts.append((f"CP({len(cp_due)})", BOLD_RED))

    # PFR state
    if pfr.get("required"):
        if not pfr.get("diagnosis_written"):
            alerts.append(("PFR", BOLD_RED))
        elif not pfr.get("observation_file"):
            alerts.append(("PFR", YELLOW))

    # DCP state
    if dc.get("needs_classification") and not dc.get("tier"):
        alerts.append(("DCP?", YELLOW))
    elif dc.get("active"):
        if not dc.get("retrospective_completed") or not dc.get("observation_captured"):
            alerts.append(("DCP", YELLOW))
    ...
    if critic and obs_count == 0:
        ...
        if sev in ("warning", "blocking"):
            alerts.append(("OBS!", YELLOW))

    # Cap at 4 + overflow
    if len(alerts) > 4:
        overflow = len(alerts) - 4
        alerts = alerts[:4]
        alerts.append((f"+{overflow}", YELLOW))
...
def render_line1(project_state: dict | None, alerts: list[tuple[str, str]]) -> str | None:
    ...
    line = f"{ORANGE}{SHRIMP}{RESET} {BOLD_CYAN}{stage_label}{RESET}"
    ...
    if alerts:
        ...
        line += " " + " ".join(alert_strs)

    return line


def render_line2(pct: float, git: dict, duration_ms: int) -> str:
    """Render context bar + git + duration line."""
    ...
    segments = [f"{bar} {pct_str}"]
    if git_str:
        segments.append(git_str)
    segments.append(dur_str)
...
def main() -> None:
    ...
    line1 = render_line1(project_state, alerts) if is_prawduct else None
    line2 = render_line2(pct, git, duration_ms)
