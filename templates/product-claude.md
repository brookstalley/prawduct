...
4. **Write tests** alongside implementation, including **property-based tests from the chunk's requirements** (Hypothesis for Python, fast-check for TypeScript, proptest for Rust). Property tests express what the spec says should hold for generated inputs; see `build-governance.md` for details. When the suite exceeds ~30s, add pytest-xdist (`-n auto --dist loadgroup`). Organize tests into subdirectories by concern as the suite grows.
