...
### 6. Merge Hygiene
No debug code, no unintended file changes, no TODOs/placeholders, no secrets → **WARNING**. **Derived views** (when `views_enabled: true` in `project-state.yaml`): build-plan `## Status` checkboxes are a derived view of change-log.md `status=shipped` tagged entries. The canonical source is the change-log tag line; the Status block is regenerated by `product-hook regen-views`. Status diff with no matching change-log tag-line edit → **WARNING** (regen wasn't run or tag is missing). Don't review the Status changes as hand-curated content.
