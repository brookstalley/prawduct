...
**Applies only when `.prawduct/project-state.yaml` has `adversarial.enabled: true`.** Read the chunk's diff and identify surfaces from `methodology/adversarial-surfaces.md` independently of the chunk's declared `attack_surfaces` and any record of adversarial pass invocation — backstop for builder oversight. **WARNING** when a touched surface is not in the chunk's declared `attack_surfaces` (declaration stale or chunk grew beyond plan). **WARNING** when a declared surface was touched but no adversarial pass was invoked AND no documented dismissal exists in `.prawduct/.adversarial-findings.json`. **NOTE** when `attack_surfaces: []` and the diff legitimately doesn't touch the taxonomy — confirms correct declaration. Critic does NOT run adversarial — flags only; builder dispositions.
...
2. **Dispatch** three subagents in parallel, each receiving the project dir, changed files, and signals. **Tell each subagent not to run any tests — the review is code analysis only.**
   - **Correctness reviewer** — Goals 1, 2, 3, 8: "You are a Critic review subagent. Read `.prawduct/critic-review.md` for goal definitions. Review ONLY Goals 1, 2, 3, and 8 (Goal 8 only applies when `adversarial.enabled: true`). Do NOT run any tests — review through code analysis only. Project: `[dir]`. Changed files: [list]. Signals: [summary]. Report findings in the Critic output format."
