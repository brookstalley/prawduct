...
2. **Dispatch** three subagents in parallel, each receiving the project dir, changed files, and signals:
   - **Correctness reviewer** — Goals 1, 2, 3: "You are a Critic review subagent. Read `.prawduct/critic-review.md` for goal definitions. Review ONLY Goals 1-3. Project: `[dir]`. Changed files: [list]. Signals: [summary]. Report findings in the Critic output format."
   - **Design reviewer** — Goals 4, 7: same context, review ONLY Goals 4 and 7.
   - **Sustainability reviewer** — Goals 5, 6: same context, review ONLY Goals 5 and 6.
