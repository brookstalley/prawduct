...
**In-flight background work auto-defers — don't waive.** When the Critic/reflection gate would block only because a harness-tracked background `Workflow`/`Task` is still producing the diff, the Stop hook reads Claude Code's `background_tasks` array (v2.1.145+) and **defers** the block (a `GATES DEFERRED` note) instead of blocking — you need do nothing. The deferral is stateless: it re-arms once the array empties, so the Critic/reflection still fire when the work lands (deferred, never skipped). Only harness-tracked work counts; an *untrackable* wait (CI, a remote queue) leaves the array empty and still blocks. (STH-3W7F.)
