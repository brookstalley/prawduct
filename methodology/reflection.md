...
**Work cycle boundary**: Is the current work complete and the next task independent? If so, recommend `/clear` — but `/clear` is a handoff, not just a recommendation. Before recommending it: (1) persist any pending requirements, decisions, or plans to artifacts, (2) update the build plan Status section in `.prawduct/artifacts/build-plan.md` — mark completed chunks and update the Context line — so the next session's briefing and handoff file surface the task, (3) then recommend `/clear` with a note of what was persisted. See `methodology/building.md` "Session Scope Discipline" for the full protocol.
