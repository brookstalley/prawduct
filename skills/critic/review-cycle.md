...
Four modes: `chunk`, `final`, `cumulative`, `verify-resolutions`. The canonical caller is `/prawduct:critic` (no args) — the SKILL forwards any invocation arguments verbatim to `prawduct-hook infer-critic-mode <args…>` (no argument when none were delivered), which picks the mode from the explicit token / build-plan / git state, and records `mode_chosen_by` as the verbatim rationale string the helper returns (e.g., `"rule-3 final: last unchecked chunk of 4-chunk plan is in progress"`, or `"explicit-args"` when a forwarded mode token won).

Three precedence layers, highest first — all three implemented inside the helper, not in skill prose:

1. **Per-invocation override** — an explicit mode argument (`/prawduct:critic chunk` etc.). Recorded as `mode_chosen_by: "explicit-args"`. **The skill must forward, never parse (CRT-2N7V):** Skill-tool invocation of a fork-context skill does not substitute `$ARGUMENTS` (anthropics/claude-code#34164 — the forked Critic sees the literal placeholder), so the SKILL's step 1 collects arguments from whichever delivery path carried them (substituted placeholder, launch message, or trailing `ARGUMENTS:` line) and hands them to the helper. Before this, a Skill-tool-invoked explicit mode was silently ignored and inference ran instead — `mode_chosen_by` is the audit trail that distinguishes the two.
...
The envelope is shared by every event kind; the kind-specific payload nests under a family-named key (`review` for `review.critic` and `review.pr` — the PR reviewer's evidence joins via `/prawduct:pr`). Consumers key on the envelope and **skip unknown event kinds and unknown fields** — later kinds (`build.chunk`, …) join without schema change. `duration_seconds` and `actor.model` are nullable, never invented; `scope` is the build-plan feature key (passed explicitly by the reviewer; `active_build_plan` is only the fallback).

One gate consumes it today: `check-cumulative-critic` falls back to the newest qualifying `review.critic` event **from this session** (envelope `ts >= .session-start`; fail closed without the marker — CRT-8W3F) when the latest findings file is the wrong kind for the PR gate — so a chunk review after the cumulative no longer destroys the gate's evidence, while a days-old cumulative from prior work can't sneak through. The history also exists to be aggregated (reviewer-model efficiency per role, findings density per path, wall clock per feature — the questions the schema was designed to answer). The ledger is unbounded-but-tiny (one line per event); if a long-lived repo ever needs pruning, truncate oldest lines — every line is self-contained. No prune tooling exists until a real ledger needs it.
