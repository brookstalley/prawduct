...
`/prawduct:pr create` calls `prawduct-hook check-cumulative-critic` and refuses to open the PR if the gate fails. The gate requires: cumulative-mode findings file present, schema-valid, recorded in the current session (mtime later than `.session-start`), and free of unresolved BLOCKING findings. WARNING and NOTE are advisory at the PR gate — they do not block, matching the PR reviewer's own severity contract.
