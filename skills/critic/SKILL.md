...
1. **Resolve mode.**
   ...
   - Then run `prawduct-hook critic-begin` to set the critic-active session-mutation guard. (It also clears any leftover `.critic-partials/` from a prior waived or stale-failed coordinator review, so every review starts from a clean partials dir.)
