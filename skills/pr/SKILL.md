...
**Nothing about this branch's work may require a post-merge commit on the integration branch.** Protected bases take commits only by PR, and a dedicated bookkeeping-only PR is ceremony, not governance — so every status flip, archive, and derived view rides IN this branch, atomic with the merge (an abandoned PR then abandons its bookkeeping too — state can't drift). Before the review gates, confirm:
...
- **Change-log status:** run `prawduct-hook resolve-base`. When the base is an integration branch (`develop`), the Step 1c entry stays **statusless** — a statusless tagged entry IS the release-pending state; the `develop→main` release flips it to `status=shipped`. When the base is the **release surface** (trunk — `resolve-base` prints the `main` family), this merge ships the work: tag the entry `status=shipped` — plus `release=vX.Y.Z` when the product tracks versions (`release-notes.md` groups by `release=`; a shipped entry without it flips checkboxes but never appears in release notes) — and run `prawduct-hook regen-views` so the derived views ride in the PR too.
...
4. Merge using **merge-commit** strategy (`gh pr merge --merge`) — the default — or the project-configured strategy from `project-preferences.md`. Merge-commit preserves each commit's identity, so after the merge the branch's commits stay reachable from the base and its merge-base stays correct; squash collapses them to one new SHA, stranding a reused worktree branch with a pre-squash merge-base that makes every "what's new" computation over-count already-merged work.
5. Delete remote branch, switch to base branch, pull, delete local branch
6. Clean up evidence file (gitignored, local — no commit involved)
