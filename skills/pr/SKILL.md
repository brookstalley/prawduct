...
**Run `prawduct-hook check-cumulative-critic`.** This gate requires a blocking-free Critic record that vouches for HEAD: a HEAD-covering `cumulative`-mode record (reviewed over `merge-base...HEAD`; "HEAD-covering" = its `commit_reviewed` is HEAD, or only docs (`.md`) / `.prawduct` metadata changed since — CRT-7M2D, CRT-5D8Q), OR a `verify-resolutions` **chain record** extending one (`extends_cumulative` anchor X, record at HEAD, all non-`.md`, non-metadata changes in `X..HEAD` within its reviewed scope — CRT-4J8W). If it exits non-zero, **STOP**: produce the missing record (stderr says which: `/prawduct:critic cumulative` for the first review, `verify-resolutions` for the chain path below), resolve any blocking findings, then re-check. Do NOT proceed to Step 3 until this gate passes.
