...
**Run `prawduct-hook check-cumulative-critic`.** This gate requires a fresh, blocking-free `cumulative`-mode Critic record covering `merge-base...HEAD`. If it exits non-zero, **STOP**: invoke `/prawduct:critic cumulative` to produce the missing record, resolve any blocking findings, then re-check. Do NOT proceed to Step 3 until this gate passes.
