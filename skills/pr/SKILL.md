...
- **Change-log status:** run `prawduct-hook resolve-base`. When the base is an integration branch (`develop`), the Step 1c entry stays **statusless** — a statusless tagged entry IS the release-pending state; the `develop→main` release flips it to `status=shipped`. When the base is the **release surface** (trunk — `resolve-base` prints the `main` family), this merge ships the work: tag the entry `status=shipped` on the branch and run `prawduct-hook regen-views` so the derived views ride in the PR too.
...
Land all of this **before** Step 2's cumulative review (the same sequencing rule that step already states: every non-`.md` change lands before the one cumulative run, so the record covers HEAD).
...
7. **Confirm the bookkeeping merged WITH the PR — there is nothing to commit here.** Create-flow Step 1d put the backlog archives, the change-log status, the derived views, and (trunk) the plan retirement in the branch, so the merge just landed them atomically. If something was missed, it converges without a dedicated commit: fold it into the next PR that touches the repo — **never** push a bookkeeping commit to the integration branch and **never** open a housekeeping-only PR (the release flips statusless entries regardless; the backlog skill's reconcile step catches unarchived items). Run `prawduct-hook resolve-base` to know which case applies:
