...
- **`revisit:` expiry** — `revisit: YYYY-MM-DD` (machine-fired: a past date raises an advisory while the item is open) or `revisit: <event trigger text>` (walked by the janitor Norm Health sweep, never probe-fired). It is the expiry clock on a norm **exception** or **stopgap** per `docs/norms.md` (§ Exceptions expire) — a temporary non-application of a live norm files a backlog item carrying `revisit:`, so the exception expires *visibly* instead of persisting silently.
